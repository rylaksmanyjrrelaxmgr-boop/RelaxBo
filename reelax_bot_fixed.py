...
# ===================== دوال القوائم والأزرار (الجزء الثاني) =====================
...
async def filter_messages_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # المستوى 1: ردود مخصصة (قاعدة البيانات)
    reply = await db_get_reply(text.lower())
    
    # المستوى 2: ردود مضمنة (مطابقة تامة) - التصحيح
    if not reply:
        reply = ALL_REPLIES.get(text.lower())
    
    # المستوى 3: ردود مضمنة (مطابقة جزئية) - التصحيح
    if not reply:
...
async def main():
    ...
    # ===== معالجات التحكم في الردود (جديدة) =====
...
# ===================== دوال التحكم في الردود (المفقودة) =====================
    ...
    async def _get_stats(conn):
        cur = await conn.execute(
            "SELECT COUNT(*) FROM group_replies WHERE chat_id=?",
            (chat_id,)
        )
...
📌 **ملاحظة:** الردود المدمجة (200 رد) 
لا يمكن حذفها، ولكن يمكن تعطيلها."""
