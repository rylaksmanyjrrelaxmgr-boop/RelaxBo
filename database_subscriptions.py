...
"""
database_subscriptions.py - وحدة إدارة الباقات والاشتراكات والفواتير والإحالات (v7.5.4)
================================================================================
🆕 v7.5.4 (مستقل تماماً — إصلاح MRO crash):
    ✅ إزالة `from database_common import ...` (كان سبب الخطأ)
    ✅ تعريف TimeUtils محلياً (بلا استيراد خارجي)
    ✅ كشف USE_POSTGRES/USE_MYSQL من DATABASE_URL مباشرة
    ✅ CONFIG/PATHS مع fallback
    ✅ user_cache مع fallback
    ✅ _table_exists معرّف محلياً
    ✅ SubscriptionsMixin alias (توافق مع database.py)

🆕 v7.5.3:
    ✅ SubscriptionsMixin alias — database.py يستورد الاسم بالجمع

📌 v7.5.2:
    - جميع الاستعلامات مطابقة لـschema الفعل (status/end_date)
================================================================================
"""
...
USE_POSTGRES = "postgres" in _URL_LOWER or "postgresql" in _URL_LOWER
USE_MYSQL = "mysql" in _URL_LOWER or "mariadb" in _URL_LOWER


# =====================================================================
# 1) CONFIG / PATHS (مع fallback)
# =====================================================================
...
# =====================================================================
# 2) user_cache (من cache.py مع fallback)
# =====================================================================
...
try:
    ...
    class _DummyUserCache:
        """بديل بسيط إن لم يكن cache.py متاحاً."""
...
# =====================================================================
# 3) TimeUtils (نسخة محلية مطابقة لـ database.TimeUtils)
# =====================================================================
...
class TimeUtils:
    """أدوات الوقت — نسخة محلية مطابقة لـ database.TimeUtils."""
    ...
    @staticmethod
    def safe_parse_iso(date_str: Optional[Any]) -> Optional[datetime]:
        if date_str is None:
            return None
...
# =====================================================================
# 4) _table_exists (اختياري — قد تستخدمه mixins أخرى)
# =====================================================================

async def _table_exists(conn, table: str) -> bool:
    """فحص وجود جدول في قاعدة البيانات الحالية."""
    try:
        if USE_POSTGRES:
...
class SubscriptionMixin:
    ...
    يفترض أن الفئة الأم (Database) توفّر:
    - self.fetchval / self.fetchone / self.fetchall / self.execute
        ...
        try:
            async with await self._get_user_lock(user_id):
                ...
                async with self.transaction() as conn:
                    ...
                    current_end_dt = (
                        TimeUtils.safe_parse_iso(current_end)
                    ...
                    if days_granted > 0:
                            ...
                            await self._execute_with_conn(
                                ...
                                """INSERT INTO subscriptions 
                                   (user_id, plan_id, status, start_date, 
                                    end_date, provider, created_at, updated_at)
                                   VALUES ($1, $2, 'active', $3, $4, 'trial', $5, $6)""",
                                user_id, trial_plan_id,
                                TimeUtils.utc_now(), new_end,
                        ...
                        if USE_POSTGRES:
                            ...
                            await self._execute_with_conn(
                                ...
                                "UPDATE users SET subscription_end = ? "
                                "WHERE user_id = ?",
                                current_end_dt.strftime('%Y-%m-%d %H:%M:%S'),
                                user_id
                            )
    ...
    async def get_gift_plans(self) -> List[Dict]:
        return await self.fetchall(
            ...
            "FROM plans WHERE is_active = 1 AND is_gift = 1 "
            "ORDER BY price"
        )
    ...
    async def grant_subscription_days(
        ...
        plan_id: int = None, provider: str = 'manual'
    ) -> bool:
        try:
            if days <= 0:
                return False
                ...
                async with self.transaction() as conn:
                    ...
                    if not plan_id:
                        plan_id = await self._fetchval_with_conn(
                            ...
                            "AND is_active = 1 ORDER BY max_channels DESC "
                            "LIMIT 1"
                        )
                        if not plan_id:
                            plan_id = await self._fetchval_with_conn(
            ...
            logger.error(
                f"❌ Error in grant_subscription_days: {e}", exc_info=True
            )
            return False
    ...
    async def create_subscription(
        ...
        provider: str = 'xtr', provider_sub_id: str = None
    ) -> int:
        try:
            ...
            logger.error(
                f"❌ Error in create_subscription: {e}", exc_info=True
            )
            return 0
    ...
    async def expire_expired_subscriptions(self) -> None:
        try:
            ...
            logger.error(
                f"❌ Error in expire_expired_subscriptions: {e}",
                exc_info=True
            )
    ...
    async def add_payment_log(
        ...
        return await self.execute(
            ...
            "VALUES (?,?,?,?,?)",
            (user_id, provider, event_type, json.dumps(data),
             TimeUtils.utc_now())
        ) > 0
        ...
        try:
            async with await self._get_user_lock(user_id):
                async with self.transaction() as conn:
                    ...
                    if not plan:
                        logger.error(
                            f"❌ الخطة {plan_id} غير موجودة أو غير نشطة"
                        )
                        return False
                    ...
                    if not invoice:
                        logger.error(
                            f"❌ Invoice not found or not pending: "
                            f"{invoice_number}"
                        )
                        return False
                    if invoice['plan_id'] != plan_id:
                        logger.error(
                            f"❌ Plan mismatch: "
                            f"invoice plan {invoice['plan_id']} vs {plan_id}"
                        )
                        return False
    ...
    async def create_gift_code(
        ...
        try:
            ...
            logger.error(
                f"❌ Error in create_gift_code: {e}", exc_info=True
            )
            return None
                ...
                async with self.transaction() as conn:
                    ...
                    if USE_POSTGRES:
                        ...
                        count = await self._fetchval_with_conn(
                            conn,
                            "SELECT COUNT(*) FROM referrals "
                            "WHERE referrer_id = %s AND DATE(created_at) = %s",
                            referrer_id, today, default=0
                        )
                    else:
                        count = await self._fetchval_with_conn(
                    ...
                    if count >= max_ref:
                        logger.warning(
                            f"⚠️ User {referrer_id} reached "
                            f"daily referral limit"
                        )
                        return False
    ...
    async def get_referral_stats(self, user_id: int) -> Dict:
        try:
            ...
            logger.error(
                f"❌ Error in get_referral_stats: {e}", exc_info=True
            )
                ...
                async with self.transaction() as conn:
                    ...
                    if not plan_id:
                        ...
                        if not plan_id:
                            ...
                            if not plan_id:
                                logger.warning(
                                    f"⚠️ لا توجد خطة نشطة للمستخدم "
                                    f"{user_id} لصرف مكافأة الإحالة"
                                )
                                return 0
