...
- مسار سريع لـ SQLite يجعل الأداء مطابقاً للنسخة الأولى (بدون تحويلات إضافية)
- تجمع اتصالات محسّن مع دعم خادم قراءة منفصل (DATABASE_URL_READ)
- ضبط مهلات الاتصال والاستعلام عبر متغيرات البيئة
...
try:
    ...
    DB_POOL_MAX_LIFETIME = int(os.getenv("DB_POOL_MAX_LIFETIME", "3600"))
    DB_POOL_MAX_IDLE = int(os.getenv("DB_POOL_MAX_IDLE", "600"))
    DB_CONNECTION_TIMEOUT = int(os.getenv("DB_CONNECTION_TIMEOUT", "30"))
    DB_QUERY_TIMEOUT = int(os.getenv("DB_QUERY_TIMEOUT", "30"))
    DB_LOG_QUERY_TIME = os.getenv("DB_LOG_QUERY_TIME", "0").lower() in ("1", "true", "yes", "on")
...
class Database:
    ...
    def __init__(self):
        ...
        self._max_connections = DB_POOL_MAX
        self._min_connections = DB_POOL_MIN
        self._max_lifetime = DB_POOL_MAX_LIFETIME
    ...
    async def initialize(self, read_only: bool = False):
        ...
        if USE_POSTGRES:
            ...
            pool = await asyncmy.create_pool(
                ...
                pool_recycle=self._max_lifetime,
                autocommit=False,
                charset='utf8mb4',
                init_command=f"SET time_zone = '+00:00', max_execution_time = {self._query_timeout * 1000}, wait_timeout = {self._max_lifetime}"
            )
            if read_only:
                self._pool_read = pool
                logger.info(f"✅ Pool MySQL للقراءة جاهز (max={self._max_connections})")
            else:
                self._pool = pool
            ...
            if read_only:
                ...
                logger.info("✅ اتصال SQLite للقراءة جاهز (مع WAL)")
            else:
                self._sqlite_conn = await aiosqlite.connect(
                    str(PATHS.DB),
                    timeout=self._connection_timeout,
    ...
    async def close(self):
        ...
        if USE_POSTGRES and self._pool:
            ...
            self._pool = None
        if USE_POSTGRES and self._pool_read:
            await self._pool_read.close()
            self._pool_read = None
        elif USE_MYSQL and self._pool:
            self._pool.close()
            await self._pool.wait_closed()
            self._pool = None
        elif USE_MYSQL and self._pool_read:
            self._pool_read.close()
            await self._pool_read.wait_closed()
            self._pool_read = None
    ...
    async def _get_connection(self, read_only: bool = False):
        if not self._initialized:
            await self.initialize(read_only=read_only)
        if USE_POSTGRES or USE_MYSQL:
            pool = self._pool_read if (read_only and self._pool_read) else self._pool
            if pool is None:
    ...
    async def _return_connection(self, conn):
        if USE_POSTGRES or USE_MYSQL:
            ...
            try:
                if self._pool_read:
                    await self._pool_read.release(conn)
                    return
            except Exception:
                pass
    ...
    # =====================================================================
    # 6. دوال المساعد الداخلية (مع مسار سريع لـ SQLite)
    # =====================================================================
    ...
    # =====================================================================
    # 7. أقفال المستخدمين والقنوات (مع تنظيف تلقائي)
    # =====================================================================
    ...
    # =====================================================================
    # 9. دوال الترحيل والفهارس والبيانات الافتراضية والاستيراد
    # =====================================================================
    ...
    # =====================================================================
    # 10. النسخ الاحتياطي والاستعادة والصيانة
    # =====================================================================
                        ...
                        if USE_POSTGRES:
                            ...
                            await conn.execute(
                                """INSERT IGNORE INTO auto_replies 
                                   (chat_id, keyword, reply, reply_type, reply_media_id, reply_buttons, created_at, is_active, usage_count)
                                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                                (item.get('chat_id', -1), item.get('keyword', '').lower(), item.get('reply', ''),
                                 item.get('reply_type', 'text'), item.get('reply_media_id'), item.get('reply_buttons'),
                                 item.get('created_at', TimeUtils.sql_iso()), item.get('is_active', 1), item.get('usage_count', 0))
                            )
                        else:
                            await conn.execute(
                                """INSERT OR IGNORE INTO auto_replies 
    ...
    async def get_referral_stats(self, user_id: int) -> Dict:
        try:
            async with self.connection() as conn:
                ...
                if USE_POSTGRES:
                    reward = await conn.fetchrow("SELECT COALESCE(total_reward_days, 0) as total_reward, COALESCE(claimed_reward_days, 0) as claimed FROM referral_rewards WHERE user_id = $1", user_id)
                elif USE_MYSQL:
                    cursor = await conn.cursor()
        ...
        try:
            async with await self._get_user_lock(user_id):
                async with self.transaction() as conn:
                    ...
                    if USE_POSTGRES:
                        current_end = await conn.fetchval(
                            "SELECT MAX(end_date) FROM subscriptions WHERE user_id = $1 AND status = 'active' AND end_date > $2",
                            user_id, TimeUtils.utc_now()
                        )
                    elif USE_MYSQL:
                        cursor = await conn.cursor()
    ...
    async def redeem_gift_code(self, user_id: int, code: str) -> tuple:
        try:
            ...
            async with await self._get_user_lock(user_id):
                async with self.transaction() as conn:
                    if USE_POSTGRES:
                        gift_code = await conn.fetchrow("SELECT * FROM gift_codes WHERE code = $1", code)
                    elif USE_MYSQL:
                        cursor = await conn.cursor()
                    ...
                    if gift_code['creator_id'] == user_id:
                        return False, -1
                    if USE_POSTGRES:
                        plan = await conn.fetchrow("SELECT id, name, description, price, duration_days AS days FROM plans WHERE id = $1 AND is_gift = 1 AND is_active = 1", (gift_code['plan_id'],))
                    elif USE_MYSQL:
                        cursor = await conn.cursor()
    ...
    async def activate_subscription_with_payment(self, user_id: int, invoice_number: str, payment_id: str, plan_id: int) -> bool:
        try:
            async with await self._get_user_lock(user_id):
                async with self.transaction() as conn:
                    if USE_POSTGRES:
                        invoice = await conn.fetchrow("SELECT * FROM invoices WHERE number = $1 AND user_id = $2 AND status = 'pending'", invoice_number, user_id)
                    elif USE_MYSQL:
                        cursor = await conn.cursor()
                    ...
                    if invoice['plan_id'] != plan_id:
                        ...
                        return False
                    if USE_POSTGRES:
                        plan = await conn.fetchrow("SELECT * FROM plans WHERE id = $1 AND is_active = 1", plan_id)
                    elif USE_MYSQL:
                        cursor = await conn.cursor()
    ...
    # =====================================================================
    # 12. دوال إضافية للتحكم في تجمع الاتصالات
    # =====================================================================

    async def get_pool_stats(self) -> Dict:
        """إحصائيات عن تجمع الاتصالات (للرصد)"""
        stats = {
