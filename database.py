...
- جميع الإصلاحات الحرجة مُطبَّقة:
...
# =====================================================================
# 0.3 كاش داخلي للاستعلامات المتكررة
# =====================================================================
...
class Database:
    ...
    async def add_hidden_admin(self, chat_id: int, admin_id: int, added_by: int) -> bool:
        return await self.execute(
            ...
            (chat_id, admin_id, added_by, TimeUtils.utc_now()),
        ) > 0
    ...
    async def add_anonymous_admin(self, chat_id: int, anonymous_id: int, added_by: int = None, user_id: int = None) -> bool:
        return await self.execute(
            ...
            (chat_id, anonymous_id, added_by, user_id, TimeUtils.utc_now()),
        ) > 0
    ...
    async def get_banned_words(self, chat_id: int) -> List[str]:
        ...
        specific = await self.fetchall("SELECT word FROM banned_words WHERE chat_id = ?", (chat_id,))
    ...
    async def remove_banned_word(self, word: str, chat_id: int) -> bool:
        ...
        try:
            async with self.connection() as conn:
                deleted = await self._execute_with_conn(
                    conn, "DELETE FROM banned_words WHERE word = ? AND chat_id = ?", word, chat_id
                )
                if deleted > 0:
                    await self._invalidate_banned_words_local_cache(chat_id)
                    if CACHE_AVAILABLE:
    ...
    async def add_admin_log(self, chat_id: int, admin_id: int, action: str, target_id: int = None, reason: str = "") -> bool:
        return await self.execute(
            ...
            (chat_id, admin_id, action, target_id, reason, TimeUtils.utc_now()),
        ) > 0
    ...
    async def add_auto_reply(self, chat_id: int, keyword: str, reply: str,
                             reply_type: str = "text", media_id: str = None, buttons: str = None) -> bool:
    ...
    async def remove_auto_reply(self, chat_id: int, keyword: str) -> bool:
        ...
        try:
            async with self.connection() as conn:
                deleted = await self._execute_with_conn(
                    conn, "DELETE FROM auto_replies WHERE chat_id = ? AND keyword = ?", chat_id, keyword
                )
                return deleted > 0
        except Exception as e:
            ...
            async with self.transaction() as conn:
                if USE_POSTGRES:
                    ...
                    users = await self._fetchall_with_conn(
                        ...
                        """SELECT DISTINCT user_id FROM subscriptions
                           WHERE status = 'expired'
                             AND end_date > CURRENT_TIMESTAMP AT TIME ZONE 'UTC' - INTERVAL '1 day'""",
                    )
                elif USE_MYSQL:
                    await conn.execute(
                    ...
                    users = await self._fetchall_with_conn(
                        conn,
                        """SELECT DISTINCT user_id FROM subscriptions
                           WHERE status = 'expired' AND end_date > UTC_TIMESTAMP() - INTERVAL 1 DAY""",
                    )
                else:
                    await conn.execute(
        ...
        try:
            async with self.transaction() as conn:
                if USE_POSTGRES:
                    ...
                    if expired_count > 0:
                        await conn.execute(
                            """INSERT INTO penalty_archive (user_id, chat_id, penalty_type, duration, start_time, end_time, reason, issued_by, status, created_at, archived_at)
                               ...
                               FROM user_penalties WHERE status = 'active' AND end_time IS NOT NULL AND end_time <= CURRENT_TIMESTAMP AT TIME ZONE 'UTC'"""
                        )
                        await conn.execute(
                    ...
                    await conn.execute(
                        "DELETE FROM penalty_archive WHERE archived_at < CURRENT_TIMESTAMP AT TIME ZONE 'UTC' - INTERVAL '90 days'"
                    )
                elif USE_MYSQL:
                    cursor = await conn.cursor()
                    await cursor.execute(
