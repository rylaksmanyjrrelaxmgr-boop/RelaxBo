...
جميع الأنظمة: تشفير، جدولة، أمان، مسابقات، ويب، إحالات، ترجمة، تذكيرات، مخفيين، NSFW، واجهة ويب متطورة
المطور: @RelaxMgr
"""
...
async def get_db():
    ...
    if _db is None:
        ...
        await _db.executescript("""
            -- جميع الجداول كما في السابق مع إضافة جدول الجلسات
    ...
    def __init__(self):
        self.lang = {
            "ar": {
                ...
                "send_dates": "📅 أرسل التواريخ (YYYY-MM-DD)",
                ...
                "user_banned_auto": "🚫 تم حظر {user} تلقائياً",
            ...
            "en": {
                ...
                "send_dates": "📅 Send dates (YYYY-MM-DD)",
                ...
                "user_banned_auto": "🚫 {user} auto-banned",
...
# ==================== معالجات الأوامر الأساسية (مختصرة) ====================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("ref_"):
        ...
        if ref and ref["user_id"] != user.id:
            await db_execute("INSERT OR IGNORE INTO referrals(ref_id,new_id,referred_at) VALUES(?,?,?)", ref["user_id"], user.id, datetime.now().isoformat())
...
# باقي الأوامر (مختصرة جداً للطول)
async def claim(update: Update, context): 
    uid, cid = update.effective_user.id, update.effective_chat.id
    if update.effective_chat.type not in ["group","supergroup"]: return
    if not await is_authorized(context.bot, cid, uid): return await update.message.reply_text("❌")
...
async def ban_user(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ حظر {target.first_name}")
...
async def unban_user(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ فك حظر {target.first_name}")
...
async def mute_user(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"🔇 كتم {target.first_name}")
    except Exception as e: await update.message.reply_text(f"❌ {e}")
async def unmute_user(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"🔊 فك كتم {target.first_name}")
    except Exception as e: await update.message.reply_text(f"❌ {e}")
async def kick_user(update, context):
    ...
    try:
        await context.bot.ban_chat_member(cid, target.id)
        await context.bot.unban_chat_member(cid, target.id)
        await update.message.reply_text(f"👢 طرد {target.first_name}")
    except Exception as e: await update.message.reply_text(f"❌ {e}")
...
async def promote_user(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"⭐ ترقية {target.first_name}")
    except Exception as e: await update.message.reply_text(f"❌ {e}")
async def demote_user(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"⬇️ تنزيل {target.first_name}")
    except Exception as e: await update.message.reply_text(f"❌ {e}")
...
async def warn_user(update, context):
    ...
    if warns >= 3:
        ...
        await update.message.reply_text(f"⚠️ تحذير {target.first_name} ({warns}/3)")
...
async def slowmode_command(update, context):
    ...
    await update.message.reply_text(f"⏱️ وضع بطيء: {seconds}ث")
async def welcome_command(update, context):
    ...
    await update.message.reply_text("✅ ترحيب")
async def goodbye_command(update, context):
    ...
    await update.message.reply_text("✅ وداع")
async def add_banned_word(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
    if not context.args: return
    word = context.args[0].lower()
    await db_execute("INSERT OR IGNORE INTO banned_words(word,chat_id,added_by,added_at) VALUES(?,?,?,?)", word, cid, uid, datetime.now().isoformat())
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back":
        ...
        try:
            ...
            await add_points(uid, 5)
            await query.edit_message_text("✅", reply_markup=main_keyboard(uid))
        except: await query.edit_message_text("⚠️", reply_markup=main_keyboard(uid))
    elif data == "winners":
        ...
        await query.edit_message_text("🎁 30 يوم", reply_markup=main_keyboard(uid))
    elif data == "subscribe":
    ...
    if s and s["welcome_msg"]:
        for member in update.message.new_chat_members:
            if not member.is_bot:
                msg = s["welcome_msg"].replace("{user}", member.first_name).replace("{chat}", update.effective_chat.title)
...
async def on_user_leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if s and s["goodbye_msg"]:
        ...
        if not member.is_bot:
            msg = s["goodbye_msg"].replace("{user}", member.first_name).replace("{chat}", update.effective_chat.title)
            try: await update.message.reply_text(msg)
            except: pass
...
async def private_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "ADD_CH":
            ...
            try:
                ...
                if dt > datetime.now():
                    ...
                    if ch:
                        ...
                        await update.message.reply_text(f"✅ {dt.strftime('%Y-%m-%d %H:%M')}")
        ...
        if text:
            ...
            await update.message.reply_text(f"✅ تذكرة #{new_num}", reply_markup=main_keyboard(uid))
        context.user_data.pop("state")
        ...
        if uid == OWNER and text:
            ...
            await update.message.reply_text(f"📨 {sent}", reply_markup=main_keyboard(uid))
        context.user_data.pop("state")
        ...
        try:
            ...
            await db_execute("INSERT OR IGNORE INTO group_security(chat_id) VALUES(?)", cid)
            await db_execute("UPDATE group_security SET slow_mode=?, slow_mode_seconds=? WHERE chat_id=?", seconds > 0, seconds, cid)
            await update.message.reply_text(f"⏱️ {seconds}ث")
        except: await update.message.reply_text("❌")
        context.user_data.pop("state")
        ...
        cid = int(state.split("_")[2])
        await db_execute("INSERT OR IGNORE INTO banned_words(word,chat_id,added_by,added_at) VALUES(?,?,?,?)", text.lower(), cid, uid, datetime.now().isoformat())
                        ...
                        if end_str:
                            ...
                            if 0 < days <= rem["days_before"]:
                                ...
                                if int(time.time()) - last > 86400:
                                    ...
                                    await db_execute("UPDATE reminders SET last_reminder_sent=? WHERE user_id=?", int(time.time()), rem["user_id"])
                    except: pass
        except: pass
...
# قالب HTML كامل مع Bootstrap، Chart.js، WebSocket، وأزرار تحكم
...
async def main():
    ...
    logger.info(f"✅ {BOT_NAME} v20.0.3 - جميع الأنظمة متكاملة مع واجهة الويب")
