...
async def remove(update: Update, context):
    ...
    await db("DELETE FROM owners WHERE chat_id=? AND user_id=?", c, t.id); await db("DELETE FROM admins WHERE chat_id=? AND user_id=?", c, t.id)
    await update.message.reply_text(f"✅ {t.first_name}")
async def lst(update: Update, context):
    c = update.effective_chat.id
    o = await db("SELECT user_id FROM owners WHERE chat_id=?", c); a = await db("SELECT user_id FROM admins WHERE chat_id=?", c)
    m = "👑:\n"
    for r in o:
...
async def subscribe_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await db_has_active_subscription(uid):
        ...
        kb = []
        for name, d, s in STARS_PACKAGES:
