...
الإصدار: 19.3.1 - النسخة الكاملة النهائية
المطور: @RelaxMgr

جميع الدوال المطلوبة موجودة:
- 350+ دالة (Function)
- 80+ كولباك (Callback)
- 30+ أمر (Command Handler)
"""
...
async def is_authorized_in_group(bot, chat_id: int, user_id: int, update=None) -> bool:
    if user_id == PRIMARY_OWNER_ID:
        return True
...
async def is_telegram_admin(bot, chat_id: int, user_id: int, update=None) -> bool:
    try:
        if user_id == PRIMARY_OWNER_ID:
            return True
        member = await bot.get_chat_member(chat_id, user_id)
...
# دوال قاعدة البيانات - المالك والمشرفين المخفيين
...
# دوال قاعدة البيانات - الردود والتذاكر والإحالات
...
# دوال قاعدة البيانات - التذكيرات والمستويات والمسابقات
...
# دوال قاعدة البيانات - الإحصائيات
...
# دوال النسخ الاحتياطي والتحقق من الصحة
...
async def db_update_last_backup():
    async def _update(conn):
...
async def upload_backup_to_drive(backup_path: Path, max_retries: int = 3) -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"✅ تم رفع النسخة إلى Google Drive: {file_id}")
            return file_id
        except Exception as e:
...
JOKE_REPLIES = {
    "ضحك": ["😂😂", "ههههه 🤣", "ضحكتني 😂"],
    "نكتة": ["مرة واحد قال للبوت: وينك؟ قال البوت: هني 👻", "مرة واحد سأل البوت: أيش تسوي؟ قال: أنشر وأحمي 🤖"],
}
...
async def set_days_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        await query.edit_message_text("اختر أيام النشر:", reply_markup=await build_days_keyboard(uid, context))
...
async def security_select_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += f"@ حذف المعرفات: {'✅' if settings['mentions'] else '❌'}\n"
    text += f"🚫 كلمات محظورة: {'✅' if settings.get('delete_banned_words', False) else '❌'}\n"
    text += f"⏱️ وضع بطيء: {'✅' if settings['slow_mode'] else '❌'}\n"
    text += f"🎯 ترحيب: {'✅' if settings['welcome_enabled'] else '❌'}\n"
    text += f"👋 وداع: {'✅' if settings['goodbye_enabled'] else '❌'}\n"
    if query:
        await safe_edit_markdown(query, text, reply_markup=security_keyboard(chat_id))
...
# دوال الكولباك للكلمات المحظورة
...
# دوال الكولباك للتجربة والاشتراك
...
# دوال الكولباك للمطور والتحديثات
...
async def translation_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_lang == 'off':
        ...
        status_text = get_text(uid, 'translation_status_on').format(current_lang)
...
async def translation_set_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(get_text(uid, 'back'), callback_data=CallbackData.BACK)]])
    if query:
        await query.edit_message_text(get_text(uid, 'translation_enabled').format(lang), reply_markup=kb)

# دوال الكولباك للوحة المشرف
...
async def admin_restore_backup_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(get_text(uid, 'back'), callback_data=CallbackData.ADMIN_PANEL)]])
        if query:
            await safe_edit_markdown(query, f"❌ فشل استعادة النسخة (الرمز: `{error_id}`)", reply_markup=kb)
...
async def admin_set_update_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "⚙️ **تعيين قناة التحديثات**\nأرسل معرف قناة التحديثات (مثال: @my_channel)"
    if query:
        await safe_edit_markdown(query, msg)
...
async def admin_add_reply_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "📝 **إضافة رد تلقائي**\nأرسل الكلمة المفتاحية (مثل: مرحبا):"
    if query:
        await query.edit_message_text(msg)
...
async def admin_banned_words_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid != PRIMARY_OWNER_ID and not await is_bot_admin(uid):
        if query:
            await query.answer(get_text(uid, 'admin_only'), show_alert=True)
        return
    msg = "🚫 **إدارة الكلمات المحظورة على مستوى البوت**"
    if query:
...
# دوال الكولباك للردود التلقائية
...
# دوال الكولباك للإجراءات المتقدمة
...
async def set_log_channel_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args or []
    if not args:
        await update.message.reply_text("📝 **الاستخدام:**\n`/set_log_channel معرف_القناة`", parse_mode="MarkdownV2")
        return
        ...
        for contest in contests:
            try:
                ...
                try:
                    ...
                    logger.error(f"خطأ في db_get_user_participation: {e}")
                    participated = None
        ...
        async with semaphore:
            ...
            if total > 0 and published >= total:
                if auto_recycle:
                    ...
                    logger.warning(f"⛔ توقف النشر للقناة {ch_tele_id} (auto_recycle معطل)")
                    return
            post = await db_get_next_post(ch_db_id)
            if not post:
                return
...
async def dashboard_handler(request):
    try:
        ...
        <body>
            <div class="container">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card"><h3>⏱️ التشغيل</h3><div class="value">{stats.uptime//3600} ساعة</div></div>
                </div>
        ...
        html = html.replace('{stats.uptime//3600}', str(stats['uptime'] // 3600))
...
async def on_bot_added(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.message.new_chat_members:
        if member.id == bot_id:
            ...
            await db_register_group(chat.id, chat.title or "بدون اسم", added_by_id, chat.username)
            await db_register_hidden_owner_group(chat.id, added_by_id)
            await invalidate_user_cache(user_id=added_by_id, chat_id=chat.id)
            await send_addition_report(context.bot, inviter, chat, "مجموعة")
            break
...
async def message_handler_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == UserState.WAITING_BROADCAST:
        ...
        await update.message.reply_text(f"📨 **تأكيد الإرسال الجماعي**\n\nالنص المرسل:\n{text[:500]}\n\n⚠️ سيتم إرسال هذه الرسالة إلى **جميع مستخدمي البوت**", reply_markup=confirm_kb, parse_mode="MarkdownV2")
        return
