...
async def is_telegram_admin(bot, chat_id: int, user_id: int) -> bool:
    ...
    try:
        ...
        logger.error(f"خطأ في is_telegram_admin: {e}")
        return False
...
async def get_anonymous_admins(bot, chat_id: int) -> List[int]:
    ...
    try:
        ...
        logger.error(f"خطأ في get_anonymous_admins: {e}")
        return []
...
async def is_authorized_in_group(bot, chat_id: int, user_id: int, update: Update = None) -> bool:
    ...
    try:
        ...
        # نتحقق من وجود update مع sender_chat
        if update and update.effective_message and update.effective_message.sender_chat:
            ...
            # إذا كان sender_chat هو نفس المجموعة، هذا يعني مشرف مخفي
            if sender_chat.id == chat_id:
                # نتحقق من أن المرسل مشرف في المجموعة
                try:
                    member = await bot.get_chat_member(chat_id, user_id)
                    if member.status in ['creator', 'administrator']:
                        # تسجيل المشرف المخفي في قاعدة البيانات (صامت)
                    ...
                    logger.error(f"خطأ في التحقق من sender_chat: {e}")
        
        # 4. التحقق من المشرف الحقيقي عبر API تيليجرام
        try:
            member = await bot.get_chat_member(chat_id, user_id)
            if member.status in ['creator', 'administrator']:
                # تسجيل المشرف في قاعدة البيانات (صامت)
                await db_add_hidden_admin(chat_id, user_id, PRIMARY_OWNER_ID)
                await invalidate_user_cache(user_id=user_id, chat_id=chat_id)
                _admin_cache[cache_key] = True
                _admin_cache_time[cache_key] = now
        ...
        # 5. محاولة جلب قائمة المشرفين المخفيين (Anonymous Admins)
        try:
        ...
        logger.error(f"خطأ في is_authorized_in_group: {e}")
        return False
...
async def check_admin_access(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Tuple[bool, str, int]:
    ...
    المميزات:
    1. في الخاص (private): يسمح فقط لـ MAIN_ADMIN_ID أو المشرفين الخارقين
    2. في المجموعة (group/supergroup): يتحقق من المشرفين الحقيقيين والمخفيين
    3. دعم المشرفين المخفيين عبر sender_chat
    4. تسجيل صامت (بدون رسائل)
    
    الإرجاع:
        (is_admin, error_message, chat_id)
    """
    ...
    if update.effective_chat and update.effective_chat.type == 'private':
        # في الخاص، نسمح فقط للمطور الأساسي والمشرفين الخارقين
        if await is_bot_admin(user_id):
            # نحتاج إلى معرف المجموعة من السياق
            if context.args and len(context.args) > 0:
                try:
                    potential_chat_id = int(context.args[0])
                    if potential_chat_id < 0:  # مجموعات تيليجرام تبدأ بـ -
                        chat_id = potential_chat_id
                    else:
                        # إذا كان موجباً، قد يكون معرف مستخدم
                        if len(str(potential_chat_id)) > 10:
                            chat_id = potential_chat_id
                except ValueError:
                    pass
            
            # إذا لم نجد chat_id، نستخدم المحفوظ
            if chat_id == 0:
                chat_id = context.user_data.get('admin_chat_id', 0)
            
            if chat_id != 0:
                # نتحقق من الصلاحيات في المجموعة المحددة
    ...
    # ===== حالة المجموعة (Group/Supergroup) =====
    if update.effective_chat and update.effective_chat.type in ['group', 'supergroup']:
        # التحقق من chat_id
        if chat_id == 0:
            return False, "❌ لم يتم تحديد المجموعة", 0
        
        # 1. التحقق من المشرف المخفي عبر sender_chat
        if update.effective_message and update.effective_message.sender_chat:
            sender_chat = update.effective_message.sender_chat
            if sender_chat.id == chat_id:
                # هذا مشرف مخفي، نقبل الأمر تلقائياً
                # تسجيل صامت
                await db_add_hidden_admin(chat_id, user_id, PRIMARY_OWNER_ID)
                await invalidate_user_cache(user_id=user_id, chat_id=chat_id)
                return True, "", chat_id
        
        # 2. التحقق من الصلاحيات عبر is_authorized_in_group
        if await is_authorized_in_group(context.bot, chat_id, user_id, update):
            return True, "", chat_id
        
        # 3. التحقق من صلاحيات تيليجرام مباشرة (كحل أخير)
...
• دعم كامل للمشرفين المخفيين (Anonymous Admins)
• تسجيل صامت (بدون رسائل في المجموعة)
...
async def admin_auto_reply_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat_id, chat_name, username, banned in groups:
        ...
        keyboard.append([InlineKeyboardButton(f"{status} {chat_name[:30]}", callback_data=f"{CallbackData.ADMIN_AUTO_REPLY_SELECT_PREFIX}{chat_id}")])
...
async def syncgroup_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    وتسجيله كمالك مخفي إذا كان مشرفاً في المجموعة (تسجيل صامت)
    """
    ...
    try:
        ...
        if member.status in ['creator', 'administrator']:
            # تسجيل المالك المخفي (صامت - بدون رسالة في المجموعة)
            await db_register_hidden_owner_group(chat_id, user_id)
            await invalidate_user_cache(user_id=user_id, chat_id=chat_id)
            
            # تسجيل جميع المشرفين الآخرين (صامت)
            try:
            ...
            # رسالة تأكيد فقط (بدون ذكر المالك المخفي)
            await update.message.reply_text(
...
async def handle_moderation_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # استخدام check_admin_access للتحقق من الصلاحيات
...
async def on_bot_added(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج إضافة البوت إلى مجموعة - تسجيل صامت"""
    ...
    for member in update.message.new_chat_members:
        if member.id == bot_id:
            ...
            try:
                ...
                for admin in admins:
                    if admin.status in ['creator', 'administrator'] and not admin.user.is_bot:
                        await db_add_hidden_admin(chat.id, admin.user.id, PRIMARY_OWNER_ID)
                await invalidate_user_cache(chat_id=chat.id)
                print(f"✅ تم تسجيل {len(admins)} مشرف في المجموعة {chat.id} (صامت)")
            except Exception as e:
                print(f"⚠️ خطأ في تسجيل المشرفين: {e}")
            
            # إرسال رسالة ترحيب (بدون ذكر المالك المخفي)
            try:
...
async def message_handler_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق من الصور والفيديوهات والملفات
    ...
    # معالجة باقي الحالات (مختصرة للاختصار)
    # ... بقية الحالات مشابهة للكود الأصلي ...
    ...
    if context.user_data.get('support_mode') and chat.type == 'private' and text and not text.startswith('/'):
        ...
        # تحديث رقم التذكرة التالي
        async def _update_ticket_number(conn):
            await conn.execute("UPDATE settings SET value=? WHERE key='last_ticket_number'", (str(ticket_num + 1),))
            await conn.commit()
        await execute_db(_update_ticket_number)
    ...
    try:
        ...
        <body>
            <div class="container">
                ...
                <div class="footer">
                    <p>ريلاكس مانيجر v19.2.7 | © 2026</p>
                </div>
            </div>
            <script>
...
# ===================== دالة تهيئة قاعدة البيانات المحسنة =====================
async def init_db_improved():
    ...
    async with aiosqlite.connect(str(DB_PATH), timeout=DB_TIMEOUT) as conn:
        ...
        # جداول المالك والمشرفين المخفيين
        await conn.execute("""
...
# ===================== استيراد الكلمات المحظورة عند التشغيل =====================
...
async def main():
    ...
    application.add_handler(CommandHandler("mute", mute_command_handler))
    application.add_handler(CommandHandler("unmute", unmute_command_handler))
    application.add_handler(CommandHandler("warn", warn_command_handler))
    application.add_handler(CommandHandler("kick", kick_command_handler))
    application.add_handler(CommandHandler("restrict", restrict_command_handler))
    application.add_handler(CommandHandler("pin", pin_command_handler))
    ...
    # ===== معالجات الردود والكلمات المحظورة =====
    ...
    application.add_handler(CallbackQueryHandler(star_button_handler, pattern="^star_rating$"))
    application.add_handler(CallbackQueryHandler(help_button_handler, pattern="^help_needed$"))
    application.add_handler(CallbackQueryHandler(rating_handler, pattern="^rating_"))
    application.add_handler(CallbackQueryHandler(custom_button_handler, pattern="^btn_"))
    application.add_handler(CallbackQueryHandler(inline_keyboard_handler, pattern="^inline_keyboard$"))
    ...
    print(f"🚀 تم تشغيل {BOT_NAME} (الإصدار 19.2.7)")
    print("✅ جميع التحسينات المطلوبة تم تطبيقها:")
    print("   • ✅ دعم كامل للمشرفين المخفيين (Anonymous Admins)")
    print("   • ✅ تسجيل صامت (بدون رسائل في المجموعة)")
    print("   • ✅ دالة check_admin_access الموحدة")
    print("   • ✅ الفصل الذكي بين الخاص والمجموعة")
    print("   • ✅ صلاحية مطلقة لـ MAIN_ADMIN_ID")
    print("   • ✅ دعم اختيار المستخدم بثلاث طرق")
    print("   • ✅ إزالة رسائل 'تم تسجيل مالك مخفي' نهائياً")
    print("   • ✅ أزرار تفاعلية (تقييم، مساعدة، إلخ)")
