...
الإصدار: 19.3.3 - إصلاح شامل لجميع الأخطاء
المطور: @RelaxMgr
"""
...
class CallbackData:
    ...
    SECURITY_STICKERS_PREFIX = "security:stickers:"  # جديد
    SECURITY_VIDEOS_PREFIX = "security:videos:"      # جديد
...
async def db_get_security_settings(chat_id: int):
    ...
    if CACHETOOLS_AVAILABLE and chat_id in _security_cache:
        ...
        for key in ['delete_stickers', 'delete_videos']:
        ...
        async def _get(conn):
            ...
            await conn.execute(
                ...
                (chat_id, 0, 0, 1, 0, 5, 0, default_settings['welcome_text'],
                 0, default_settings['goodbye_text'], 0, 'none', 60, 0, 0)
            )
            await conn.commit()
            if CACHETOOLS_AVAILABLE:
...
async def db_create_contest(creator_id: int, title: str, description: str, prize: str, end_date: str, contest_type: str = 'raffle'):
    """إنشاء مسابقة جديدة"""
    async def _create(conn):
        cur = await conn.execute("""
            INSERT INTO contests (creator_id, title, description, prize, end_date, contest_type, created_at, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'active')
            RETURNING id
        """, (creator_id, title, description[:500], prize, end_date, contest_type, utc_now_iso()))
        row = await cur.fetchone()
        await conn.commit()
        return row[0] if row else None
    ...
    async def _get(conn):
        cur = await conn.execute("""
            SELECT c.id, c.title, c.description, c.prize, c.end_date, 
                   COUNT(cp.id) as participants, c.contest_type
            FROM contests c
...
async def db_get_contest(contest_id: int):
    ...
    async def _get(conn):
        cur = await conn.execute("""
            SELECT id, creator_id, title, description, prize, end_date, status, winner_id, contest_type
            FROM contests WHERE id = ?
        """, (contest_id,))
        row = await cur.fetchone()
        if row:
            return {
                'id': row[0], 'creator_id': row[1], 'title': row[2],
                'description': row[3], 'prize': row[4], 'end_date': row[5],
                'status': row[6], 'winner_id': row[7], 'contest_type': row[8]
            }
        return None
    return await execute_db(_get)

async def db_add_contest_participant(user_id: int, contest_id: int, answer: str = None):
    """إضافة مشارك في مسابقة"""
    async def _add(conn):
        await conn.execute("""
            INSERT OR IGNORE INTO contest_participants (user_id, contest_id, answer, joined_at)
            VALUES (?, ?, ?, ?)
        """, (user_id, contest_id, answer[:500] if answer else None, utc_now_iso()))
        await conn.commit()
    return await execute_db(_add)

async def db_get_random_participant(contest_id: int):
    """اختيار مشارك عشوائي من مسابقة"""
    async def _get(conn):
        cur = await conn.execute("""
...
# ===================== دوال قاعدة البيانات للمجموعات والردود =====================

async def db_get_hidden_admins(chat_id: int) -> List[Dict]:
    """جلب قائمة المشرفين المخفيين في مجموعة"""
    async def _get(conn):
        cur = await conn.execute("""
            SELECT admin_id, added_by, added_at 
            FROM hidden_admins 
...
async def db_get_auto_reply_settings(chat_id: int) -> dict:
    ...
    async def _get(conn):
        cur = await conn.execute("""
            SELECT enabled, only_admins, ignore_bots 
        ...
        # إعدادات افتراضية
        await conn.execute("""
            INSERT OR IGNORE INTO auto_reply_settings (chat_id, enabled, only_admins, ignore_bots)
            VALUES (?, 1, 0, 1)
        """, (chat_id,))
        await conn.commit()
        return {'enabled': True, 'only_admins': False, 'ignore_bots': True}
...
async def db_set_auto_reply_enabled(chat_id: int, enabled: bool) -> None:
    """تفعيل/تعطيل الردود التلقائية لمجموعة"""
    async def _set(conn):
        await conn.execute("""
            INSERT OR REPLACE INTO auto_reply_settings (chat_id, enabled, updated_at)
            VALUES (?, ?, CURRENT_TIMESTAMP)
...
async def db_toggle_auto_reply(chat_id: int) -> bool:
    """تبديل حالة الردود التلقائية وإرجاع الحالة الجديدة"""
    settings = await db_get_auto_reply_settings(chat_id)
    new_status = not settings['enabled']
    await db_set_auto_reply_enabled(chat_id, new_status)
    return new_status
...
async def db_get_user_auto_reply_status(user_id: int) -> bool:
    """جلب حالة الردود التلقائية لمستخدم"""
    async def _get(conn):
        cur = await conn.execute(
            "SELECT auto_reply_enabled FROM users WHERE user_id = ?",
            (user_id,)
...
async def db_set_user_auto_reply_status(user_id: int, enabled: bool) -> None:
    """تعيين حالة الردود التلقائية لمستخدم"""
    async def _set(conn):
        await conn.execute(
            "UPDATE users SET auto_reply_enabled = ? WHERE user_id = ?",
            (1 if enabled else 0, user_id)
...
async def execute_ban(bot, chat_id: int, user_id: int, until_date=None, reason: str = "", moderator_id: int = None):
    ...
    try:
        ...
        async def _log(conn):
            await conn.execute("""
                INSERT INTO moderation_log (chat_id, user_id, action, duration_minutes, moderator_id, reason, created_at) 
                VALUES (?, ?, 'ban', 0, ?, ?, ?)
...
async def check_bot_admin_permissions(bot, chat_id: int) -> dict:
    """التحقق من صلاحيات البوت في مجموعة"""
    try:
...
async def apply_penalty(bot, chat_id, user_id, settings):
    """تطبيق العقوبة التلقائية على مستخدم"""
...
async def track_chat_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.message.new_chat_members:
        if member.id == context.bot.id:
            ...
            added_by = update.message.from_user.id if update.message.from_user else 0
            
            await db_register_group(chat_id, chat_name, added_by, username)
            await db_sync_group_admins(chat_id, context.bot, added_by)
            ...
            await update.message.reply_text(
                ...
                f"🆔 المعرف: `{chat_id}`\n\n"
                f"🔐 استخدم `/syncgroup` لتفعيل جميع الميزات\n"
                f"👑 استخدم `/register_hidden_owner` لتسجيل مالك مخفي"
            )
            break
...
async def track_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تحديث المشرفين عند تغيير الصلاحيات
    ...
    if update.chat_member.new_chat_member.status == 'left' or update.chat_member.new_chat_member.status == 'kicked':
        # حذف التحذيرات
        async def _clear_warnings(conn):
            await conn.execute("DELETE FROM user_warnings WHERE user_id=? AND chat_id=?", (user_id, chat_id))
            await conn.commit()
...
async def on_bot_added(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج إضافة البوت للمجموعة (مباشر)"""
    await track_chat_add(update, context)

# ===================== دوال تنظيف الكاش =====================

async def cleanup_points_cache():
    """تنظيف نقاط المستخدمين كل ساعة"""
    while True:
        await asyncio.sleep(3600)
        now = time_module.time()
        to_delete = [uid for uid, (_, ts) in user_points_last_hour.items() if now - ts > 3600]
...
async def banned_words_add_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "🚫 **إضافة كلمة محظورة**\n\nأرسل الكلمة التي تريد إضافتها إلى قائمة الكلمات المحظورة في هذه المجموعة:\n\n📌 سيتم حذف أي رسالة تحتوي على هذه الكلمة."
    if query:
...
async def banned_words_list_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج عرض الكلمات المحظورة لمجموعة"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    ...
    if not words:
        ...
        for word, added_by, added_at in words:
            try:
                ...
                date_str = added_mecca.strftime("%Y-%m-%d")
            except:
                date_str = "?"
            msg += f"• `{word}` (أضيف بواسطة `{added_by}` في {date_str})\n"
...
async def banned_words_remove_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج حذف كلمة محظورة من مجموعة"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    ...
    words = await db_get_banned_words(chat_id)
    if not words:
        msg = "📭 **لا توجد كلمات محظورة لحذفها.**"
        if query:
...
async def admin_replies_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج إدارة الردود (لوحة الأدمن)"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    ...
    if query:
        await safe_edit_markdown(query, msg, reply_markup=get_replies_keyboard())
    else:
...
async def admin_add_reply_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "📝 **إضافة رد جديد**\n\nأرسل الكلمة المفتاحية (مثال: `مرحباً`):"
    if query:
        await query.edit_message_text(msg, parse_mode="MarkdownV2")
    else:
        await update.message.reply_text(msg, parse_mode="MarkdownV2")
...
async def admin_del_reply_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق إذا كان حذفاً مباشراً من الزر
    data = query.data if query else ""
    if data.startswith("admin_del_reply_"):
        keyword = data.replace("admin_del_reply_", "")
    ...
    msg = "🗑️ **حذف رد**\n\nأرسل الكلمة المفتاحية التي تريد حذف ردها:\n\n📌 الكلمات المتاحة:\n"
    for keyword, _ in replies:
        msg += f"• `{keyword}`\n"
...
async def nsfw_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg += f"📊 نسبة الحساسية: {threshold_percent}%\n"
    msg += f"📂 حجم الصورة الأقصى: {NSFW_MAX_FILE_SIZE // (1024*1024)} ميجابايت\n"
    msg += f"🎬 حجم الفيديو الأقصى: {NSFW_MAX_VIDEO_SIZE // (1024*1024)} ميجابايت\n"
    msg += f"🔄 عدد الإطارات: {NSFW_FRAMES}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📌 اختر الإجراء المناسب:"
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(
            f"{'🔴 تعطيل' if NSFW_ENABLED else '🟢 تفعيل'}",
            callback_data=CallbackData.NSFW_TOGGLE
        )],
        [InlineKeyboardButton(
            f"📊 تغيير الحساسية ({threshold_percent}%)",
            callback_data=CallbackData.NSFW_THRESHOLD_SET
        )],
...
async def nsfw_threshold_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "📊 **تغيير نسبة الحساسية**\n\nأرسل النسبة المئوية المطلوبة (1-100):\n\n📌 النسبة الحالية: {int(NSFW_THRESHOLD * 100)}%\n📌 كلما زادت النسبة، زادت صرامة الكشف."
...
async def admin_banned_words_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "🚫 **الكلمات المحظورة العامة**\n━━━━━━━━━━━━━━━━━━━━━━\nهذه الكلمات محظورة في جميع المجموعات."
    
    if query:
        await safe_edit_markdown(query, msg, reply_markup=get_banned_words_admin_keyboard())
    else:
...
async def admin_add_banned_word_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "🚫 **إضافة كلمة محظورة عامة**\n\nأرسل الكلمة التي تريد إضافتها إلى القائمة العامة:\n\n📌 سيتم حذف أي رسالة تحتوي على هذه الكلمة في جميع المجموعات."
...
async def admin_remove_banned_word_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not words:
        msg = "📭 **لا توجد كلمات محظورة عامة لحذفها.**"
        if query:
            await query.edit_message_text(msg)
        else:
            await update.message.reply_text(msg)
...
async def admin_del_banned_word_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج حذف كلمة محظورة عامة (مباشر من الزر)"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    ...
    data = query.data if query else ""
    if data.startswith("admin_del_banned_word_"):
        word = data.replace("admin_del_banned_word_", "")
        async def _remove(conn):
...
async def auto_reply_toggle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ تم تغيير حالة الردود التلقائية إلى: {status_text}")
...
async def auto_reply_admins_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تبديل حالة الردود للمشرفين فقط"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    ...
    status_text = "👑 مشرفين فقط" if new_status else "👥 الجميع"
    await query.edit_message_text(f"✅ تم تغيير إعدادات المستخدمين إلى: {status_text}")
    await group_settings_callback(update, context)

async def auto_reply_reset_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """إعادة تعيين الردود التلقائية لمجموعة"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    ...
    await query.edit_message_text(
        ...
        "سيتم حذف جميع الردود المخصصة واستعادة الردود الافتراضية.\n"
        "هل أنت متأكد؟",
...
async def auto_reply_confirm_reset_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _reset_replies(conn):
        await conn.execute("DELETE FROM group_replies WHERE keyword NOT IN (SELECT keyword FROM default_replies)")
        await conn.commit()
    await execute_db(_reset_replies)
    
    await query.edit_message_text("✅ تم إعادة تعيين الردود التلقائية بنجاح")
    await group_settings_callback(update, context)
...
async def auto_reply_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"📊 **إحصائيات الردود التلقائية**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📌 الحالة: {'🟢 مفعل' if settings['enabled'] else '🔴 معطل'}\n"
    msg += f"👥 المستخدمون: {'👑 مشرفين فقط' if settings['only_admins'] else '👥 الجميع'}\n"
    msg += f"🤖 تجاهل البوتات: {'✅' if settings['ignore_bots'] else '❌'}\n"
    msg += f"📝 عدد الردود المسجلة: {len(replies)}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━\n"
    
    if replies:
        msg += "📌 **أمثلة على الردود:**\n"
        for keyword, reply in list(replies)[:5]:
            msg += f"• `{keyword}` → {reply[:30]}{'...' if len(reply) > 30 else ''}\n"
        if len(replies) > 5:
            msg += f"\n... و {len(replies) - 5} ردود أخرى"
...
async def user_auto_reply_toggle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تبديل حالة الردود التلقائية لمستخدم"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    target_user_id = int(query.data.split(":")[-1]) if query else context.user_data.get('target_user_id')
    ...
    current_status = await db_get_user_auto_reply_status(target_user_id)
    new_status = not current_status
    await db_set_user_auto_reply_status(target_user_id, new_status)
...
async def admin_auto_reply_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالج إعدادات الردود التلقائية للمجموعة"""
    query = update.callback_query
    if query:
        await query.answer()
    user_id = update.effective_user.id
    ...
    if not groups:
        await query.edit_message_text(
            "📭 **لا توجد مجموعات**\n\n"
            "أضف البوت إلى مجموعة واجعلة مشرفاً أولاً."
        )
        return
    ...
    await query.edit_message_text(
        "🤖 **إعدادات الردود التلقائية للمجموعات**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        "اختر مجموعة لتعديل إعدادات الردود التلقائية:",
...
async def admin_auto_reply_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"🤖 **إعدادات الردود التلقائية**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📌 الحالة: {'🟢 مفعل' if settings['enabled'] else '🔴 معطل'}\n"
    msg += f"👥 المستخدمون: {'👑 مشرفين فقط' if settings['only_admins'] else '👥 الجميع'}\n"
    msg += f"🤖 تجاهل البوتات: {'✅' if settings['ignore_bots'] else '❌'}\n"
...
# ===================== دوال إنشاء المسابقات (معالجات الحالات) =====================
...
async def handle_contest_creation_states(update: Update, context: ContextTypes.DEFAULT_TYPE, state):
    ...
    if state == UserState.WAITING_CONTEST_TITLE:
        ...
        await update.message.reply_text(
            ...
            "أرسل تاريخ الانتهاء بالصيغة: `YYYY-MM-DD`\n"
            "مثال: `2025-12-31`\n\n"
            "🕐 سيتم إغلاق المسابقة تلقائياً في منتصف الليل من هذا التاريخ"
        )
        return True
        ...
        try:
            ...
            contest_id = await db_create_contest(
                user_id,
                context.user_data.get('contest_title', 'مسابقة جديدة'),
                context.user_data.get('contest_description', ''),
                context.user_data.get('contest_prize', 'جائزة'),
                end_date.isoformat(),
                'raffle'
            )
            ...
            context.user_data.pop('contest_end_date', None)
            context.user_data.pop('state', None)
            
            await update.message.reply_text(
                f"✅ **تم إنشاء المسابقة بنجاح!**\n\n"
                f"📌 معرف المسابقة: `{contest_id}`\n"
                f"📝 العنوان: {context.user_data.get('contest_title', '')}\n"
                f"🎁 الجائزة: {context.user_data.get('contest_prize', 'جائزة')}\n"
                f"📅 تنتهي في: {end_date.strftime('%Y-%m-%d')}\n\n"
                f"🔗 للمشاركة: استخدم /contests"
            )
            return True
        ...
        await db_add_contest_participant(user_id, contest_id, text if text != "/skip" else None)
...
async def build_days_keyboard(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    """بناء لوحة اختيار أيام الأسبوع للجدولة"""
    ...
    for i, name in enumerate(day_names):
        if i in selected:
            keyboard.append([InlineKeyboardButton(f"✅ {name}", callback_data=f"{CallbackData.SCHEDULE_DAY_SELECT_PREFIX}{i}")])
        else:
            keyboard.append([InlineKeyboardButton(f"⬜ {name}", callback_data=f"{CallbackData.SCHEDULE_DAY_SELECT_PREFIX}{i}")])
...
async def handle_moderation_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bot_perms['can_act']:
        await update.message.reply_text(f"⚠️ {bot_perms['reason']}")
        return
    
    command = update.message.text.split()[0].lower()
    args = update.message.text.split()[1:] if len(update.message.text.split()) > 1 else []
    
    # استخراج المستخدم المستهدف من الرد أو من الوسائط
    target_user_id = None
    reason = ""
    ...
    if update.message.reply_to_message and update.message.reply_to_message.from_user:
        ...
        await update.message.reply_text(f"❌ يرجى الرد على رسالة المستخدم أو إرسال معرفه\n\nمثال: `{command} 123456789 السبب`")
        return
    
    if target_user_id == user_id:
        await update.message.reply_text("❌ لا يمكنك تنفيذ هذا الإجراء على نفسك!")
        return
    
    if target_user_id == context.bot.id:
        await update.message.reply_text("❌ لا يمكنك تنفيذ هذا الإجراء على البوت!")
        return
    ...
    if command == '/ban':
        ...
        try:
            ...
            pass
        success, msg = await execute_mute(context.bot, chat_id, target_user_id, duration, reason=reason, moderator_id=user_id)
    elif command == '/warn':
...
async def register_hidden_owner_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق من أن المستخدم مشرف حقيقي في المجموعة
    try:
        bot_member = await context.bot.get_chat_member(chat_id, user_id)
        if bot_member.status not in ['creator', 'administrator']:
            await update.message.reply_text("❌ يجب أن تكون مشرفاً في المجموعة لتسجيل مالك مخفي!")
            return
    except Exception as e:
        await update.message.reply_text(f"❌ لا يمكن التحقق من صلاحياتك: {str(e)[:100]}")
        return
    ...
    try:
        ...
        if bot_member.status not in ['creator', 'administrator']:
            await update.message.reply_text("❌ يجب أن يكون البوت مشرفاً في المجموعة!")
            return
    except:
    ...
    await update.message.reply_text(
        f"✅ **تم تسجيلك كمالك مخفي للمجموعة!**\n\n"
        f"📌 المجموعة: {chat_name}\n"
        f"🆔 المعرف: `{chat_id}`\n\n"
        f"🔐 يمكنك الآن إدارة المجموعة من الخاص باستخدام الأوامر:\n"
        f"• `/add_hidden_admin` - إضافة مشرف مخفي\n"
        f"• `/remove_hidden_admin` - إزالة مشرف مخفي\n"
        f"• `/list_hidden_admins` - عرض المشرفين المخفيين"
    )
...
async def add_hidden_admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق من أن المستخدم مالك مخفي
    if not await db_is_hidden_owner(chat_id, user_id) and user_id != PRIMARY_OWNER_ID:
        await update.message.reply_text("❌ أنت لست مالكاً مخفياً لهذه المجموعة!")
        return
    ...
    if not args:
        await update.message.reply_text(
            "📝 **إضافة مشرف مخفي**\n\n"
            "الاستخدام: `/add_hidden_admin معرف_المستخدم`\n"
    ...
    if target_user_id == user_id:
        await update.message.reply_text("❌ لا يمكنك إضافة نفسك كمشرف مخفي!")
        return
    
    success = await db_add_hidden_admin(chat_id, target_user_id, user_id)
    if success:
        await update.message.reply_text(f"✅ تم إضافة المشرف المخفي `{target_user_id}` بنجاح")
    else:
...
async def remove_hidden_admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            "📝 **إزالة مشرف مخفي**\n\n"
            "الاستخدام: `/remove_hidden_admin معرف_المستخدم`\n"
    ...
    success = await db_remove_hidden_admin(chat_id, target_user_id)
    if success:
        await update.message.reply_text(f"✅ تم إزالة المشرف المخفي `{target_user_id}` بنجاح")
    else:
...
async def list_hidden_admins_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """عرض المشرفين المخفيين في مجموعة"""
    if update.effective_chat.type not in ['group', 'supergroup']:
        await update.message.reply_text("⚠️ هذا الأمر يعمل فقط في المجموعات!")
        return
    ...
    # التحقق من أن المستخدم مالك مخفي أو مشرف
    if not await db_is_hidden_owner(chat_id, user_id) and not await db_is_real_admin(chat_id, user_id):
        await update.message.reply_text("❌ ليس لديك صلاحية لعرض المشرفين المخفيين!")
        return
    ...
    for admin in admins:
        ...
        try:
            added_dt = datetime.fromisoformat(admin['added_at'])
            added_mecca = utc_to_mecca(added_dt)
            text += f"  التاريخ: {added_mecca.strftime('%Y-%m-%d %H:%M')}\n\n"
        except:
            text += f"  التاريخ: غير معروف\n\n"
...
async def sendcode_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ENABLE_2FA and ADMIN_2FA_SECRET and PYOTP_AVAILABLE:
        ...
        await update.message.reply_text(
            ...
            "أدخل رمز 2FA من تطبيق المصادقة:"
        )
        return
...
async def db_get_channel_stats(channel_db_id: int) -> dict:
    ...
    async def _get(conn):
        # إجمالي المنشورات
        cur = await conn.execute(
            "SELECT COUNT(*) as total, SUM(published) as published FROM posts WHERE channel_db_id = ?",
            (channel_db_id,)
        )
        row = await cur.fetchone()
        total_posts = row[0] if row else 0
        published_posts = row[1] if row and row[1] else 0
        
        # المشاهدات
        cur = await conn.execute(
            "SELECT COALESCE(SUM(views_count), 0) as total_views, COALESCE(AVG(views_count), 0) as avg_views FROM posts WHERE channel_db_id = ? AND published = 1",
            (channel_db_id,)
        )
        row = await cur.fetchone()
        total_views = row[0] if row else 0
        avg_views = row[1] if row else 0
        
        # آخر منشور
        cur = await conn.execute(
            "SELECT created_at FROM posts WHERE channel_db_id = ? AND published = 1 ORDER BY created_at DESC LIMIT 1",
            (channel_db_id,)
        )
        row = await cur.fetchone()
        ...
        row = await cur.fetchone()
        first_post_time = row[0] if row else None
        
        # متوسط الوقت بين المنشورات
        avg_time = 0
        if published_posts > 1:
            cur = await conn.execute("""
                SELECT AVG(julianday(created_at) - julianday(prev_created_at)) * 24
                FROM (
        ...
        if published_posts > 0:
            ...
            """, (channel_db_id,))
            row = await cur.fetchone()
            best_hour = int(row[0]) if row else 0
            ...
            """, (channel_db_id,))
            row = await cur.fetchone()
            best_day = int(row[0]) if row else 0
        
        # المنشورات اليوم والأسبوع والشهر
        today = utc_now().date().isoformat()
        week_start = (utc_now() - timedelta(days=utc_now().weekday())).date().isoformat()
        month_start = utc_now().replace(day=1).date().isoformat()
        
        cur = await conn.execute(
            "SELECT COUNT(*) FROM posts WHERE channel_db_id = ? AND published = 1 AND date(created_at) = ?",
            (channel_db_id, today)
        )
        published_today = (await cur.fetchone())[0] if (await cur.fetchone()) else 0
        
        cur = await conn.execute(
            "SELECT COUNT(*) FROM posts WHERE channel_db_id = ? AND published = 1 AND date(created_at) >= ?",
            (channel_db_id, week_start)
        )
        published_this_week = (await cur.fetchone())[0] if (await cur.fetchone()) else 0
        
        cur = await conn.execute(
            "SELECT COUNT(*) FROM posts WHERE channel_db_id = ? AND published = 1 AND date(created_at) >= ?",
            (channel_db_id, month_start)
        )
        published_this_month = (await cur.fetchone())[0] if (await cur.fetchone()) else 0
        
        # الأكثر مشاهدة
        cur = await conn.execute(
            "SELECT text, views_count FROM posts WHERE channel_db_id = ? AND published = 1 ORDER BY views_count DESC LIMIT 1",
            (channel_db_id,)
        )
        row = await cur.fetchone()
        most_viewed = {'text': row[0][:100] if row else '', 'views': row[1] if row else 0} if row else None
        
        # الأقل مشاهدة
        cur = await conn.execute(
            "SELECT text, views_count FROM posts WHERE channel_db_id = ? AND published = 1 AND views_count > 0 ORDER BY views_count ASC LIMIT 1",
            (channel_db_id,)
        )
        row = await cur.fetchone()
        least_viewed = {'text': row[0][:100] if row else '', 'views': row[1] if row else 0} if row else None
...
async def db_get_channel_stats_summary(user_id: int) -> dict:
    ...
    async def _get(conn):
        ...
        return {
            ...
            'total_views': total_views,
            'avg_views_per_channel': round(total_views / total_channels, 1) if total_channels > 0 else 0,
...
async def admin_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg += f"✅ النشطاء: {active}\n"
    msg += f"⛔ المحظورون: {banned}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
    
    # عرض أول 20 مستخدم
    for uid, ban_status in users[:20]:
        status = "⛔" if ban_status == 1 else "✅"
        try:
            user = await context.bot.get_chat(uid)
            name = user.first_name or str(uid)
        except:
            name = str(uid)
        msg += f"{status} `{uid}` - {name[:20]}\n"
    
    if len(users) > 20:
        msg += f"\n... و {len(users) - 20} مستخدم آخر"
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("⛔ المستخدمين المحظورين", callback_data=CallbackData.ADMIN_BANNED_USERS)],
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_PANEL)]
    ])
...
async def admin_banned_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not banned_users:
        ...
        msg = f"⛔ **المستخدمين المحظورين ({len(banned_users)})**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for uid, _ in banned_users[:20]:
            try:
                user = await context.bot.get_chat(uid)
                name = user.first_name or str(uid)
            except:
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_USERS)]
    ])
...
async def admin_unban_all_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, banned in users:
        if banned == 1:
            await db_set_ban(uid, False)
            unbanned += 1
    
    await query.edit_message_text(f"✅ تم إلغاء حظر {unbanned} مستخدم")
    await admin_banned_users_callback(update, context)
...
async def admin_all_channels_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        ...
        msg = f"📡 **قنوات المستخدمين ({len(channels)})**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for user_id, ch_id, ch_tele, ch_name, banned in channels[:20]:
            status = "⛔" if banned == 1 else "✅"
            name = ch_name if ch_name != ch_tele else ch_tele
            msg += f"{status} `{ch_tele}` - {name[:20]}\n"
...
async def admin_banned_channels_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channels = await db_all_users_channels(only_banned=True, limit=50)
    
    if not channels:
        msg = "✅ **لا توجد قنوات محظورة**"
    else:
        msg = f"⛔ **القنوات المحظورة ({len(channels)})**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for user_id, ch_id, ch_tele, ch_name, banned in channels[:20]:
            name = ch_name if ch_name != ch_tele else ch_tele
            msg += f"• `{ch_tele}` - {name[:20]}\n"
            msg += f"  👤 المستخدم: `{user_id}`\n"
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_ALL_CHANNELS)]
    ])
...
async def admin_groups_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not groups:
        ...
        for chat_id, chat_name, username, added_by, added_at, banned in groups[:20]:
            status = "⛔" if banned == 1 else "✅"
            name = chat_name[:20] if chat_name else str(chat_id)
            msg += f"{status} `{chat_id}` - {name}\n"
            msg += f"   👤 أضيف بواسطة: `{added_by}`\n"
...
async def admin_banned_groups_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    groups = await db_get_all_groups(only_banned=True, limit=50)
    
    if not groups:
        msg = "✅ **لا توجد مجموعات محظورة**"
    else:
        msg = f"⛔ **المجموعات المحظورة ({len(groups)})**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for chat_id, chat_name, username, added_by, added_at, banned in groups[:20]:
            name = chat_name[:20] if chat_name else str(chat_id)
            msg += f"• `{chat_id}` - {name}\n"
            msg += f"  👤 أضيف بواسطة: `{added_by}`\n"
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_GROUPS)]
    ])
...
async def admin_unban_all_groups_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await admin_banned_groups_callback(update, context)
...
async def admin_bot_channels_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not channels:
        msg = "📭 **لا توجد قنوات بوت**"
    else:
        msg = f"📢 **قنوات البوت ({len(channels)})**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for channel_id, channel_name, added_by, added_at, banned in channels[:20]:
            status = "⛔" if banned == 1 else "✅"
            name = channel_name[:20] if channel_name else str(channel_id)
            msg += f"{status} `{channel_id}` - {name}\n"
            msg += f"   👤 أضيف بواسطة: `{added_by}`\n"
...
async def admin_banned_bot_channels_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channels = await db_get_all_bot_channels(only_banned=True, limit=50)
    
    if not channels:
        msg = "✅ **لا توجد قنوات بوت محظورة**"
    else:
        msg = f"⛔ **قنوات البوت المحظورة ({len(channels)})**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for channel_id, channel_name, added_by, added_at, banned in channels[:20]:
            name = channel_name[:20] if channel_name else str(channel_id)
            msg += f"• `{channel_id}` - {name}\n"
            msg += f"  👤 أضيف بواسطة: `{added_by}`\n"
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_BOT_CHANNELS)]
    ])
...
async def admin_unban_all_bot_channels_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await admin_banned_bot_channels_callback(update, context)
...
async def admin_monitor_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    active_users = len(await db_get_all_active_users_for_report())
    
    msg = f"📊 **مراقبة المستخدمين**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"👥 إجمالي المستخدمين: {total_users}\n"
    msg += f"✅ المستخدمين النشطين (آخر 30 يوم): {active_users}\n"
    msg += f"📈 نسبة النشاط: {round(active_users / total_users * 100, 1) if total_users > 0 else 0}%\n"
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("👥 عرض المستخدمين", callback_data=CallbackData.ADMIN_USERS)],
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_PANEL)]
    ])
...
async def admin_remove_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for admin_id in admins[:10]:
        try:
            ...
            name = str(admin_id)
        msg += f"• `{admin_id}` - {name[:20]}\n"
    
    if len(admins) > 10:
        msg += f"\n... و {len(admins) - 10} مشرف آخر"
...
async def admin_ram_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ram = get_ram_usage()
    msg = f"🖥️ **حالة الرام**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📊 المستخدم: {ram['percent']}%\n"
    msg += f"📈 المستخدم: {ram['used']} / {ram['total']} GB\n"
    msg += f"📉 المتاح: {ram['free']} GB\n"
...
async def admin_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"📊 **إحصائيات عامة**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"👥 إجمالي المستخدمين: {total}\n"
    msg += f"✅ النشطاء: {total - banned}\n"
    msg += f"⛔ المحظورون: {banned}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📡 القنوات: {channels}\n"
    msg += f"👥 المجموعات: {groups}\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📝 المنشورات غير المنشورة: {posts}\n"
...
async def admin_metrics_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"📈 **مقاييس الأداء**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"⏱️ وقت التشغيل: {int(stats['uptime'] / 3600)} ساعة\n"
    msg += f"📊 إجمالي الأوامر: {stats['total_commands']}\n"
    msg += f"⚡ متوسط وقت الاستجابة: {stats['avg_response_time']:.2f} ثانية\n"
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_STATS)]
    ])
...
async def admin_backup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "💾 **إدارة النسخ الاحتياطية**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    
    if not backups:
        msg += "📭 لا توجد نسخ احتياطية"
    else:
        msg += f"📌 عدد النسخ: {len(backups)}\n\n"
        for backup in backups[:5]:
            stat = backup.stat()
            size_mb = stat.st_size / (1024 * 1024)
            name = backup.name[:30]
            msg += f"• {name}... ({size_mb:.1f} م.ب)\n"
        
        if len(backups) > 5:
            msg += f"\n... و {len(backups) - 5} نسخة أخرى"
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📥 إنشاء نسخة", callback_data=CallbackData.ADMIN_BACKUP)],
...
async def admin_restore_backup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "⚠️ **تنبيه:** الاستعادة ستستبدل قاعدة البيانات الحالية!",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def admin_restore_backup_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(f"✅ تم استعادة النسخة: {backup_name}")
    except Exception as e:
        await query.edit_message_text(f"❌ فشل الاستعادة: {str(e)[:100]}")
...
async def admin_backup_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = f"⚙️ **إعدادات النسخ الاحتياطي**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📌 النسخ التلقائي: {status_text}\n"
    msg += f"📂 عدد النسخ المحفوظة: {MAX_BACKUPS}\n"
    msg += f"☁️ Google Drive: {'🟢 مفعل' if CLOUD_BACKUP_ENABLED else '🔴 معطل'}\n"
    msg += f"🔐 التشفير: {'🟢 مفعل' if DB_ENCRYPTION else '🔴 معطل'}\n"
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(
            f"{'🔴 تعطيل' if auto_backup else '🟢 تفعيل'} النسخ التلقائي",
            callback_data=CallbackData.ADMIN_TOGGLE_AUTO_BACKUP
        )],
...
async def admin_change_interval_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    current_seconds = await db_get_publish_interval_seconds()
    current_minutes = current_seconds // 60
    
    context.user_data['state'] = UserState.WAITING_INTERVAL_MINUTES
    context.user_data['admin_interval'] = True
    
    msg = f"⏱️ **تغيير وقت النشر العام**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"📌 الوقت الحالي: {current_minutes} دقيقة\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += f"أرسل الوقت الجديد بالدقائق (مثال: 30):"
...
async def admin_send_update_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channel:
        ...
        msg += f"━━━━━━━━━━━━━━━━━━━━━━\n"
        msg += f"أرسل نص التحديث (يدعم HTML):"
    else:
        msg = "❌ **لم يتم تعيين قناة تحديثات**\n\n"
        msg += "استخدم زر '⚙️ قناة التحديثات' أولاً"
...
async def admin_set_update_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "📢 **تعيين قناة التحديثات**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += "أرسل معرف القناة:\n"
    msg += "• مثال: `@my_channel`\n"
    msg += "• مثال: `-1001234567890`\n\n"
    msg += "⚠️ تأكد من أن البوت مشرف في القناة ويملك صلاحية الإرسال"
...
async def admin_show_update_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channel:
        msg = f"📢 **قناة التحديثات الحالية**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        msg += f"📌 القناة: @{channel}\n"
        msg += f"🔗 الرابط: https://t.me/{channel}"
    else:
        msg = "📢 **لم يتم تعيين قناة تحديثات**"
...
async def admin_set_force_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    current = await db_get_force_subscribe_channel()
    msg = "🔒 **تعيين قناة الاشتراك الإجباري**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    if current:
        msg += f"📌 القناة الحالية: @{current}\n"
    else:
        msg += "📌 لا توجد قناة حالياً\n"
    msg += f"━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += "أرسل معرف القناة (مثال: @my_channel):"
...
async def admin_broadcast_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "📨 **إرسال رسالة جماعية**\n━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += "أرسل النص الذي تريد إرساله لجميع المستخدمين:\n\n"
    msg += "⚠️ سيتم إرسال هذه الرسالة لجميع مستخدمي البوت"
...
async def admin_manage_sendcode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if allowed_user:
        try:
            user = await context.bot.get_chat(allowed_user)
            name = user.first_name or str(allowed_user)
        except:
            name = str(allowed_user)
        msg += f"👤 المستخدم المصرح له: `{allowed_user}` ({name})\n"
    else:
        msg += "📌 لا يوجد مستخدم مصرح له\n"
    
    msg += f"\n📌 المستخدم المصرح له يمكنه استلام كود البوت باستخدام /sendcode"
...
async def admin_show_log_channel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channel_id:
        msg = f"📋 **قناة التقارير**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        msg += f"📌 معرف القناة: `{channel_id}`\n"
        try:
            chat = await context.bot.get_chat(int(channel_id))
            msg += f"📌 اسم القناة: {chat.title or 'غير معروف'}\n"
        except:
            pass
    else:
        msg = "📋 **لم يتم تعيين قناة تقارير**"
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("⚙️ تعيين", callback_data=CallbackData.ADMIN_SET_LOG_CHANNEL)],
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_PANEL)]
    ])
...
async def admin_toggle_channel_ban_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _toggle(conn):
        ...
        row = await cur.fetchone()
        current_banned = row[0] if row else 0
        new_banned = 1 if current_banned == 0 else 0
        await conn.execute("UPDATE user_channels SET banned=? WHERE id=?", (new_banned, channel_db_id))
        await conn.commit()
        return new_banned
    new_banned = await execute_db(_toggle)
    
    status = "محظورة" if new_banned == 1 else "نشطة"
    await query.edit_message_text(f"✅ تم تغيير حالة القناة إلى: {status}")
    await admin_all_channels_callback(update, context)
...
async def admin_toggle_group_ban_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _toggle(conn):
        ...
        current_banned = row[0] if row else 0
        new_banned = 1 if current_banned == 0 else 0
        await conn.execute("UPDATE bot_groups SET banned=? WHERE chat_id=?", (new_banned, chat_id))
        await conn.commit()
        return new_banned
    new_banned = await execute_db(_toggle)
...
async def health_check_handler(request):
    """معالج Health Check للخادم"""
    try:
        db_healthy = await check_database_health()
        tg_healthy = await check_telegram_health()
        ram = get_ram_usage()
        
        status = "healthy" if db_healthy and tg_healthy else "unhealthy"
        return web.json_response({
            'status': status,
            'timestamp': utc_now_iso(),
            'database': 'healthy' if db_healthy else 'unhealthy',
            'telegram': 'healthy' if tg_healthy else 'unhealthy',
...
class UserState:
    ...
    WAITING_INTERVAL_MINUTES = "waiting_interval_minutes"
    WAITING_INTERVAL_HOURS = "waiting_interval_hours"
    WAITING_INTERVAL_DAYS = "waiting_interval_days"
    WAITING_DATES = "waiting_dates"
    WAITING_PUBLISH_TIME = "waiting_publish_time"
    WAITING_SCHEDULE_POST = "waiting_schedule_post"
    WAITING_REMINDER_DAYS = "waiting_reminder_days"
    WAITING_UPDATE_TEXT = "waiting_update_text"
    WAITING_UPDATE_CHANNEL = "waiting_update_channel"
    WAITING_FORCE_CHANNEL = "waiting_force_channel"
    WAITING_BROADCAST = "waiting_broadcast"
    WAITING_SENDCODE_USER = "waiting_sendcode_user"
    WAITING_LOG_CHANNEL = "waiting_log_channel"
    WAITING_KEYWORD = "waiting_keyword"
    WAITING_REPLY = "waiting_reply"
    WAITING_ADMIN_ID_ADD = "waiting_admin_id_add"
    WAITING_ADMIN_ID_REMOVE = "waiting_admin_id_remove"
    WAITING_GROUP_BANNED_WORD = "waiting_group_banned_word"
    WAITING_REMOVE_GROUP_BANNED_WORD = "waiting_remove_group_banned_word"
    WAITING_GLOBAL_BANNED_WORD = "waiting_global_banned_word"
    WAITING_REMOVE_GLOBAL_BANNED_WORD = "waiting_remove_global_banned_word"
    WAITING_NSFW_THRESHOLD = "waiting_nsfw_threshold"
    WAITING_BAN_USER = "waiting_ban_user"
    WAITING_MUTE_USER = "waiting_mute_user"
    WAITING_WARN_USER = "waiting_warn_user"
    WAITING_KICK_USER = "waiting_kick_user"
    WAITING_RESTRICT_USER = "waiting_restrict_user"
    WAITING_UNBAN_USER = "waiting_unban_user"
    WAITING_PIN_MESSAGE = "waiting_pin_message"
    WAITING_CONTEST_TITLE = "waiting_contest_title"
    WAITING_CONTEST_DESCRIPTION = "waiting_contest_description"
    WAITING_CONTEST_PRIZE = "waiting_contest_prize"
    WAITING_CONTEST_END_DATE = "waiting_contest_end_date"
    WAITING_CONTEST_ANSWER = "waiting_contest_answer"
...
async def filter_messages_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if NSFW_ENABLED:
        ...
        elif update.message.video and CV2_AVAILABLE:
            file = await context.bot.get_file(update.message.video.file_id)
...
async def init_db_improved():
    async with aiosqlite.connect(str(DB_PATH), timeout=DB_TIMEOUT) as conn:
        ...
        try:
            ...
            logger.error(f"خطأ في تحديث group_security: {e}")
        ...
        try:
            ...
            logger.error(f"خطأ في تحديث users: {e}")
        ...
        try:
            ...
            logger.error(f"خطأ في تحديث posts: {e}")
        ...
        try:
            ...
            logger.error(f"خطأ في تحديث contests: {e}")
        ...
        try:
            ...
            logger.error(f"خطأ في تحديث schedule: {e}")
...
async def main():
    ...
    print("   • ✅ **إضافة جدول group_admins لتخزين المشرفين الحقيقيين**")
    print("   • ✅ **مزامنة المشرفين تلقائياً عند إضافة البوت وعند /syncgroup**")
    print("   • ✅ **رسالة ترويجية للعضو العادي عند إرسال أمر**")
    print("   • ✅ **إضافة دالة check_admin_access للتحقق من صلاحية المشرف**")
    ...
    print("   • ✅ **إضافة زر حذف الملصقات 🎴**")
    print("   • ✅ **إضافة زر حذف الفيديوهات 🎬**")
    print("   • ✅ **إصلاح safe_int لتحويل القيم بأمان**")
    print("   • ✅ **إضافة ensure_security_columns للتأكد من الأعمدة**")
