...
=====================================================================
الإصدار: v7.7.0 (قوائم فرعية للأمان)
=====================================================================
🆕 v7.7.0:
    ✅ تقسيم صفحة الأمان إلى 8 قوائم فرعية
    ✅ sec_menu_auto / advanced / welcome / warns / penalties
    ✅ sec_menu_violations / actions / stats
    ✅ sec_back_main للرجوع للرئيسية

🆕 v7.5.25:
    ✅ إصلاح SyntaxError في _handle_admin
...
class CallbackHandlers:
    ...
    async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            if base_data == "status_only":
                await safe_edit(query, await _trans('status', lang, "📊 الحالة"), bot=context.bot)
                return
            ...
            if base_data == "gift_plans":
                ...
                if not plans:
                    ...
                    return
                kb = [[InlineKeyboardButton(f"🎁 {p['days']} يوم - {p['price']} ⭐",
                                            callback_data=f"buy_gift:{p['id']}")] for p in plans]
            ...
            if base_data == CB.CONTEST_WINNERS:
                ...
                text = "🏆 الفائزون\n\n" + "\n".join(
                    f"• {w['title']} - {_mask_id(w['winner_id'])}" for w in winners) if winners else "📭 لا يوجد"
                await safe_edit(query, text, reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("🔙 رجوع", callback_data=CB.BACK)]]), bot=context.bot)
                StateManager.clear(user_id)
                return
    ...
    async def _show_main_menu_inline(query, context, user_id) -> bool:
        try:
            ...
            await safe_edit(query, title, reply_markup=kb, bot=context.bot)
            return True
        except Exception as e:
    ...
    # =================================================================
    # معالجة الأزرار ذات الصيغة الخاصة
    # =================================================================
    ...
    async def _handle_parameterized(update, context, query, user_id, lang, data) -> bool:
        try:
            ...
            # 🆕 القوائم الفرعية للأمان
            if data.startswith("sec_menu_") or data.startswith("sec_back_main"):
                await CallbackHandlers._handle_security(update, context, query, user_id, lang)
                return True
            ...
            if data.startswith("set_warn_count:"):
                ...
                await safe_edit(
                    ...
                    reply_markup=KeyboardFactory.build("sec_warns_menu", chat_id=chat_id, lang=lang),
                    bot=context.bot,
                )
                return True
            ...
            if data.startswith("set_duration:"):
                ...
                settings = await DB.get_security_settings(chat_id)
                await safe_edit(query, KeyboardFactory._format_security_text(settings),
                                reply_markup=KeyboardFactory.build("sec_penalties_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return True
            ...
            if data.startswith("sec_set_antiflood_penalty:"):
                ...
                if penalty_type in ('ban', 'mute', 'kick', 'restrict', 'none'):
                    ...
                    settings = await DB.get_security_settings(chat_id)
                    await safe_edit(query, KeyboardFactory._format_security_text(settings),
                                    reply_markup=KeyboardFactory.build("sec_advanced_menu", chat_id=chat_id, lang=lang),
                                    bot=context.bot)
                else:
                    await safe_edit(query, "❌ نوع عقوبة غير صالح", bot=context.bot)
                return True
            ...
            if data.startswith("sec_set_violation_penalty:"):
                ...
                if penalty_type in ('ban', 'mute', 'kick', 'restrict', 'none'):
                    ...
                    settings = await DB.get_security_settings(chat_id)
                    await safe_edit(query, KeyboardFactory._format_security_text(settings),
                                    reply_markup=KeyboardFactory.build("sec_violations_menu", chat_id=chat_id, lang=lang),
                                    bot=context.bot)
                else:
                    await safe_edit(query, "❌ نوع عقوبة غير صالح", bot=context.bot)
                return True
    ...
    async def _render_referral(query, context, user_id, lang):
        ...
        text = (f"🔗 نظام الإحالات\n\n📎 رابطك:\n{link}\n\n"
                f"👥 المُحالين: {stats['total']}\n🎁 الأيام المتاحة: {stats['available']} يوم")
        kb = InlineKeyboardMarkup([
    ...
    async def _handle_buy_subscription(update, context, query, user_id, data):
        ...
        try:
            await context.bot.send_invoice(
                chat_id=user_id, title=f"💎 {plan['name']}",
    ...
    async def _handle_post_publish(update, context, query, user_id):
        ...
        if ch_info:
            task = asyncio.create_task(CallbackHandlers._publish_single(
    ...
    async def _show_groups_list(update, context, query, user_id, lang):
        ...
        for i, g in enumerate(groups, 1):
            ...
            kb.append([
                ...
                InlineKeyboardButton(f"{number} 🗑️ حذف", callback_data=f"grp_del:{gid}"),
            ])
        kb.append([InlineKeyboardButton("🔙 رجوع", callback_data=CB.BACK)])
    ...
    async def _show_channel_list(update, context, query, user_id, lang=None):
        ...
        for ch in page_channels:
            ...
            kb.append([
                ...
                InlineKeyboardButton("📅 جدولة", callback_data=f"sched_open:{ch['id']}"),
            ])
            kb.append([
                InlineKeyboardButton("📊 إحصائيات", callback_data=f"{CB.CH_STATS}:{ch['id']}"),
                InlineKeyboardButton("🗑️ حذف", callback_data=f"{CB.CH_DEL}:{ch['id']}"),
            ])
        nav = []
        if page > 0:
    ...
    # =================================================================
    # معالجات الأمان (مع القوائم الفرعية)
    # =================================================================
    ...
    async def _handle_security(update, context, query, user_id, lang=None, return_to_main=False):
        ...
        try:
            ...
            # ═══════════════════════════════════════════════
            if action == "menu_auto":
                await safe_edit(query, "🗑️ <b>الحذف التلقائي</b>\n\nاختر ما تريد حذفه تلقائياً:",
                                reply_markup=KeyboardFactory.build("sec_auto_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return

            if action == "menu_advanced":
                await safe_edit(query, "⚙️ <b>الأمان المتقدم</b>\n\nإعدادات الفيضان، الليل، الوضع البطيء...",
                                reply_markup=KeyboardFactory.build("sec_advanced_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return

            if action == "menu_welcome":
                await safe_edit(query, "👋 <b>الترحيب والانضمام</b>\n\nإعدادات الترحيب والوداع والانضمام:",
                                reply_markup=KeyboardFactory.build("sec_welcome_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return

            if action == "menu_warns":
                await safe_edit(query, "⚠️ <b>التحذيرات</b>\n\nإعدادات التحذيرات والعقوبات:",
                                reply_markup=KeyboardFactory.build("sec_warns_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return

            if action == "menu_penalties":
                await safe_edit(query, "🚫 <b>العقوبات</b>\n\nأنواع العقوبات ومددها:",
                                reply_markup=KeyboardFactory.build("sec_penalties_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return

            if action == "menu_violations":
                await safe_edit(query, "🚨 <b>المخالفات</b>\n\nإعدادات المخالفات والعقوبات التلقائية:",
                                reply_markup=KeyboardFactory.build("sec_violations_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return

            if action == "menu_actions":
                await safe_edit(query, "🛠️ <b>الإجراءات</b>\n\nالإجراءات المتقدمة والسجل:",
                                reply_markup=KeyboardFactory.build("sec_actions_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return
            ...
            if action == "menu_stats":
                ...
                await safe_edit(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_back_main:{chat_id}")]
                    ]),
                    bot=context.bot,
                )
                return
            ...
            if action in toggle_map:
                ...
                auto_actions = ('links', 'mentions', 'video', 'audio', 'anim',
                                'service', 'doc', 'sticker', 'forward', 'poll',
                                'game', 'voice', 'videonote')
                advanced_actions_set = ('flood', 'night', 'slow', 'nsfw', 'maxlen')
                welcome_actions = ('welcome', 'goodbye', 'approve_join', 'reject_join')

                if action in auto_actions:
                    menu_name = "sec_auto_menu"
                    menu_title = "🗑️ <b>الحذف التلقائي</b>"
                elif action in advanced_actions_set:
                    menu_name = "sec_advanced_menu"
                    menu_title = "⚙️ <b>الأمان المتقدم</b>"
                elif action in welcome_actions:
                    menu_name = "sec_welcome_menu"
                    menu_title = "👋 <b>الترحيب والانضمام</b>"
                else:
                    menu_name = "security"
                    menu_title = "🔐 <b>الأمان</b>"

                text_with_state = (f"{menu_title}\n\n"
                                   f"آخر تعديل: <b>{action}</b> = {'✅ مفعّل' if new_val else '❌ معطّل'}")
                await safe_edit(query, text_with_state,
                                reply_markup=KeyboardFactory.build(menu_name, chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return
            ...
            if action == "warn":
                kb = InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_warns:{chat_id}")],
                ])
            ...
            if action == "warn_toggle":
                ...
                await safe_edit(query,
                                "⚠️ <b>التحذيرات</b>\n\n"
                                f"آخر تعديل: تحذيرات = {'✅ مفعّل' if new_val else '❌ معطّل'}",
                                reply_markup=KeyboardFactory.build("sec_warns_menu", chat_id=chat_id, lang=lang),
                                bot=context.bot)
                return
            ...
            if action == "del_pen":
                kb = InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_penalties:{chat_id}")],
                ])
    ...
    async def _show_banned_words_menu(update, context, query, chat_id, lang):
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_auto:{chat_id}")],
        ])
    ...
    async def _show_penalty_type_selection(update, context, query, chat_id, lang, setting_key):
        ...
        kb.append([InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_back_main:{chat_id}")])
    ...
    async def _show_penalty_durations(update, context, query, chat_id, lang, penalty_type='mute'):
        ...
        for i in range(0, len(durations), 2):
            ...
            kb.append(row)
        kb.append([InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_penalties:{chat_id}")])
        type_name = {
    ...
    async def _show_violation_penalties(update, context, query, chat_id, lang):
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_violations:{chat_id}")],
        ])
    ...
    async def _show_antiflood_settings(update, context, query, chat_id, lang):
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_advanced:{chat_id}")],
        ])
    ...
    async def _show_advanced_actions(update, context, query, chat_id, lang):
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_actions:{chat_id}")],
        ])
    ...
    async def _show_admin_logs(update, context, query, chat_id, lang):
        ...
        text = "📋 سجل المشرفين\n\n" + "\n".join(
            f"• {l.get('admin_id')} → {l.get('action')}" for l in logs) if logs else "📭 لا يوجد"
        await safe_edit(query, text, reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("🔙 رجوع", callback_data=f"sec_menu_actions:{chat_id}")]]), bot=context.bot)
    ...
    async def _handle_admin(update, context, query, user_id, lang=None):
        ...
        try:
            ...
            if data == CB.ADMIN_BANNED:
                ...
                text = ("⛔ المحظورين\n\n" + "\n".join(str(u.get('user_id', '?')) for u in banned_users)) if banned_users else "📭 لا يوجد محظورون"
                kb = InlineKeyboardMarkup([
            ...
            if data == CB.ADMIN_LIST_ADMINS:
                ...
                text = "👑 المشرفون\n\n" + "\n".join(f"• {a.get('user_id', '?')}" for a in (admins or [])) if admins else "📭 لا يوجد"
                kb = InlineKeyboardMarkup([
            ...
            if data == CB.ADMIN_INVOICES:
                ...
                text = "🧾 الفواتير\n\n" + "\n".join(
                    ...
                    for i in (invoices or [])) if invoices else "📭 لا توجد"
                kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=CB.ADMIN)]])
                await safe_edit(query, text, reply_markup=kb, bot=context.bot)
                return
            ...
            if data == CB.ADMIN_TICKETS:
                ...
                text = "🎫 التذاكر المعلقة\n\n" + "\n".join(
                    ...
                    for t in (tickets or [])[:10]) if tickets else "📭 لا توجد تذاكر"
                kb = InlineKeyboardMarkup([
            ...
            if data == CB.ADMIN_PAYMENT_LOGS:
                ...
                text = "💳 سجلات الدفع\n\n" + "\n".join(
                    ...
                    for l in (logs or [])) if logs else "📭 لا توجد"
                await safe_edit(query, text, bot=context.bot)
                return
            ...
            if data == CB.ADMIN_BANNED_CH:
                ...
                text = "🚫 القنوات المحظورة\n\n" + "\n".join(
                    ...
                    for c in (banned_channels or [])) if banned_channels else "📭 لا توجد"
                kb = InlineKeyboardMarkup([
            ...
            if data == CB.ADMIN_BANNED_GR:
                ...
                text = "🚫 المجموعات المحظورة\n\n" + "\n".join(
                    ...
                    for g in (banned_groups or [])) if banned_groups else "📭 لا توجد"
                kb = InlineKeyboardMarkup([
            ...
            if data == CB.ADMIN_REPLIES:
                ...
                text = "💬 الردود العامة\n\n" + "\n".join(
                    f"• {r.get('keyword', '?')}" for r in (replies or [])) if replies else "📭 لا توجد"
                kb = InlineKeyboardMarkup([
            ...
            if data == CB.ADMIN_BANNED_WORDS:
                ...
                text = "🚫 الكلمات المحظورة العامة\n\n" + "\n".join(
                    f"• {w}" for w in (words or [])[:30]) if words else "📭 لا توجد"
                kb = InlineKeyboardMarkup([
            ...
            if data == "admin_list_banned":
                ...
                text = "📋 قائمة الكلمات المحظورة العامة\n\n" + "\n".join(
                    f"• {w}" for w in (words or [])) if words else "📭 لا توجد"
                await safe_edit(query, text, bot=context.bot)
                return
    ...
    async def _handle_auto_reply(update, context, query, user_id, lang=None):
        ...
        try:
            ...
            if action == "list":
                ...
                text = "📋 الردود\n\n" + "\n".join(f"• {r.get('keyword', '?')}" for r in (rows or [])) if rows else "📭 لا يوجد"
                await safe_edit(query, text, reply_markup=InlineKeyboardMarkup(
        ...
        try:
            if prefix.startswith("ban_"):
                ...
                if action == "list":
                    ...
                    text = "🚫 الكلمات\n\n" + "\n".join(f"• {w}" for w in (words or [])[:50]) if words else "📭 لا يوجد"
                    await safe_edit(query, text, bot=context.bot)
                    return
                if action == "rem":
            ...
            if prefix.startswith("act_"):
                ...
                if action == "pin":
                    if not update.message or not update.message.reply_to_message:
                        await safe_edit(query, "📌 استخدم الأمر /pin بالرد على الرسالة.", bot=context.bot)
                        return
                    StateManager.set(user_id, UserState.WAIT_PIN)
                    context.user_data['adv_chat'] = chat_id
                    await safe_edit(query, "📌 قم بالرد على الرسالة المطلوب تثبيتها:", bot=context.bot)
                    return
                if action == "log":
            ...
            if prefix.startswith("pen_"):
                ...
                if action in penalty_types:
                    ...
                    settings = await DB.get_security_settings(chat_id)
                    await safe_edit(query, KeyboardFactory._format_security_text(settings),
                                    reply_markup=KeyboardFactory.build("sec_penalties_menu", chat_id=chat_id, lang='ar'),
                                    bot=context.bot)
                    return
                await safe_edit(query, "⚠️ غير معروف", bot=context.bot)
                return
    ...
    async def _handle_contests(update, context, query, user_id):
        ...
        try:
            ...
            if data == CB.CONTEST_WINNERS:
                winners = await DB.get_contest_winners(10)
                text = "🏆 الفائزون\n\n" + "\n".join(
                    f"• {w.get('title', '?')} - {w.get('winner_id', '?')}"
                    for w in (winners or [])) if winners else "📭 لا يوجد"
                await safe_edit(query, text, bot=context.bot)
                StateManager.clear(user_id)
                return
