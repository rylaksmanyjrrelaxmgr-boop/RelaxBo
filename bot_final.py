...
# -*- coding: utf-8 -*-
"""
ريلاكس مانيجر - الإصدار النهائي الكامل 20.0.3
جميع الأنظمة مع واجهة ويب متطورة
"""
import os, sys, asyncio, time, re, json, hashlib, base64, io, csv, shutil, logging, sqlite3, random, secrets, traceback
...
def install_pkg(pkg):
    try: __import__(pkg.split("==")[0]); return True
        ...
        try: import subprocess; subprocess.check_call([sys.executable, "-m", "pip", "install", pkg, "--quiet"]); return True
        except: return False

for pkg in ["python-telegram-bot>=21.0","aiosqlite==0.19.0","python-dotenv==1.0.0","aiohttp==3.9.0","deep-translator==1.11.0","cryptography==41.0.7","watchdog>=3.0.0","cachetools==5.3.0","jinja2==3.1.2","bleach==6.0.0","Pillow==10.0.0","zstandard==0.21.0","psutil==5.9.0"]:
    install_pkg(pkg)
...
if not TOKEN: print("❌ BOT_TOKEN غير موجود"); sys.exit(1)
OWNER = int(os.getenv("OWNER_ID", "0"))
if OWNER == 0: print("❌ OWNER_ID غير موجود"); sys.exit(1)
...
def enc(text): return fernet.encrypt(text.encode()).decode() if text else None
def dec(text): return fernet.decrypt(text.encode()).decode() if text else None
...
async def get_db():
    ...
    if _db is None:
        ...
        await _db.executescript("""
            CREATE TABLE IF NOT EXISTS users(user_id INTEGER PRIMARY KEY, banned INTEGER DEFAULT 0, language TEXT DEFAULT 'ar', points INTEGER DEFAULT 0, level INTEGER DEFAULT 1, active_channel INTEGER, referral_code TEXT, subscription_end TEXT, auto_publish INTEGER DEFAULT 1, warns INTEGER DEFAULT 0, trial_used INTEGER DEFAULT 0, auto_recycle INTEGER DEFAULT 1);
            CREATE TABLE IF NOT EXISTS channels(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, channel_id TEXT, channel_name TEXT, auto_publish INTEGER DEFAULT 1, banned INTEGER DEFAULT 0, created_at TEXT);
            CREATE TABLE IF NOT EXISTS posts(id INTEGER PRIMARY KEY AUTOINCREMENT, channel_db_id INTEGER, text TEXT, media_type TEXT DEFAULT 'text', media_file_id TEXT, published INTEGER DEFAULT 0, scheduled_time TEXT, fail_count INTEGER DEFAULT 0, views_count INTEGER DEFAULT 0, created_at TEXT);
            ...
            CREATE TABLE IF NOT EXISTS hidden_owners(chat_id INTEGER, user_id INTEGER, PRIMARY KEY(chat_id, user_id));
            CREATE TABLE IF NOT EXISTS hidden_admins(chat_id INTEGER, user_id INTEGER, added_by INTEGER, added_at TEXT, PRIMARY KEY(chat_id, user_id));
            CREATE TABLE IF NOT EXISTS group_security(chat_id INTEGER PRIMARY KEY, lock_links INTEGER DEFAULT 0, lock_mentions INTEGER DEFAULT 0, slow_mode INTEGER DEFAULT 0, slow_mode_seconds INTEGER DEFAULT 5, welcome_msg TEXT, goodbye_msg TEXT);
            ...
            CREATE TABLE IF NOT EXISTS user_warnings(user_id INTEGER, chat_id INTEGER, warnings INTEGER DEFAULT 0, PRIMARY KEY(user_id, chat_id));
            ...
            CREATE TABLE IF NOT EXISTS contests(id INTEGER PRIMARY KEY AUTOINCREMENT, creator_id INTEGER, title TEXT, description TEXT, prize TEXT, end_date TEXT, status TEXT DEFAULT 'active', winner_id INTEGER);
            CREATE TABLE IF NOT EXISTS contest_parts(contest_id INTEGER, user_id INTEGER, joined_at TEXT, PRIMARY KEY(contest_id, user_id));
            CREATE TABLE IF NOT EXISTS contest_winners(contest_id INTEGER PRIMARY KEY, winner_id INTEGER, announced_at TEXT);
            CREATE TABLE IF NOT EXISTS referrals(ref_id INTEGER, new_id INTEGER, referred_at TEXT, PRIMARY KEY(ref_id, new_id));
            CREATE TABLE IF NOT EXISTS referral_rewards(user_id INTEGER PRIMARY KEY, referral_count INTEGER DEFAULT 0, total_reward_days INTEGER DEFAULT 0, claimed_reward_days INTEGER DEFAULT 0);
            ...
            CREATE TABLE IF NOT EXISTS user_translation(user_id INTEGER PRIMARY KEY, lang TEXT DEFAULT 'off');
            CREATE TABLE IF NOT EXISTS tickets(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, username TEXT, message TEXT, num INTEGER, status TEXT DEFAULT 'pending', created_at TEXT);
            ...
            CREATE TABLE IF NOT EXISTS web_sessions(session_id TEXT PRIMARY KEY, user_data TEXT, expires INTEGER);
...
async def db_execute(query, *params):
    db = await get_db(); cursor = await db.execute(query, params); await db.commit()
    if query.strip().upper().startswith("SELECT"):
        rows = await cursor.fetchall(); return [dict(row) for row in rows]
    return []

async def db_execute_one(query, *params):
    db = await get_db(); cursor = await db.execute(query, params); await db.commit()
...
def t(user_id, key, **kwargs):
    texts = {
        "ar": {
            ...
            "add_ch": "➕ إضافة قناة", "my_ch": "📡 قنواتي", "settings": "⚙️ الإعدادات",
            "add_posts": "📥 إضافة 15 منشور", "publish_one": "📤 نشر واحد", "my_posts": "📋 منشوراتي",
            "recycle": "♻️ إعادة تدوير", "stats": "📊 إحصائياتي", "full_stats": "📈 إحصائيات كاملة",
            "ch_stats": "📊 إحصائيات القناة", "my_groups": "👥 مجموعاتي", "rank": "🏆 رتبتي",
            "top": "⭐ أفضل 10", "schedule": "⏰ جدولة منشور", "publish_all": "📤 نشر الكل",
            "help": "❓ المساعدة", "language": "🌐 اللغة", "trial": "🎁 تجربة مجانية",
            "subscribe": "💎 اشتراك", "support": "📞 الدعم", "referral": "🔗 الإحالات",
            "reminder": "⏰ التذكيرات", "translation": "🌐 الترجمة", "add_to_group": "➕ إضافة إلى مجموعة",
            "admin_panel": "👑 لوحة الأدمن", "developer": "👨‍💻 المطور", "updates": "📢 التحديثات",
            "contests": "🏆 المسابقات", "back": "🔙 رجوع", "subscribed": "✅ مفعل",
            "not_subscribed": "❌ غير مفعل", "auto_on": "مفعل", "auto_off": "معطل",
            "no_channels": "📭 لا توجد قنوات", "channel_added": "✅ تم إضافة القناة {ch}",
            "channel_deleted": "✅ تم حذف القناة", "no_posts": "📭 لا توجد منشورات",
            "post_published": "✅ تم نشر المنشور", "cancelled": "❌ تم الإلغاء",
            "error": "⚠️ حدث خطأ", "admin_only": "🔒 هذا الأمر للمشرفين فقط",
            ...
            "translation_off": "معطلة ❌", "translation_on": "مفعلة ✅ إلى {lang}",
        ...
        "en": {
            ...
            "main_title": "🌿 **{bot}**\n👤 ID: `{uid}`\n👥 Groups: {groups}\n💎 Subscription: {sub}\n📡 Active Channel: {ch}\n📝 Unpublished: {posts}\n⚙️ Auto Publish: {auto}",
            "add_ch": "➕ Add Channel", "my_ch": "📡 My Channels", "settings": "⚙️ Settings",
            "add_posts": "📥 Add 15 Posts", "publish_one": "📤 Publish One", "my_posts": "📋 My Posts",
            "recycle": "♻️ Recycle", "stats": "📊 My Stats", "full_stats": "📈 Full Stats",
            "ch_stats": "📊 Channel Stats", "my_groups": "👥 My Groups", "rank": "🏆 My Rank",
            "top": "⭐ Top 10", "schedule": "⏰ Schedule Post", "publish_all": "📤 Publish All",
            "help": "❓ Help", "language": "🌐 Language", "trial": "🎁 Free Trial",
            "subscribe": "💎 Subscribe", "support": "📞 Support", "referral": "🔗 Referrals",
            "reminder": "⏰ Reminders", "translation": "🌐 Translation", "add_to_group": "➕ Add to Group",
            "admin_panel": "👑 Admin Panel", "developer": "👨‍💻 Developer", "updates": "📢 Updates",
            "contests": "🏆 Contests", "back": "🔙 Back", "subscribed": "✅ Active",
            "not_subscribed": "❌ Inactive", "auto_on": "Enabled", "auto_off": "Disabled",
            "no_channels": "📭 No channels", "channel_added": "✅ Channel {ch} added",
            "channel_deleted": "✅ Channel deleted", "no_posts": "📭 No posts",
            "post_published": "✅ Post published", "cancelled": "❌ Cancelled",
            "error": "⚠️ Error occurred", "admin_only": "🔒 Admins only",
            "group_only": "🔒 Groups only", "trial_activated": "🎁 30 days trial activated",
            "subscription_warning": "⚠️ Subscription expires in {days} days",
            "translation_off": "Disabled ❌", "translation_on": "Enabled ✅ to {lang}",
...
async def add_points(user_id, points):
    ...
    if row:
        ...
        if level > row["level"]: await db_execute("UPDATE users SET level=? WHERE user_id=?", level, user_id)
...
async def get_subscription_days_left(user_id):
    row = await db_execute_one("SELECT subscription_end FROM users WHERE user_id=?", user_id)
    if row and row["subscription_end"]:
        try:
            end = datetime.fromisoformat(dec(row["subscription_end"]))
...
async def is_authorized(bot, chat_id, user_id):
    if user_id == OWNER: return True
    if await db_execute_one("SELECT 1 FROM hidden_owners WHERE chat_id=? AND user_id=?", chat_id, user_id): return True
    if await db_execute_one("SELECT 1 FROM hidden_admins WHERE chat_id=? AND user_id=?", chat_id, user_id): return True
    try:
...
def has_links(text): return bool(re.search(r'https?://\S+', text or ""))
def has_mentions(text): return bool(re.search(r'@\w+', text or ""))
...
def admin_keyboard(user_id):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("👥 المستخدمين", callback_data="adm_users"), InlineKeyboardButton("🚫 المحظورين", callback_data="adm_banned")],
        [InlineKeyboardButton("📡 القنوات", callback_data="adm_channels"), InlineKeyboardButton("📊 المجموعات", callback_data="adm_groups")],
        [InlineKeyboardButton("💬 الردود", callback_data="adm_replies"), InlineKeyboardButton("📋 التذاكر", callback_data="adm_tickets")],
        [InlineKeyboardButton("🏆 إنشاء مسابقة", callback_data="adm_contest"), InlineKeyboardButton("🏅 إعلان فائز", callback_data="declare_winner")],
        [InlineKeyboardButton("📨 إذاعة", callback_data="adm_broadcast"), InlineKeyboardButton("🏆 الفائزين", callback_data="winners")],
        [InlineKeyboardButton("💾 نسخ احتياطي", callback_data="adm_backup"), InlineKeyboardButton("📥 تصدير", callback_data="adm_export")],
        [InlineKeyboardButton("🫀 فحص النبض", callback_data="ping_status"), InlineKeyboardButton("🔄 تحديث", callback_data="reload_files")],
...
async def add_owner(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
...
async def add_admin(update, context):
    ...
    target = await get_target(update, context)
    if not target: return
    await db_execute("INSERT OR IGNORE INTO hidden_admins(chat_id,user_id,added_by,added_at) VALUES(?,?,?,?)", cid, target.id, uid, datetime.now().isoformat())
    await update.message.reply_text("✅")
...
async def remove_admin(update, context):
    ...
    target = await get_target(update, context)
    if not target: return
    if target.id == OWNER: return await update.message.reply_text("❌")
...
async def list_admins(update, context):
    ...
    text = "👑 الملاك:\n" + "\n".join(f"• {o['user_id']}" for o in owners)
...
async def promote_user(update, context):
    ...
    if not target: return
    try:
        await context.bot.promote_chat_member(cid, target.id, can_manage_chat=True, can_delete_messages=True, can_restrict_members=True, can_promote_members=True, can_change_info=True, can_invite_users=True, can_pin_messages=True)
...
async def demote_user(update, context):
    ...
    if not target: return
    try:
        await context.bot.promote_chat_member(cid, target.id, can_manage_chat=False, can_delete_messages=False, can_restrict_members=False, can_promote_members=False, can_change_info=False, can_invite_users=False, can_pin_messages=False)
        await update.message.reply_text(t(uid, "demoted").format(user=target.first_name))
    except Exception as e: await update.message.reply_text(f"❌ {e}")

async def purge_messages(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
...
async def info_command(update, context):
    target = await get_target(update, context)
    if target:
        warns = await db_execute_one("SELECT warnings FROM user_warnings WHERE user_id=? AND chat_id=?", target.id, update.effective_chat.id)
...
async def warn_user(update, context):
    ...
    if warns >= 3:
        try: await context.bot.ban_chat_member(cid, target.id)
        except: pass
        await update.message.reply_text(f"🚫 حظر تلقائي {target.first_name}")
    else:
        await update.message.reply_text(t(uid, "warning_given").format(user=target.first_name, warns=warns))

async def transfer_ownership(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
    target = await get_target(update, context)
    if not target: return
    ...
    await update.message.reply_text(f"👑 نقل الملكية إلى {target.first_name}")
...
async def lock_mentions(update, context):
    ...
    await update.message.reply_text("👥 قفل المنشنات")

async def unlock_mentions(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
    await db_execute("UPDATE group_security SET lock_mentions=0 WHERE chat_id=?", cid)
    await update.message.reply_text("👥 فتح المنشنات")

async def slowmode_command(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
    try: seconds = int(context.args[0]) if context.args else 0
    except: return
    await db_execute("INSERT OR IGNORE INTO group_security(chat_id) VALUES(?)", cid)
    await db_execute("UPDATE group_security SET slow_mode=?, slow_mode_seconds=? WHERE chat_id=?", seconds > 0, seconds, cid)
...
async def add_banned_word(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
    if not context.args: return
    word = context.args[0].lower()
    await db_execute("INSERT OR IGNORE INTO banned_words(word,chat_id) VALUES(?,?)", word, cid)
    await update.message.reply_text(f"✅ {word}")

async def remove_banned_word(update, context):
    uid, cid = update.effective_user.id, update.effective_chat.id
    if not await is_authorized(context.bot, cid, uid): return
    if not context.args: return
    word = context.args[0].lower()
...
async def list_banned_words(update, context):
    cid = update.effective_chat.id
    words = await db_execute("SELECT word FROM banned_words WHERE chat_id=? OR chat_id=-1", cid)
    await update.message.reply_text("🚫 " + ", ".join(w["word"] for w in words) if words else "📭")

async def add_reply(update, context):
    uid = update.effective_user.id
    if not await is_authorized(context.bot, update.effective_chat.id, uid): return
    if len(context.args) < 2: return
    await db_execute("INSERT OR REPLACE INTO replies VALUES(?,?)", context.args[0].lower(), " ".join(context.args[1:]))
    await update.message.reply_text("✅")
...
async def remove_reply(update, context):
    ...
    if not await is_authorized(context.bot, update.effective_chat.id, uid): return
    if not context.args: return
    await db_execute("DELETE FROM replies WHERE keyword=?", context.args[0].lower())
    await update.message.reply_text("✅")

async def list_replies(update, context):
    replies = await db_execute("SELECT * FROM replies")
    await update.message.reply_text("💬 " + "\n".join(f"{r['keyword']} → {r['reply'][:30]}" for r in replies) if replies else "📭")
...
async def log_command(update, context):
    ...
    await update.message.reply_text("📜 " + "\n".join(f"{l['action']} → {l['target_id']}" for l in logs) if logs else "📭")

async def group_settings(update, context):
    cid = update.effective_chat.id
    s = await db_execute_one("SELECT * FROM group_security WHERE chat_id=?", cid) or {}
    await update.message.reply_text(f"🔗 روابط: {'✅' if s.get('lock_links') else '❌'}\n👥 منشنات: {'✅' if s.get('lock_mentions') else '❌'}\n⏱️ بطء: {s.get('slow_mode_seconds', 0)}ث")

async def reload_command(update, context):
    if update.effective_user.id != OWNER: return
...
async def ping_command(update, context):
    ...
    try: await db_execute_one("SELECT 1"); db_status = "✅"
    except: db_status = "❌"
    await msg.edit_text(t(uid, "ping_status", ping=ping, db=db_status, time=datetime.now().strftime("%H:%M:%S")))
...
async def panel_command(update, context):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔇 كتم", callback_data=f"act_mute_{cid}_{target.id}"),
         InlineKeyboardButton("🔊 فك", callback_data=f"act_unmute_{cid}_{target.id}")],
        [InlineKeyboardButton("🚫 حظر", callback_data=f"act_ban_{cid}_{target.id}"),
         InlineKeyboardButton("✅ فك", callback_data=f"act_unban_{cid}_{target.id}")],
        ...
        [InlineKeyboardButton("🗑️ حذف", callback_data=f"act_del_{cid}_{update.message.reply_to_message.message_id}")],
...
async def broadcast_command(update, context):
    ...
    sent = 0
    for u in users:
        try: await context.bot.send_message(u["user_id"], text); sent += 1; await asyncio.sleep(0.1)
        except: pass
...
async def button_handler(update: Update, context):
    ...
    if data == "back":
        ...
        chs = await db_execute("SELECT * FROM channels WHERE user_id=?", uid)
        if not chs: return await query.edit_message_text(t(uid, "no_channels"))
        kb = []
        for ch in chs:
            kb.append([InlineKeyboardButton(f"📢 {ch['channel_name']}", callback_data=f"sel_ch_{ch['id']}")])
            kb.append([InlineKeyboardButton("🗑️", callback_data=f"del_ch_{ch['id']}")])
        ...
        await query.edit_message_text("✅", reply_markup=main_keyboard(uid))
        ...
        await query.edit_message_text("📥 أرسل 15 منشوراً")
        ...
        if not post: return
        try:
            if post["media_type"] == "photo": await context.bot.send_photo(ch["channel_id"], post["media_file_id"], caption=post["text"])
            elif post["media_type"] == "video": await context.bot.send_video(ch["channel_id"], post["media_file_id"], caption=post["text"])
            else: await context.bot.send_message(ch["channel_id"], post["text"])
            await db_execute("UPDATE posts SET published=1 WHERE id=?", post["id"])
            await add_points(uid, 5)
            await query.edit_message_text("✅", reply_markup=main_keyboard(uid))
        except Exception as e: await query.edit_message_text(f"❌ {e}")
    elif data == "my_posts":
        ch = await db_execute_one("SELECT id FROM channels WHERE user_id=? AND active_channel=id LIMIT 1", uid)
        if not ch: return
        ...
        for p in posts:
            text += f"• {p['text'][:30]}...\n"
            kb.append([InlineKeyboardButton(f"🗑️ #{p['id']}", callback_data=f"del_post_{p['id']}")])
        kb.append([InlineKeyboardButton("🗑️ الكل", callback_data="del_all_posts")])
        kb.append([InlineKeyboardButton("🔙", callback_data="back")])
        ...
        ch = await db_execute_one("SELECT id FROM channels WHERE user_id=? AND active_channel=id LIMIT 1", uid)
        if ch: await db_execute("DELETE FROM posts WHERE channel_db_id=?", ch["id"])
        await query.edit_message_text("✅", reply_markup=main_keyboard(uid))
    elif data == "recycle":
        ch = await db_execute_one("SELECT id FROM channels WHERE user_id=? AND active_channel=id LIMIT 1", uid)
        if ch: await db_execute("UPDATE posts SET published=0 WHERE channel_db_id=?", ch["id"])
        await query.edit_message_text("♻️", reply_markup=main_keyboard(uid))
        ...
        await query.edit_message_text(f"✅ {count}", reply_markup=main_keyboard(uid))
    elif data == "stats":
        ...
        for ch in chs:
            ...
            text += f"• {ch['channel_name']}: {unpub['c'] if unpub else 0}\n"
        ...
        unpub = await db_execute_one("SELECT COUNT(*) as c FROM posts WHERE channel_db_id=? AND published=0", ch["id"])
        await query.edit_message_text(f"📊 {ch['channel_name']}\n📝 الكل: {total['c'] if total else 0}\n✅ منشور: {pub['c'] if pub else 0}\n⏳ غير: {unpub['c'] if unpub else 0}")
    elif data == "my_groups":
        gs = await db_execute("SELECT * FROM groups WHERE added_by=?", uid)
        if not gs: return await query.edit_message_text("📭")
        kb = []
        for g in gs: kb.append([InlineKeyboardButton(f"📌 {g['chat_name']}", callback_data=f"gset_{g['chat_id']}")])
        kb.append([InlineKeyboardButton("🔙", callback_data="back")])
        await query.edit_message_text("👥", reply_markup=InlineKeyboardMarkup(kb))
    elif data.startswith("gset_"):
        try: cid = int(data.split("_")[1])
        except: return
        s = await db_execute_one("SELECT * FROM group_security WHERE chat_id=?", cid) or {}
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"🔗 روابط: {'✅' if s.get('lock_links') else '❌'}", callback_data=f"gset_links_{cid}_{1-s.get('lock_links',0)}"),
             InlineKeyboardButton(f"👥 منشنات: {'✅' if s.get('lock_mentions') else '❌'}", callback_data=f"gset_mentions_{cid}_{1-s.get('lock_mentions',0)}")],
            [InlineKeyboardButton(f"⏱️ بطء: {s.get('slow_mode_seconds',0)}ث", callback_data=f"gset_slow_{cid}")],
            [InlineKeyboardButton("📝 ترحيب", callback_data=f"gset_welcome_{cid}"),
             InlineKeyboardButton("👋 وداع", callback_data=f"gset_goodbye_{cid}")],
            [InlineKeyboardButton("🚫 كلمات", callback_data=f"gset_banned_{cid}"),
             InlineKeyboardButton("💬 ردود", callback_data=f"gset_replies_{cid}")],
            [InlineKeyboardButton("📜 سجل", callback_data=f"gset_log_{cid}"),
             InlineKeyboardButton("🔙", callback_data="my_groups")],
        ])
        await query.edit_message_text("⚙️", reply_markup=kb)
    elif data.startswith("gset_links_"):
        parts = data.split("_"); cid = int(parts[2]); val = int(parts[3])
        await db_execute("INSERT OR IGNORE INTO group_security(chat_id) VALUES(?)", cid)
        await db_execute("UPDATE group_security SET lock_links=? WHERE chat_id=?", val, cid)
        await query.answer(f"{'✅' if val else '❌'}")
        s = await db_execute_one("SELECT * FROM group_security WHERE chat_id=?", cid) or {}
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"🔗 روابط: {'✅' if s.get('lock_links') else '❌'}", callback_data=f"gset_links_{cid}_{1-s.get('lock_links',0)}"),
             InlineKeyboardButton(f"👥 منشنات: {'✅' if s.get('lock_mentions') else '❌'}", callback_data=f"gset_mentions_{cid}_{1-s.get('lock_mentions',0)}")],
        ...
        await query.edit_message_text("⏱️ أرسل الثواني:")
    elif data.startswith("gset_welcome_"):
        cid = int(data.split("_")[2])
        context.user_data["state"] = f"SET_WELCOME_{cid}"
        await query.edit_message_text("📝 أرسل الترحيب:")
    elif data.startswith("gset_goodbye_"):
        cid = int(data.split("_")[2])
        context.user_data["state"] = f"SET_GOODBYE_{cid}"
        await query.edit_message_text("👋 أرسل الوداع:")
    elif data.startswith("gset_banned_"):
        cid = int(data.split("_")[2])
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ إضافة", callback_data=f"bw_add_{cid}"),
             InlineKeyboardButton("🗑️ حذف", callback_data=f"bw_del_{cid}")],
            [InlineKeyboardButton("📋 عرض", callback_data=f"bw_list_{cid}"),
             InlineKeyboardButton("🔙", callback_data=f"gset_{cid}")],
        ])
        await query.edit_message_text("🚫", reply_markup=kb)
    elif data.startswith("gset_replies_"):
        cid = int(data.split("_")[2])
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ إضافة", callback_data=f"rep_add_{cid}"),
             InlineKeyboardButton("🗑️ حذف", callback_data=f"rep_del_{cid}")],
            [InlineKeyboardButton("📋 عرض", callback_data=f"rep_list_{cid}"),
             InlineKeyboardButton("🔙", callback_data=f"gset_{cid}")],
        ])
        await query.edit_message_text("💬", reply_markup=kb)
        ...
        cid = int(data.split("_")[2])
        logs = await db_execute("SELECT * FROM mod_log WHERE chat_id=? ORDER BY id DESC LIMIT 20", cid)
        await query.edit_message_text("📜 " + "\n".join(f"{l['action']} → {l['target_id']}" for l in logs) if logs else "📭", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data=f"gset_{cid}")]]))
        ...
        cid = int(data.split("_")[2])
        words = await db_execute("SELECT word FROM banned_words WHERE chat_id=? OR chat_id=-1", cid)
        await query.edit_message_text("🚫 " + ", ".join(w["word"] for w in words) if words else "📭", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data=f"gset_banned_{cid}")]]))
        ...
        await query.edit_message_text("💬 " + "\n".join(f"{r['keyword']} → {r['reply'][:30]}" for r in replies) if replies else "📭", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data=f"gset_replies_{cid}")]]))
    elif data.startswith("act_mute_"):
        parts = data.split("_"); cid = int(parts[2]); tid = int(parts[3])
        if not await is_authorized(context.bot, cid, uid): return await query.answer("🔒")
        try:
            await context.bot.restrict_chat_member(cid, tid, ChatPermissions(can_send_messages=False))
            await query.edit_message_text(f"🔇 {tid}")
        ...
        if warns >= 3:
            ...
            await query.edit_message_text(f"🚫 حظر تلقائي {tid}")
        else: await query.edit_message_text(f"⚠️ تحذير {tid} ({warns}/3)")
    elif data.startswith("act_del_"):
        parts = data.split("_"); cid = int(parts[2]); mid = int(parts[3])
        if not await is_authorized(context.bot, cid, uid): return await query.answer("🔒")
        try:
        ...
        if not contests: return await query.edit_message_text("📭")
        kb = []
        for c in contests: kb.append([InlineKeyboardButton(f"📌 {c['title']} - {c['prize']}", callback_data=f"join_{c['id']}")])
        ...
        try:
            await db_execute("INSERT INTO contest_parts(contest_id,user_id,joined_at) VALUES(?,?,?)", contest_id, uid, datetime.now().isoformat())
            await add_points(uid, 5)
            await query.edit_message_text(t(uid, "contest_joined"), reply_markup=main_keyboard(uid))
        except: await query.edit_message_text(t(uid, "contest_already"), reply_markup=main_keyboard(uid))
    elif data == "winners":
        ...
        await query.edit_message_text("🏆 " + "\n".join(f"{w['title']} → {w['winner_id']}" for w in winners) if winners else "📭", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="contests")]]))
    elif data == "referral":
        ...
        await query.edit_message_text(f"🎁 {avail} يوم", reply_markup=main_keyboard(uid))
    elif data == "ref_list":
        refs = await db_execute("SELECT new_id FROM referrals WHERE ref_id=?", uid)
        await query.edit_message_text("👥 " + "\n".join(f"• {r['new_id']}" for r in refs) if refs else "📭", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙", callback_data="referral")]]))
    elif data == "reminder":
        rem = await db_execute_one("SELECT * FROM reminders WHERE user_id=?", uid) or {}
        if not rem: await db_execute("INSERT INTO reminders(user_id) VALUES(?)", uid); rem = await db_execute_one("SELECT * FROM reminders WHERE user_id=?", uid)
        sub = "✅" if rem.get("sub") else "❌"
        daily = "✅" if rem.get("daily") else "❌"
        weekly = "✅" if rem.get("weekly") else "❌"
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton(f"📈 أسبوعي: {weekly}", callback_data="rem_weekly"),
             InlineKeyboardButton(f"⏰ {rem.get('days_before',3)} أيام", callback_data="rem_days")],
            [InlineKeyboardButton("🔙", callback_data="back")],
        ])
        await query.edit_message_text("⏰", reply_markup=kb)
    elif data == "rem_sub":
        ...
        await query.answer(f"{'✅' if new_val else '❌'}")
    elif data == "rem_daily":
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("العربية", callback_data="trans_ar"), InlineKeyboardButton("English", callback_data="trans_en")],
            [InlineKeyboardButton("Français", callback_data="trans_fr"), InlineKeyboardButton("Türkçe", callback_data="trans_tr")],
            [InlineKeyboardButton("Русский", callback_data="trans_ru"), InlineKeyboardButton("Deutsch", callback_data="trans_de")],
            [InlineKeyboardButton(t(uid, "translation_off"), callback_data="trans_off"), InlineKeyboardButton("🔙", callback_data="back")],
        ])
        ...
        await query.edit_message_text("👨‍💻 @RelaxMgr")
    elif data == "updates":
        await query.edit_message_text("📢 https://t.me/RelaxUpdates")
    elif data == "help":
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("Русский 🇷🇺", callback_data="lang_ru"), InlineKeyboardButton("Deutsch 🇩🇪", callback_data="lang_de")],
            [InlineKeyboardButton("🔙", callback_data="back")],
        ])
        ...
        await query.edit_message_text(f"✅ {lang}", reply_markup=main_keyboard(uid))
    elif data == "rank":
        row = await db_execute_one("SELECT points, level FROM users WHERE user_id=?", uid)
        await query.edit_message_text(f"🏆 Lv{row['level']} | {row['points']} نقطة" if row else "0")
    elif data == "top":
        tops = await db_execute("SELECT user_id, points, level FROM users ORDER BY points DESC LIMIT 10")
        await query.edit_message_text("🏆 " + "\n".join(f"{i}. {u['user_id']} Lv{u['level']}" for i,u in enumerate(tops,1)) if tops else "📭")
    elif data == "schedule":
        context.user_data["state"] = "SCHEDULE"
        await query.edit_message_text("📝 أرسل YYYY-MM-DD HH:MM النص")
    elif data == "trial":
        row = await db_execute_one("SELECT trial_used FROM users WHERE user_id=?", uid)
        if row and row["trial_used"]: return await query.answer("❌")
        if await has_active_subscription(uid): return await query.answer("✅")
        ...
        if await has_active_subscription(uid):
            ...
            return await query.edit_message_text(f"✅ متبقي {days} يوم", reply_markup=main_keyboard(uid))
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("⭐ 1 يوم - 5🌟", callback_data="buy_1_5"),
             InlineKeyboardButton("⭐ 2 يوم - 9🌟", callback_data="buy_2_9")],
            [InlineKeyboardButton("⭐ 30 يوم - 50🌟", callback_data="buy_30_50"),
             InlineKeyboardButton("⭐ 90 يوم - 120🌟", callback_data="buy_90_120")],
            [InlineKeyboardButton("🔙", callback_data="back")],
        ])
        await query.edit_message_text("💎", reply_markup=kb)
    elif data.startswith("buy_"):
        parts = data.split("_")
        days = int(parts[1]); stars = int(parts[2])
        try:
            ...
            await query.edit_message_text("📩 فاتورة")
        except Exception as e: await query.edit_message_text(f"❌ {e}")
    elif data == "settings":
        kb = InlineKeyboardMarkup([
        ...
        await query.edit_message_text(f"{'✅' if new_val else '❌'}", reply_markup=main_keyboard(uid))
        ...
        await query.edit_message_text(f"♻️ {'✅' if new_val else '❌'}", reply_markup=main_keyboard(uid))
    elif data == "admin_panel":
        if uid != OWNER: return await query.answer("🔒")
        await query.edit_message_text("👑", reply_markup=admin_keyboard(uid))
    elif data == "adm_users":
        users = await db_execute("SELECT user_id, banned FROM users LIMIT 50")
        await query.edit_message_text("👥 " + "\n".join(f"{'🚫' if u['banned'] else '✅'} {u['user_id']}" for u in users) if users else "📭", reply_markup=admin_keyboard(uid))
    elif data == "adm_banned":
        users = await db_execute("SELECT user_id FROM users WHERE banned=1 LIMIT 50")
        await query.edit_message_text("🚫 " + "\n".join(f"• {u['user_id']}" for u in users) if users else "📭", reply_markup=admin_keyboard(uid))
    elif data == "adm_channels":
        chs = await db_execute("SELECT channel_name FROM channels LIMIT 50")
        await query.edit_message_text("📡 " + "\n".join(f"• {c['channel_name']}" for c in chs) if chs else "📭", reply_markup=admin_keyboard(uid))
    elif data == "adm_groups":
        gs = await db_execute("SELECT chat_name FROM groups LIMIT 50")
        await query.edit_message_text("📊 " + "\n".join(f"• {g['chat_name']}" for g in gs) if gs else "📭", reply_markup=admin_keyboard(uid))
    elif data == "adm_replies":
        reps = await db_execute("SELECT * FROM replies LIMIT 30")
        await query.edit_message_text("💬 " + "\n".join(f"{r['keyword']} → {r['reply'][:30]}" for r in reps) if reps else "📭", reply_markup=admin_keyboard(uid))
    elif data == "adm_tickets":
        tics = await db_execute("SELECT * FROM tickets ORDER BY num DESC LIMIT 10")
        await query.edit_message_text("📋 " + "\n".join(f"#{t['num']} {t['username']}" for t in tics) if tics else "📭", reply_markup=admin_keyboard(uid))
    elif data == "adm_contest":
        context.user_data["state"] = "CONTEST_TITLE"
        await query.edit_message_text("📝 عنوان:")
    elif data == "declare_winner":
        context.user_data["state"] = "DECLARE_WINNER"
        await query.edit_message_text("📝 contest_id user_id")
    elif data == "adm_broadcast":
        context.user_data["state"] = "BROADCAST"
        await query.edit_message_text("📨 أرسل النص:")
    elif data == "adm_backup":
        try:
            backup_file = Path("data") / f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
        ...
        if not rows: return await query.edit_message_text("❌")
        output = io.StringIO()
        ...
        await query.edit_message_text(t(uid, "ping_status", ping=ping, db=db_status, time=datetime.now().strftime("%H:%M:%S")), reply_markup=admin_keyboard(uid))
...
async def payment_callback(update, context):
    ...
    end_date = (datetime.now() + timedelta(days=new_days)).isoformat()
    await db_execute("UPDATE users SET subscription_end=? WHERE user_id=?", enc(end_date), uid)
    await update.message.reply_text(f"✅ تم تفعيل {days} يوم")

# ========== معالجات المجموعة ==========
async def auto_register_group(update, context):
    if not update.message or not update.message.new_chat_members: return
    for member in update.message.new_chat_members:
        if member.id == context.bot.id:
            cid = update.effective_chat.id
            await db_execute("INSERT OR IGNORE INTO groups(chat_id,chat_name,added_by) VALUES(?,?,?)", cid, update.effective_chat.title, update.effective_user.id)
            await db_execute("INSERT OR IGNORE INTO hidden_owners(chat_id,user_id) VALUES(?,?)", cid, update.effective_user.id)

async def on_user_join(update, context):
    if not update.message or not update.message.new_chat_members: return
    cid = update.effective_chat.id
    s = await db_execute_one("SELECT welcome_msg FROM group_security WHERE chat_id=?", cid)
    if s and s["welcome_msg"]:
        for member in update.message.new_chat_members:
            if not member.is_bot:
                try: await update.message.reply_text(s["welcome_msg"].replace("{user}", member.first_name).replace("{chat}", update.effective_chat.title))
                except: pass

async def on_user_leave(update, context):
    if not update.message or not update.message.left_chat_member: return
    cid = update.effective_chat.id
    s = await db_execute_one("SELECT goodbye_msg FROM group_security WHERE chat_id=?", cid)
    if s and s["goodbye_msg"]:
        member = update.message.left_chat_member
        if not member.is_bot:
            try: await update.message.reply_text(s["goodbye_msg"].replace("{user}", member.first_name).replace("{chat}", update.effective_chat.title))
            except: pass
...
async def group_message_filter(update, context):
    ...
    s = await db_execute_one("SELECT * FROM group_security WHERE chat_id=?", cid) or {}
    if s.get("lock_links") and has_links(text):
        try: await update.message.delete()
        except: pass
        return
    if s.get("lock_mentions") and has_mentions(text):
        try: await update.message.delete()
        except: pass
        return
    words = await db_execute("SELECT word FROM banned_words WHERE chat_id=? OR chat_id=-1", cid)
    for w in words:
        if w["word"] in text.lower():
            try: await update.message.delete()
            except: pass
            return
    reply = await db_execute_one("SELECT reply FROM replies WHERE keyword=?", text.lower())
    if reply:
        try: await update.message.reply_text(reply["reply"])
        except: pass
        return
    quick = {"مرحباً":"أهلاً 🤍","السلام عليكم":"وعليكم السلام 🌹","شكراً":"العفو","كيف حالك":"الحمد لله 🙏"}
    for k,v in quick.items():
        if k in text:
            try: await update.message.reply_text(v)
            except: pass
            break
    slow = s.get("slow_mode_seconds", 0)
    if slow > 0:
        ...
        await db_execute("INSERT OR REPLACE INTO user_messages(user_id,chat_id,message_time) VALUES(?,?,?)", uid, cid, now.isoformat())
...
async def private_message_handler(update, context):
    ...
    if text == "/cancel":
        ...
        await update.message.reply_text("❌", reply_markup=main_keyboard(uid))
        return
    if state == "ADD_CH":
        if not text.startswith("@") and not text.startswith("-100"): return
        try:
            ch = await context.bot.get_chat(text.strip())
            await db_execute("INSERT INTO channels(user_id,channel_id,channel_name,created_at) VALUES(?,?,?,?)", uid, str(ch.id), ch.title, datetime.now().isoformat())
            ch_id = (await db_execute_one("SELECT id FROM channels WHERE user_id=? ORDER BY id DESC LIMIT 1", uid))["id"]
            await db_execute("UPDATE users SET active_channel=? WHERE user_id=?", ch_id, uid)
            context.user_data.pop("state")
            await update.message.reply_text(t(uid, "channel_added", ch=ch.title), reply_markup=main_keyboard(uid))
        except Exception as e: await update.message.reply_text(f"❌ {e}")
    elif state == "ADD_POSTS":
        posts = context.user_data.get("posts", [])
        media_type, media_file_id, content = "text", None, text
        if update.message.photo:
            ...
            media_type, media_file_id = "audio", update.message.audio.file_id
            content = update.message.caption or ""
        elif update.message.voice:
            media_type, media_file_id = "voice", update.message.voice.file_id
            content = update.message.caption or ""
        ...
        if len(posts) >= 15:
            ...
            if ch:
                for tx, mt, mf in posts:
                    await db_execute("INSERT INTO posts(channel_db_id,text,media_type,media_file_id,created_at) VALUES(?,?,?,?,?)", ch["id"], tx, mt, mf, datetime.now().isoformat())
            context.user_data.clear()
            await update.message.reply_text("✅", reply_markup=main_keyboard(uid))
            ...
            try:
                ...
                if dt > datetime.now():
                    ...
                    if ch:
                        ...
                        await db_execute("INSERT INTO scheduled_tasks(user_id,task_type,task_data,execute_at) VALUES(?,?,?,?)", uid, "publish", task_data, dt.isoformat())
                        await update.message.reply_text(t(uid, "schedule_saved"))
                    else: await update.message.reply_text("❌")
                else: await update.message.reply_text("❌")
            except: await update.message.reply_text("❌")
        else: await update.message.reply_text("📝 YYYY-MM-DD HH:MM النص")
        context.user_data.pop("state")
    elif state == "SUPPORT":
        if text:
            num = (await db_execute_one("SELECT value FROM settings WHERE key='last_ticket'")) or {"value":"0"}
            new_num = int(num["value"]) + 1
            await db_execute("INSERT INTO tickets(user_id,username,message,num,created_at) VALUES(?,?,?,?,?)", uid, update.effective_user.full_name or str(uid), text, new_num, datetime.now().isoformat())
        ...
        try:
            ...
            if end_date <= datetime.now(): return await update.message.reply_text("❌")
            await db_execute("INSERT INTO contests(creator_id,title,description,prize,end_date,status,created_at) VALUES(?,?,?,?,?,?,?)", uid, context.user_data.get("contest_title"), context.user_data.get("contest_desc"), context.user_data.get("contest_prize"), end_date.isoformat(), "active", datetime.now().isoformat())
        ...
        if len(parts) == 2:
            try:
                contest_id, winner_id = int(parts[0]), int(parts[1])
                contest = await db_execute_one("SELECT * FROM contests WHERE id=? AND status='active'", contest_id)
                if not contest: return await update.message.reply_text("❌")
                if datetime.fromisoformat(contest["end_date"]) > datetime.now(): return await update.message.reply_text("❌")
                await db_execute("UPDATE contests SET status='finished', winner_id=? WHERE id=?", winner_id, contest_id)
                await db_execute("INSERT OR IGNORE INTO contest_winners(contest_id,winner_id,announced_at) VALUES(?,?,?)", contest_id, winner_id, datetime.now().isoformat())
                await update.message.reply_text(t(uid, "contest_declared"))
            except: await update.message.reply_text("❌")
        else: await update.message.reply_text("❌ contest_id user_id")
        context.user_data.pop("state")
        ...
        cid = int(state.split("_")[2])
        await db_execute("INSERT OR IGNORE INTO banned_words(word,chat_id) VALUES(?,?)", text.lower(), cid)
        ...
        if kw: await db_execute("INSERT OR REPLACE INTO replies VALUES(?,?)", kw, text)
        await update.message.reply_text("✅")
        context.user_data.clear()
        ...
        try:
            ...
            if 1 <= days <= 30:
                ...
                await update.message.reply_text(f"✅ {days} أيام")
            else: await update.message.reply_text("❌ 1-30")
        except: await update.message.reply_text("❌")
        context.user_data.pop("state")
...
async def auto_publish_loop(app):
    ...
    while True:
        ...
        try:
            ...
            for ch in channels:
                if not await has_active_subscription(ch["user_id"]): continue
                post = await db_execute_one("SELECT * FROM posts WHERE channel_db_id=? AND published=0 ORDER BY id LIMIT 1", ch["id"])
                ...
                try:
                    ...
                    if trans_lang and trans_lang["lang"] != "off":
                        translated = GoogleTranslator(source="auto", target=trans_lang["lang"]).translate(post["text"][:5000])
                        post_text = f"{post['text']}\n\n🌐 {translated}" if translated and translated != post["text"] else post["text"]
                    else: post_text = post["text"]
                    if post["media_type"] == "photo": await app.bot.send_photo(ch["channel_id"], post["media_file_id"], caption=post_text)
                    elif post["media_type"] == "video": await app.bot.send_video(ch["channel_id"], post["media_file_id"], caption=post_text)
                    elif post["media_type"] == "document": await app.bot.send_document(ch["channel_id"], post["media_file_id"], caption=post_text)
                    elif post["media_type"] == "audio": await app.bot.send_audio(ch["channel_id"], post["media_file_id"], caption=post_text)
                    elif post["media_type"] == "voice": await app.bot.send_voice(ch["channel_id"], post["media_file_id"], caption=post_text)
                    elif post["media_type"] == "animation": await app.bot.send_animation(ch["channel_id"], post["media_file_id"], caption=post_text)
                    else: await app.bot.send_message(ch["channel_id"], post_text)
                    await db_execute("UPDATE posts SET published=1 WHERE id=?", post["id"])
                    await add_points(ch["user_id"], 5)
                except Exception as e:
                    await db_execute("UPDATE posts SET fail_count=fail_count+1 WHERE id=?", post["id"])
                await asyncio.sleep(2)
        except Exception as e: logger.error(f"النشر: {e}")
...
async def scheduled_tasks_loop(app):
    ...
    while True:
        try:
            tasks = await db_execute("SELECT * FROM scheduled_tasks WHERE executed=0 AND execute_at <= ?", datetime.now().isoformat())
            for task in tasks:
                if task["task_type"] == "publish":
                    ch = await db_execute_one("SELECT id,channel_id FROM channels WHERE user_id=? AND active_channel=id LIMIT 1", task["user_id"])
                    if ch:
                        try: await app.bot.send_message(ch["channel_id"], task["task_data"])
                        except: pass
                    ...
                    try:
                        ...
                        if end_str:
                            ...
                            if 0 < days <= rem["days_before"]:
                                try: await app.bot.send_message(rem["user_id"], f"⏰ اشتراكك ينتهي خلال {days} أيام")
                                except: pass
                    except: pass
        except: pass
    ...
    while True:
        try:
            if datetime.now().hour == 8 and datetime.now().minute == 0:
                users = await db_execute("SELECT user_id FROM reminders WHERE daily=1")
                for u in users:
                    try:
                        ch = len(await db_execute("SELECT 1 FROM channels WHERE user_id=?", u["user_id"]))
                        total = len(await db_execute("SELECT 1 FROM posts WHERE channel_db_id IN (SELECT id FROM channels WHERE user_id=?)", u["user_id"]))
                        unpub = len(await db_execute("SELECT 1 FROM posts WHERE channel_db_id IN (SELECT id FROM channels WHERE user_id=?) AND published=0", u["user_id"]))
                        grps = len(await db_execute("SELECT 1 FROM groups WHERE added_by=?", u["user_id"]))
                        await app.bot.send_message(u["user_id"], f"📊 تقرير يومي\nقنوات: {ch}\nمنشورات: {total}\nغير منشورة: {unpub}\nمجموعات: {grps}")
                    except: pass
        except: pass
        await asyncio.sleep(60)
    ...
    while True:
        try:
            ...
            if now.weekday() == 6 and now.hour == 9 and now.minute == 0:
                users = await db_execute("SELECT user_id FROM reminders WHERE weekly=1")
                for u in users:
                    try:
                        ch = len(await db_execute("SELECT 1 FROM channels WHERE user_id=?", u["user_id"]))
                        total = len(await db_execute("SELECT 1 FROM posts WHERE channel_db_id IN (SELECT id FROM channels WHERE user_id=?)", u["user_id"]))
                        unpub = len(await db_execute("SELECT 1 FROM posts WHERE channel_db_id IN (SELECT id FROM channels WHERE user_id=?) AND published=0", u["user_id"]))
                        grps = len(await db_execute("SELECT 1 FROM groups WHERE added_by=?", u["user_id"]))
                        refs = len(await db_execute("SELECT 1 FROM referrals WHERE ref_id=?", u["user_id"]))
                        await app.bot.send_message(u["user_id"], f"📈 تقرير أسبوعي\nقنوات: {ch}\nمنشورات: {total}\nغير منشورة: {unpub}\nمجموعات: {grps}\nإحالات: {refs}")
                    except: pass
        except: pass
        await asyncio.sleep(60)
