...
async def global_error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if update and update.effective_user and context and context.bot:
            try:
                await safe_send_markdown(context.bot, update.effective_user.id, f"❌ خطأ: `{str(error)[:300]}` (كود: {error_id})")
            except Exception as e:
                logger.error(f"Failed to send error message to user: {e}")
                try:
                    await context.bot.send_message(chat_id=update.effective_user.id, text=f"❌ خطأ: `{str(error)[:300]}` (كود: {error_id})")
                except:
                    pass
