...
"""
نظام الحظر المتطور - الإصدار 6.1 (المحسن بالكامل مع مصادقة ورسم بياني)
...
<head>
    ...
    <title>نظام الحظر المتطور v6.1</title>
...
<body>
    <div class="container">
        <div class="header">
            <div><h1 style="color:var(--accent);">🛡️ نظام الحظر المتطور v6.1</h1><div class="subtitle" style="color:var(--text-muted);">ريلاكس مانيجر · تحكم كامل</div></div>
...
async def login_handler(request):
    ...
    try:
        ...
        if password and password == ADMIN_PASSWORD:
            ...
            return web.Response(text=LOGIN_PAGE.replace('</form>', '</form><div class="error">كلمة مرور خاطئة</div>'), content_type='text/html', status=401)
    except:
        return web.Response(text=LOGIN_PAGE, content_type='text/html', status=401)
...
def start_web_server_background(port=None):
    ...
    def run():
        ...
        try:
            ...
            logger.info(f"✅ نظام الحظر v6.1 يعمل على http://0.0.0.0:{port}")
        except OSError as e:
    ...
    logger.info("🚀 تم تشغيل نظام الحظر v6.1")

if __name__ == '__main__':
    asyncio.run(init_db())
    threading.Timer(300, clear_expired_sessions).start()
    port = int(os.getenv('PORT', os.getenv('WEB_PORT', 10000)))
    start_web_server_background(port)
    print(f"🌐 نظام الحظر v6.1 متاح على:")
