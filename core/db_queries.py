from core.database import execute_db
...
async def db_register_user(user_id: int) -> bool:
    async def _register(conn):
        ...
        if await cur.fetchone():
            return False
        await conn.execute("INSERT INTO users (user_id, language) VALUES (?, 'ar')", (user_id,))
        await conn.commit()
        return True
...
async def db_save_posts(channel_db_id: int, posts: list) -> int:
    async def _save(conn):
        values = [(channel_db_id, text, media_type, media_file_id, utc_now_iso()) for text, media_type, media_file_id in posts]
...
async def db_get_publish_interval():
    async def _get(conn):
        ...
        row = await cur.fetchone()
        return int(row[0]) if row else 720
    return await execute_db(_get)

async def db_get_due_channels(limit=20):
    async def _get(conn):
