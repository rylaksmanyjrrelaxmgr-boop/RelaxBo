...
def get_text(user_id: int, key: str) -> str:
    ...
    texts = {
        ...
        'en': {
            ...
            'help': "❓ **Help**\n━━━━━━━━━━━━━━━━━━━━━━\n📌 **Available Commands:**\n/start - Main Menu\n/trial - Free Trial\n/subscribe - Subscribe\n/syncgroup - Activate Group\n/security - Security Settings\n/register_hidden_owner - Register Hidden Owner\n/rank - Your Rank\n/top - Top 10\n/stats - Channel Stats\n/lock - Lock Group\n/unlock - Unlock Group\n/schedule - Schedule Post\n/panel - Control Panel\n/language - Change Language\n/support - Support Center\n/help - This Help\n/developer - Developer\n/updates - Updates\n/banchannel - Ban User Channel\n/unbanchannel - Unban User Channel\n/bannedchannels - List Banned Channels",
...
async def db_get_banned_channels() -> list:
...
async def db_ban_user_channels(user_id: int) -> int:
...
async def ban_channel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """حظر قناة المستخدم"""
    ...
    args = context.args
    if not args:
        await update.message.reply_text("📝 **الاستخدام:**\n`/banchannel معرف_القناة`\n\nمثال: `/banchannel -1001234567890`")
        return
    ...
    # حظر القناة
    success = await db_ban_channel(channel_id)
...
async def unban_channel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """إلغاء حظر قناة المستخدم"""
    user_id = update.effective_user.id
    if user_id != MAIN_ADMIN_ID and not await is_bot_admin(user_id):
        await update.message.reply_text("🔒 هذا الأمر للمطور فقط!")
    ...
    args = context.args
    if not args:
        await update.message.reply_text("📝 **الاستخدام:**\n`/unbanchannel معرف_القناة`\n\nمثال: `/unbanchannel -1001234567890`")
        return
    ...
    # إلغاء حظر القناة
    success = await db_unban_channel(channel_id)
...
async def banned_channels_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channels = await db_get_banned_channels()
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 تحديث", callback_data=CallbackData.BANNED_CHANNELS)],
...
def get_admin_keyboard(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🚫 حظر قناة", callback_data=CallbackData.BAN_CHANNEL)],
        [InlineKeyboardButton("✅ إلغاء حظر قناة", callback_data=CallbackData.UNBAN_CHANNEL)],
        [InlineKeyboardButton("📋 القنوات المحظورة", callback_data=CallbackData.BANNED_CHANNELS)],
        ...
        try:
            ...
            for row in rows:
                ...
                if not has_permission:
                    continue
                # ✅ التحقق من أن القناة غير محظورة
                if await db_is_channel_banned(ch_tele_id):
                    logger.info(f"⛔ القناة {ch_tele_id} محظورة، تم تخطيها")
                    continue
...
async def message_handler_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'waiting_for_log_channel':
        ...
        try:
            ...
            if identifier_clean.startswith('-100') or identifier_clean.lstrip('-').isdigit():
                ...
                chat = await context.bot.get_chat(f"@{identifier_clean}")
                chat_id = chat.id
            bot_member = await context.bot.get_chat_member(chat_id, context.bot.id)
            if bot_member.status not in ['administrator', 'creator']:
                await update.message.reply_text("❌ **البوت ليس مشرفاً في هذه القناة.**", parse_mode="MarkdownV2")
    ...
    if state == 'waiting_for_schedule_post':
        ...
        await main_menu_callback(update, context)
        return
    if state and state.startswith('waiting_for_ban_user_'):
        chat_id = int(state.split('_')[-1])
        context.user_data.pop('state', None)
        ...
        try:
            ...
            await update.message.reply_text("❌ معرف مستخدم غير صالح")
        return
    if state and state.startswith('waiting_for_mute_user_'):
        chat_id = int(state.split('_')[-1])
        context.user_data.pop('state', None)
        parts = text.split(maxsplit=1)
        reason = parts[1] if len(parts) > 1 else ""
        try:
            ...
            await update.message.reply_text("❌ معرف مستخدم غير صالح")
        return
    if state and state.startswith('waiting_for_warn_user_'):
        chat_id = int(state.split('_')[-1])
        context.user_data.pop('state', None)
        parts = text.split(maxsplit=1)
        reason = parts[1] if len(parts) > 1 else ""
        try:
            ...
            await update.message.reply_text("❌ معرف مستخدم غير صالح")
        return
    if state and state.startswith('waiting_for_kick_user_'):
        chat_id = int(state.split('_')[-1])
        context.user_data.pop('state', None)
        parts = text.split(maxsplit=1)
        reason = parts[1] if len(parts) > 1 else ""
        try:
            ...
            await update.message.reply_text("❌ معرف مستخدم غير صالح")
        return
    if state and state.startswith('waiting_for_restrict_user_'):
        chat_id = int(state.split('_')[-1])
        context.user_data.pop('state', None)
        parts = text.split(maxsplit=1)
        reason = parts[1] if len(parts) > 1 else ""
        try:
            ...
            await update.message.reply_text("❌ معرف مستخدم غير صالح")
        return
    if state and state.startswith('waiting_for_unban_user_'):
        chat_id = int(state.split('_')[-1])
        context.user_data.pop('state', None)
        try:
            target_id = int(text)
            ...
            await update.message.reply_text("❌ معرف مستخدم غير صالح")
        return
    if state and state.startswith('waiting_for_pin_message_'):
        chat_id = int(state.split('_')[-1])
        context.user_data.pop('state', None)
...
async def main():
    ...
    application.add_handler(CommandHandler("banchannel", ban_channel_command))
    application.add_handler(CommandHandler("unbanchannel", unban_channel_command))
    application.add_handler(CommandHandler("bannedchannels", banned_channels_command))
    ...
    application.add_handler(CallbackQueryHandler(ban_channel_command_callback, pattern=f"^{CallbackData.BAN_CHANNEL}$"))
    application.add_handler(CallbackQueryHandler(unban_channel_command_callback, pattern=f"^{CallbackData.UNBAN_CHANNEL}$"))
    application.add_handler(CallbackQueryHandler(banned_channels_command_callback, pattern=f"^{CallbackData.BANNED_CHANNELS}$"))
    ...
    commands = [
        ...
        BotCommand("banchannel", "حظر قناة مستخدم"),
        BotCommand("unbanchannel", "إلغاء حظر قناة مستخدم"),
        BotCommand("bannedchannels", "عرض القنوات المحظورة"),
    ]
    ...
    print(f"   • 🆕 إدارة حظر قنوات المستخدمين (banchannel, unbanchannel, bannedchannels)")
    try:
...
# ============================================================
# دوال حظر القنوات - الإضافة الكاملة
# ============================================================
