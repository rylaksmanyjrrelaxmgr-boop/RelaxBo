...
الإصدار: 18.0.3 - نسخة عالمية (Portable) - مُحدّثة بإصلاح المالك المخفي
المطور: @RelaxMgr
"""
...
def get_base_path() -> Path:
    """الحصول على المسار الأساسي للبوت بشكل مستقل عن البيئة"""
    return Path(__file__).parent.resolve()

BASE_PATH = get_base_path()

def get_writable_path(base_path: Path, subdir: str) -> Path:
    """الحصول على مسار قابل للكتابة"""
    paths_to_try = [
        base_path / subdir,
...
def get_temp_path() -> Path:
    """الحصول على مسار مؤقت مناسب للبيئة"""
...
# تحسينات الأداء - استخدام cachetools
try:
...
# ===================== تحميل ملفات البيئة =====================
def load_env_files():
    """تحميل ملفات البيئة من عدة مواقع ممكنة"""
    from dotenv import load_dotenv
...
def get_env_or_default(key: str, default: any, env_type: type = str) -> any:
    """الحصول على متغير بيئة مع تحويل النوع"""
    value = os.getenv(key)
    if value is None:
        return default
    try:
...
# user_translation_settings مخزنة الآن في قاعدة البيانات، نستخدم هذا القاموس كـ cache مؤقت
...
def check_single_instance():
    """التحقق من تشغيل نسخة واحدة باستخدام Socket (يعمل في كل البيئات)"""
    try:
...
def get_ram_usage():
    """الحصول على استخدام الرام بشكل مستقل عن نظام التشغيل"""
    try:
        import psutil
...
def contains_link(text):
    # التحقق من جميع أنواع الروابط
    patterns = [
        r'https?://\S+',      # http:// و https://
        r'www\.\S+',          # www.
        r't\.me/\S+',         # t.me
        r'telegram\.me/\S+',  # telegram.me
        r'\b[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)+\S*'  # example.com (بدون http)
    ]
...
class SmartTranslator:
    """مترجم ذكي مع تخزين مؤقت وتجميع الطلبات"""
    def __init__(self):
        self.cache = {}
    ...
    async def translate(self, text: str, target_lang: str) -> str:
        """ترجمة نص مع تخزين مؤقت وتجميع الطلبات المتشابهة"""
...
# ---- دوال الترجمة المخزنة في قاعدة البيانات ----
async def get_user_translation_language(user_id: int) -> str:
    """استرجاع لغة الترجمة للمستخدم من قاعدة البيانات مع cache مؤقت"""
...
async def set_user_translation_language(user_id: int, lang: str):
    """تحديث لغة الترجمة للمستخدم في قاعدة البيانات وتحديث cache"""
    async def _set(conn):
...
class AdvancedRateLimiter:
    """نظام متقدم للحد من معدل استخدام الأوامر"""
    def __init__(self):
    ...
    try:
        ...
        for admin in admins:
            if admin.user.id == user_id:
                return True
        for admin in admins:
            if admin.status == 'creator' and getattr(admin, 'is_anonymous', False):
                return True
    except Exception as e:
...
class MessageProcessor:
    """معالج رسائل محسن مع تجميع الطلبات"""
    def __init__(self):
...
async def db_reset_posts_to_unpublished(channel_db_id: int, user_id: int):
...
async def db_get_channel_stats(channel_db_id: int) -> dict:
    """
    الحصول على إحصائيات متقدمة لقناة معينة
    """
    async def _get_stats(conn):
        # 1. إحصائيات المنشورات الأساسية
        cur = await conn.execute(
            """
            SELECT 
        ...
        # 2. حساب متوسط الوقت بين المنشورات
        avg_time_between = 0
        ...
        # 3. أفضل وقت للنشر (الساعة الأكثر نشاطاً)
        best_hour = 0
        best_day = 0
        ...
        # 4. إحصائيات إضافية (اليوم، الأسبوع، الشهر)
...
async def db_get_channel_growth(channel_db_id: int, days: int = 30) -> dict:
    """
    الحصول على بيانات نمو القناة (عدد المنشورات خلال الأيام الماضية)
    """
...
async def get_google_drive_service():
    ...
    try:
        ...
        logger.error(f"خطأ في Google Drive: {e}")
        return None

async def upload_backup_to_drive(backup_path: Path) -> str:
    ...
    try:
        ...
        logger.info(f"✅ تم رفع النسخة الاحتياطية إلى Google Drive: {file.get('id')}")
        results = service.files().list(q=f"'{GOOGLE_DRIVE_FOLDER_ID}' in parents", orderBy="createdTime desc", pageSize=20).execute()
...
async def download_backup_from_drive(file_name: str) -> Path:
    try:
        service = await get_google_drive_service()
        if not service:
        ...
        files = results.get('files', [])
        if not files:
            raise Exception(f"الملف {file_name} غير موجود")
        file_id = files[0]['id']
        ...
        logger.error(f"❌ فشل تحميل النسخة من Google Drive: {e}")
        raise

async def list_cloud_backups() -> list:
    if not CLOUD_BACKUP_ENABLED:
        return []
    try:
        service = await get_google_drive_service()
        if not service:
            return []
        results = service.files().list(q=f"'{GOOGLE_DRIVE_FOLDER_ID}' in parents", orderBy="createdTime desc", fields="files(id, name, createdTime)").execute()
...
class CallbackData:
    ...
    VERIFY_HIDDEN_OWNER = "verify_hidden_owner"

# ===================== دوال الكولباك =====================

# ... (جميع دوال الكولباك موجودة هنا، نفس الكود الأصلي)
...
async def my_groups_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not groups:
        ...
        await safe_edit_markdown(query, get_text(uid, 'no_groups'), reply_markup=kb)
        return
    kb = []
    for chat_id, chat_name, username, banned in groups:
        name = chat_name[:40] + "..." if len(chat_name) > 43 else chat_name
        kb.append([
            InlineKeyboardButton(f"{'⛔' if banned else '✅'} {name}", 
                                callback_data=f"{CallbackData.GROUPS_SETTINGS_PREFIX}{chat_id}"),
            InlineKeyboardButton("🔐 أمان", 
                                callback_data=f"{CallbackData.SECURITY_SELECT_GROUP}{chat_id}")
        ])
    kb.append([InlineKeyboardButton("🔄 تحديث القائمة", callback_data=CallbackData.SECURITY_REFRESH_GROUPS)])
    kb.append([InlineKeyboardButton(get_text(uid, 'back'), callback_data=CallbackData.BACK)])
    await safe_edit_markdown(query, "👥 **مجموعاتي**\n━━━━━━━━━━━━━━━━━━━━━━\nاختر مجموعة للإعدادات أو الأمان:", 
...
async def channel_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) >= 2:
        ...
        channel_db_id = context.user_data.get('active_channel')
    ...
    ch_tele_id, ch_name = ch_info
    
    stats = await db_get_channel_stats(channel_db_id)
    
    if stats['total_posts'] == 0:
        text = f"""📊 **إحصائيات القناة: {ch_name}**
━━━━━━━━━━━━━━━━━━━━━━
📭 **لا توجد منشورات بعد**
💡 أضف منشورات أولاً باستخدام زر "إضافة 15 منشوراً"
━━━━━━━━━━━━━━━━━━━━━━"""
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📥 إضافة منشورات", callback_data=CallbackData.POSTS_ADD_15)],
    ...
    best_day_name = day_names[stats['best_publish_day']] if stats['best_publish_day'] is not None else "غير محدد"
    
    avg_time_str = ""
    if stats['avg_time_between_posts'] > 0:
        hours = stats['avg_time_between_posts']
        if hours < 1:
            avg_time_str = f"{round(hours * 60)} دقيقة"
        elif hours < 24:
            avg_time_str = f"{round(hours)} ساعة"
        else:
            avg_time_str = f"{round(hours / 24)} يوم"
    else:
        avg_time_str = "غير محدد"
...
━━━━━━━━━━━━━━━━━━━━━━
⏱️ **التوقيت:**
• متوسط الوقت بين المنشورات: `{avg_time_str}`
• أفضل ساعة للنشر: `{stats['best_publish_hour']}:00`
• أفضل يوم للنشر: `{best_day_name}`
━━━━━━━━━━━━━━━━━━━━━━
📅 **آخر النشاطات:**
• منشورات اليوم: `{stats['published_today']}`
...
━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📈 تفاصيل النمو", callback_data=f"{CallbackData.CHANNEL_GROWTH}:{channel_db_id}")],
        [InlineKeyboardButton("🔄 تحديث الإحصائيات", callback_data=f"{CallbackData.CHANNEL_STATS_REFRESH}:{channel_db_id}")],
        [InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.BACK)]
    ])
...
async def channel_growth_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    growth = await db_get_channel_growth(channel_db_id, 30)
    
    if growth['total_days'] == 0:
        await query.edit_message_text("📭 لا توجد بيانات نمو للقناة")
        return
    ...
    try:
        ...
        fig.update_layout(
            title=f'نمو القناة: {ch_name}',
            template='plotly_dark',
            xaxis=dict(title='التاريخ', showgrid=True, gridcolor='rgba(255,255,255,0.1)'),
            yaxis=dict(title='عدد المنشورات', showgrid=True, gridcolor='rgba(255,255,255,0.1)'),
            yaxis2=dict(
                title='المشاهدات',
                overlaying='y',
                side='right',
        ...
        await query.edit_message_text(f"❌ خطأ في إنشاء الرسم البياني: {str(e)[:100]}")
        
        text = f"📈 **بيانات نمو القناة: {ch_name}**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for i, date in enumerate(growth['dates']):
            text += f"• {date}: {growth['counts'][i]} منشور، {growth['views'][i]} مشاهدة\n"
            if i > 20:
                text += f"\n... و {len(growth['dates']) - i - 1} أيام أخرى"
                break
...
async def my_channel_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not summary or summary['total_posts'] == 0:
        text = f"""📊 **ملخص إحصائيات قنواتك**
━━━━━━━━━━━━━━━━━━━━━━
📡 عدد القنوات: `{len(channels)}`
📭 لا توجد منشورات في أي قناة
💡 أضف منشورات أولاً باستخدام زر "إضافة 15 منشوراً"
━━━━━━━━━━━━━━━━━━━━━━"""
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📥 إضافة منشورات", callback_data=CallbackData.POSTS_ADD_15)],
...
• 👁️ {best['views']} مشاهدة
• 📝 {best['posts']} منشور
• 📊 متوسط المشاهدات: {best['avg_views']}
━━━━━━━━━━━━━━━━━━━━━━"""
    
    keyboard_buttons = []
    for ch_db_id, ch_tele_id, ch_name, banned in channels[:10]:
        keyboard_buttons.append([
            InlineKeyboardButton(f"📊 {ch_name[:20]}", callback_data=f"{CallbackData.CHANNEL_STATS}:{ch_db_id}")
        ])
    ...
    await safe_edit_markdown(query, text, reply_markup=InlineKeyboardMarkup(keyboard_buttons))
...
• إصلاح شامل لمشكلة المالك المخفي
• إضافة أمر /verify_hidden_owner للتحقق بكلمة مرور
...
# ===================== دوال الإحالات والإشعارات والترجمة =====================
...
5. إذا كنت مالكاً مخفياً، استخدم الأمر /verify_hidden_owner في المجموعة"""
        await safe_edit_markdown(query, text, reply_markup=keyboard)
        return
    keyboard = []
    for group in groups:
...
🔒 للمالك المخفي: استخدم /verify_hidden_owner في المجموعة أولاً"""
...
async def help_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        await safe_send_markdown(update.callback_query.message, get_text(uid, 'help'))
        await update.callback_query.answer()
    elif update.message:
        await safe_send_markdown(update.message, get_text(uid, 'help'))
...
async def rank_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        await update.callback_query.answer()
        await safe_send_markdown(update.callback_query.message, text)
    else:
        await safe_send_markdown(update.message, text)
...
async def top_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not top_users:
        ...
        if update.callback_query:
            await update.callback_query.answer()
            await safe_send_markdown(update.callback_query.message, msg)
        else:
...
✅ **البوت:** @{BOT_USERNAME}"""
    ...
    if update.callback_query:
        ...
        await safe_send_markdown(update.message, text, reply_markup=keyboard)
...
async def verify_hidden_owner_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    التحقق من هوية المالك المخفي باستخدام كلمة مرور
    هذا الأمر يحل مشكلة المالك المخفي نهائياً
    """
    if update.message is None:
        return
    user = update.effective_user
    ...
    try:
        ...
        await update.message.reply_text(f"❌ خطأ في التحقق من صلاحيات البوت: {e}")
        return
    ...
    if not args:
        ...
        await update.message.reply_text(
            ...
            "📌 **الاستخدام:**\n"
            "`/verify_hidden_owner كلمة_المرور`\n\n"
            "🔑 **كلمة المرور الافتراضية:** `hidden_master`\n\n"
            "💡 **كيف يعمل؟**\n"
            "1. تأكد من أن البوت مشرف في المجموعة\n"
            "2. استخدم الأمر مع كلمة المرور الصحيحة\n"
            "3. سيتم تسجيلك كمالك مخفي تلقائياً\n"
            "4. بعد التسجيل، ستتمكن من استخدام جميع الأوامر الإدارية\n\n"
            "⚠️ يمكن تغيير كلمة المرور من لوحة التحكم",
            parse_mode="MarkdownV2"
        )
        return
    
    password = " ".join(args)  # دعم كلمات مرور متعددة الكلمات
    
    # قراءة كلمة المرور من قاعدة البيانات
    async def _get_password(conn):
        cur = await conn.execute("SELECT value FROM settings WHERE key='hidden_owner_password'")
        row = await cur.fetchone()
        return row[0] if row else "hidden_master"
    
    stored_password = await execute_db(_get_password)
    ...
    if password == stored_password:
        ...
        # تحديث حالة المالك المخفي في قاعدة البيانات
        await security_audit.log("HIDDEN_OWNER_VERIFIED", user.id, {
            "chat_id": chat.id,
        ...
        await update.message.reply_text(
            "✅ **تم التحقق من هويتك كمالك مخفي بنجاح!**\n\n"
            "🔓 الآن يمكنك استخدام جميع الأوامر الإدارية:\n"
            "• `/security` - إعدادات الأمان\n"
            "• `/panel` - لوحة التحكم\n"
            "• `/syncgroup` - تفعيل البوت\n"
            "• وجميع أوامر الإدارة الأخرى\n\n"
            "🛡️ لن يظهر اسمك في قائمة المشرفين",
            parse_mode="MarkdownV2"
        )
    else:
        await update.message.reply_text(
            ...
            "إذا نسيت كلمة المرور، تواصل مع المطور.",
            parse_mode="MarkdownV2"
        )
        await security_audit.log("HIDDEN_OWNER_VERIFICATION_FAILED", user.id, {
            "chat_id": chat.id,
...
async def register_hidden_owner_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    تسجيل المالك المخفي - تم تعديله لاستخدام نظام التحقق بكلمة المرور
    """
    if update.message is None:
        return
    user = update.effective_user
    ...
    # استخدام detect_owner_type للكشف عن المالك
    ...
    if owner_info.get('is_hidden'):
        # المالك مخفي، نطلب التحقق بكلمة المرور
        await update.message.reply_text(
            "🔒 **تم اكتشاف مالك مخفي في هذه المجموعة!**\n\n"
            "للتأكيد كمالك مخفي، استخدم الأمر:\n"
            f"`/verify_hidden_owner {stored_password}`\n\n"
            "💡 **ملاحظة:** يمكنك تغيير كلمة المرور من لوحة التحكم\n"
            "🔑 كلمة المرور الافتراضية: `hidden_master`",
            parse_mode="MarkdownV2"
        )
    else:
        # مالك عادي أو ليس مالكاً
        # نتحقق من الصلاحيات العادية
        if not await is_authorized_in_group(context.bot, chat.id, user.id):
            await update.message.reply_text(
                "❌ **غير مصرح**\n\n"
                "أنت لست مشرفاً في هذه المجموعة.\n"
                "إذا كنت المالك المخفي، استخدم:\n"
                f"`/verify_hidden_owner {stored_password}`",
                parse_mode="MarkdownV2"
            )
            return
        
        # تسجيل المستخدم كمالك مخفي (مع تحذير)
        await db_register_hidden_owner_group(chat.id, user.id)
        await update.message.reply_text(
            "✅ **تم تسجيل هذه المجموعة كمجموعة يملكها مالك مخفي!**\n\n"
            "🔓 الآن يمكنك استخدام الأوامر الإدارية دون ظهور اسمك.\n\n"
            "⚠️ **ملاحظة:** إذا كنت المالك الحقيقي، استخدم:\n"
            f"`/verify_hidden_owner {stored_password}`\n"
            "للتأكيد الكامل.",
            parse_mode="MarkdownV2"
        )
...
async def on_bot_added(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.message.new_chat_members:
        if member.id == bot_id:
            ...
            try:
                await safe_send_markdown(chat, chat.id, "🤖 **تم تفعيل البوت بنجاح!**\n📌 استخدم /security للإعدادات\n🔒 للمالك المخفي: /verify_hidden_owner\n🔧 لوحة التحكم: /panel\n🛠️ الإجراءات المتقدمة: /panel ثم اختر إجراءات متقدمة")
            except:
                pass
            break
...
async def init_db_improved():
    async with aiosqlite.connect(str(DB_PATH), timeout=DB_TIMEOUT) as conn:
        ...
        # إضافة كلمة المرور الافتراضية للمالك المخفي
        await conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('hidden_owner_password', 'hidden_master')")
...
async def main():
    ...
    # الأوامر الجديدة للمالك المخفي
    application.add_handler(CommandHandler("register_hidden_owner", register_hidden_owner_handler))
    application.add_handler(CommandHandler("verify_hidden_owner", verify_hidden_owner_handler))
    ...
    commands = [
        ...
        BotCommand("verify_hidden_owner", "التحقق من المالك المخفي"),
    ...
    print(f"🚀 تم تشغيل {BOT_NAME} (الإصدار 18.0.3)")
    print(f"✅ جميع التحسينات المطلوبة تم تطبيقها:")
    print(f"   • إصلاح شامل لمشكلة المالك المخفي")
    print(f"   • إضافة أمر /verify_hidden_owner للتحقق بكلمة مرور")
    print(f"   • دعم كامل للمالك المخفي في جميع الأوامر")
    print(f"   • إحصائيات متقدمة للقنوات")
    print(f"   • رسم بياني لنمو القناة")
    print(f"   • تحسين الأداء والاستقرار")
    print(f"   • وإصلاحات أمنية متعددة")
