...
# ذاكرة تخزين مؤقت للخدمة
_DRIVE_SERVICE_CACHE = None
_DRIVE_SERVICE_CACHE_TIME = 0
_DRIVE_SERVICE_CACHE_TTL = 3600  # ساعة واحدة
...
async def start_web_server():
    ...
    await runner.setup()
    # محاولة تشغيل على المنفذ المحدد، وإن فشل جرب منفذ عشوائي
...
async def channel_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not channel_db_id:
        await query.edit_message_text(get_text(uid, 'select_channel_first'))
        return
    ch_info = await db_get_channel_info(channel_db_id)
    if not ch_info:
        await query.edit_message_text(get_text(uid, 'channel_not_found'))
        return
    ch_tele_id, ch_name = ch_info
    stats = await db_get_channel_stats(channel_db_id)
    if stats['total_posts'] == 0:
        text = f"""📊 **{get_text(uid, 'channel_stats')}: {ch_name}**
━━━━━━━━━━━━━━━━━━━━━━
{get_text(uid, 'no_posts_yet')}
{get_text(uid, 'add_posts_first')}
━━━━━━━━━━━━━━━━━━━━━━"""
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(get_text(uid, 'add_15_posts'), callback_data=CallbackData.POSTS_ADD_15)],
            [InlineKeyboardButton(get_text(uid, 'back_btn'), callback_data=CallbackData.BACK)]
        ])
        await safe_edit_markdown(query, text, reply_markup=keyboard)
        return
    day_names = [get_text(uid, 'sunday'), get_text(uid, 'monday'), get_text(uid, 'tuesday'), 
                 get_text(uid, 'wednesday'), get_text(uid, 'thursday'), get_text(uid, 'friday'), 
                 get_text(uid, 'saturday')]
    best_day_name = day_names[stats['best_publish_day']] if stats['best_publish_day'] is not None else get_text(uid, 'not_specified')
    avg_time_str = ""
    if stats['avg_time_between_posts'] > 0:
        hours = stats['avg_time_between_posts']
        if hours < 1:
            avg_time_str = f"{round(hours * 60)} {get_text(uid, 'minutes')}"
        elif hours < 24:
            avg_time_str = f"{round(hours)} {get_text(uid, 'hours')}"
        else:
            avg_time_str = f"{round(hours / 24)} {get_text(uid, 'days')}"
    else:
        avg_time_str = get_text(uid, 'not_specified')
    text = f"""📊 **{get_text(uid, 'channel_stats')}: {ch_name}**
━━━━━━━━━━━━━━━━━━━━━━
📊 **{get_text(uid, 'posts')}:**
• {get_text(uid, 'total_posts_label')}: `{stats['total_posts']}`
• {get_text(uid, 'published')}: `{stats['published_posts']}`
• {get_text(uid, 'unpublished')}: `{stats['unpublished_posts']}`
━━━━━━━━━━━━━━━━━━━━━━
👁️ **{get_text(uid, 'views')}:**
• {get_text(uid, 'total_views_label')}: `{stats['total_views']}`
• {get_text(uid, 'avg_views_per_channel')}: `{stats['avg_views']}`
━━━━━━━━━━━━━━━━━━━━━━
⏱️ **{get_text(uid, 'timing')}:**
• {get_text(uid, 'avg_time_between')}: `{avg_time_str}`
• {get_text(uid, 'best_hour')}: `{stats['best_publish_hour']}:00`
• {get_text(uid, 'best_day')}: `{best_day_name}`
━━━━━━━━━━━━━━━━━━━━━━
📅 **{get_text(uid, 'recent_activity')}:**
• {get_text(uid, 'today_posts')}: `{stats['published_today']}`
• {get_text(uid, 'this_week')}: `{stats['published_this_week']}`
• {get_text(uid, 'this_month')}: `{stats['published_this_month']}`
━━━━━━━━━━━━━━━━━━━━━━"""
    if stats['most_viewed_post']:
        text += f"""🏆 **{get_text(uid, 'most_viewed')}:**
• {stats['most_viewed_post']['text']}
• 👁️ {stats['most_viewed_post']['views']} {get_text(uid, 'views')}
━━━━━━━━━━━━━━━━━━━━━━"""
    if stats['least_viewed_post']:
        text += f"""📉 **{get_text(uid, 'least_viewed')}:**
• {stats['least_viewed_post']['text']}
• 👁️ {stats['least_viewed_post']['views']} {get_text(uid, 'views')}
━━━━━━━━━━━━━━━━━━━━━━"""
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(get_text(uid, 'channel_growth'), callback_data=f"{CallbackData.CHANNEL_GROWTH}:{channel_db_id}")],
        [InlineKeyboardButton(get_text(uid, 'refresh_stats'), callback_data=f"{CallbackData.CHANNEL_STATS_REFRESH}:{channel_db_id}")],
        [InlineKeyboardButton(get_text(uid, 'back_btn'), callback_data=CallbackData.BACK)]
    ])
    await safe_edit_markdown(query, text, reply_markup=keyboard)
...
async def channel_growth_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if growth['total_days'] == 0:
        await query.edit_message_text(get_text(uid, 'no_stats_data'))
        return
    try:
        import plotly.graph_objects as go
        import plotly.utils
        ...
        fig.add_trace(go.Scatter(
            ...
            y=growth['views'],
            mode='lines+markers',
            name=get_text(uid, 'views'),
            yaxis='y2',
            line=dict(color='#3498db', width=2, dash='dot'),
            marker=dict(size=6, color='#2980b9')
        ))
        fig.update_layout(
            title=f'{get_text(uid, "channel_growth")}: {ch_name}',
            template='plotly_dark',
            xaxis=dict(title=get_text(uid, 'date'), showgrid=True, gridcolor='rgba(255,255,255,0.1)'),
            yaxis=dict(title=get_text(uid, 'posts'), showgrid=True, gridcolor='rgba(255,255,255,0.1)'),
            yaxis2=dict(
                title=get_text(uid, 'views'),
                overlaying='y',
                side='right',
                showgrid=False
            ),
        ...
        await context.bot.send_photo(
            chat_id=uid,
            photo=img_bytes,
            caption=f"📈 **{get_text(uid, 'channel_growth')}: {ch_name}**\n━━━━━━━━━━━━━━━━━━━━━━\n📊 {get_text(uid, 'total_posts_label')}: {growth['total_posts']}\n👁️ {get_text(uid, 'total_views_label')}: {growth['total_views']}",
        ...
        text = f"📈 **{get_text(uid, 'channel_growth')}: {ch_name}**\n━━━━━━━━━━━━━━━━━━━━━━\n"
        for i, date in enumerate(growth['dates']):
            text += f"• {date}: {growth['counts'][i]} {get_text(uid, 'posts')}، {growth['views'][i]} {get_text(uid, 'views')}\n"
            if i > 20:
                text += f"\n... و {len(growth['dates']) - i - 1} {get_text(uid, 'days')} أخرى"
                break
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(get_text(uid, 'back_btn'), callback_data=CallbackData.BACK)]
        ])
...
async def channel_stats_refresh_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not channel_db_id:
        await query.edit_message_text(get_text(uid, 'select_channel_first'))
        return
    await query.edit_message_text(get_text(uid, 'refreshing_stats'))
...
async def my_channel_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channels = await db_get_channels(uid)
    if not channels:
        await query.edit_message_text(get_text(uid, 'no_channels_registered'))
        return
    summary = await db_get_channel_stats_summary(uid)
    if not summary or summary['total_posts'] == 0:
        text = f"""{get_text(uid, 'my_stats_summary')}
━━━━━━━━━━━━━━━━━━━━━━
📡 {get_text(uid, 'total_channels')} `{len(channels)}`
{get_text(uid, 'no_posts_yet')}
{get_text(uid, 'add_posts_first')}
━━━━━━━━━━━━━━━━━━━━━━"""
        keyboard = InlineKeyboardMarkup([
    ...
    text = f"""{get_text(uid, 'my_stats_summary')}
━━━━━━━━━━━━━━━━━━━━━━
📡 **{get_text(uid, 'channels')}:**
• {get_text(uid, 'total_channels')}: `{summary['total_channels']}`
• {get_text(uid, 'active_channels')}: `{summary['active_channels']}`
━━━━━━━━━━━━━━━━━━━━━━
📝 **{get_text(uid, 'posts')}:**
• {get_text(uid, 'total_posts_label')}: `{summary['total_posts']}`
• {get_text(uid, 'published')}: `{summary['total_published']}`
• {get_text(uid, 'unpublished')}: `{summary['total_posts'] - summary['total_published']}`
━━━━━━━━━━━━━━━━━━━━━━
👁️ **{get_text(uid, 'views')}:**
• {get_text(uid, 'total_views_label')}: `{summary['total_views']}`
• {get_text(uid, 'avg_views_per_channel')}: `{summary['avg_views_per_channel']}`
━━━━━━━━━━━━━━━━━━━━━━"""
    if summary['best_channel']:
        best = summary['best_channel']
        text += f"""{get_text(uid, 'best_channel')}
• {best['name']}
• 👁️ {best['views']} {get_text(uid, 'views')}
• 📝 {best['posts']} {get_text(uid, 'posts')}
• 📊 {get_text(uid, 'avg_views_per_channel')}: {best['avg_views']}
━━━━━━━━━━━━━━━━━━━━━━"""
    keyboard_buttons = []
    ...
    for member in update.message.new_chat_members:
        if member.id == bot_id:
            ...
            if owner_info.get('is_hidden', False):
                ...
                # المالك غير مخفي → نسجّله كمالك عادي (اختياري)
