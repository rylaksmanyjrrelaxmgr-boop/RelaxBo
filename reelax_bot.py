...
الإصدار: 19.0.8 - قراءة الترجمات من ملف خارجي (محسّن)
المطور: @RelaxMgr
"""
...
TRANSLATIONS_PATH = get_writable_path(BASE_PATH, "translations")  # مسار ملفات الترجمة
...
TRANSLATIONS_PATH.mkdir(parents=True, exist_ok=True)  # إنشاء مجلد الترجمات
...
ensure_package("pyyaml", "yaml")  # لدعم قراءة ملفات YAML
...
try:
    ...
    if DEEPL_API_KEY:
        ...
        print("✅ تم تفعيل DeepL API للترجمة")
    else:
        DEEPL_AVAILABLE = False
        print("⚠️ DEEPL_API_KEY غير موجودة، سيتم استخدام Google Translate")
except ImportError:
    DEEPL_AVAILABLE = False
    print("⚠️ مكتبة deep-translator غير مثبتة، سيتم استخدام Google Translate")

# ============================================================
# نظام تحميل الترجمات من ملف خارجي (محسّن)
# ============================================================

class TranslationManager:
    """
    مدير الترجمات - يقوم بتحميل الترجمات من ملف JSON أو YAML
    ويدعم تحديثها ديناميكياً دون إعادة تشغيل البوت
    """
    ...
    def __init__(self, translations_path: Path):
        ...
        self.supported_languages = ['ar', 'en', 'fr', 'tr', 'zh', 'ru', 'de', 'es', 'it', 'pt', 'ja', 'ko']
    ...
    def get_translations_file_path(self) -> Path:
        """الحصول على مسار ملف الترجمات (يفضل JSON)"""
        json_file = self.translations_path / "translations.json"
        if json_file.exists():
            return json_file
        yaml_file = self.translations_path / "translations.yaml"
        if yaml_file.exists():
            return yaml_file
        yml_file = self.translations_path / "translations.yml"
        if yml_file.exists():
            return yml_file
        return json_file
    
    def load_translations(self) -> dict:
        """تحميل الترجمات من الملف"""
        try:
            ...
            if file_path.suffix in ['.yaml', '.yml']:
                try:
                    ...
                    if data and isinstance(data, dict):
                        ...
                        print(f"✅ تم تحميل الترجمات من {file_path} (YAML)")
                ...
                if data and isinstance(data, dict):
                    self.translations = data
                    self.last_loaded = datetime.now()
                    print(f"✅ تم تحميل الترجمات من {file_path} (JSON)")
                    return self.translations
            ...
            print(f"⚠️ ملف الترجمات غير موجود، سيتم إنشاء ملف افتراضي")
            self.create_default_translations_file()
            return self.load_translations()
        
        except Exception as e:
            print(f"❌ فشل تحميل الترجمات: {e}")
            self.create_default_translations_file()
            return self.load_translations()
        
        return self.translations
    
    def create_default_translations_file(self):
        """إنشاء ملف ترجمات افتراضي (نسخة محسّنة)"""
        default_translations = {
            "ar": {
                ...
                "back": "🔙 رجوع",
                "updated": "✅ تم التحديث",
                "reload_translations_success": "✅ تم إعادة تحميل الترجمات بنجاح!",
                "reload_translations_fail": "❌ فشل إعادة تحميل الترجمات، تحقق من الملف.",
                "claim_success": "✅ تم تسجيلك كمالك مخفي لهذه المجموعة.",
                "claim_fail": "❌ يجب أن تكون مشرفاً في المجموعة لتسجيل مالك مخفي.",
                "group_added": "✅ تم تفعيل البوت في المجموعة {0}\nأنت الآن المالك المخفي."
            },
            "en": {
                ...
                "back": "🔙 Back",
                "updated": "✅ Updated",
                "reload_translations_success": "✅ Translations reloaded successfully!",
                "reload_translations_fail": "❌ Failed to reload translations, check file.",
                "claim_success": "✅ You are now registered as hidden owner for this group.",
                "claim_fail": "❌ You must be an admin in this group to register as hidden owner.",
                "group_added": "✅ Bot activated in group {0}\nYou are now the hidden owner."
            }
        }
        
        try:
            file_path = self.translations_path / "translations.json"
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(default_translations, f, ensure_ascii=False, indent=2)
            print(f"✅ تم إنشاء ملف الترجمات الافتراضي: {file_path}")
        except Exception as e:
            print(f"❌ فشل إنشاء ملف الترجمات: {e}")
    
    def get_text(self, lang: str, key: str, *args) -> str:
        """الحصول على نص مترجم حسب اللغة والمفتاح، مع دعم التنسيق"""
    ...
    async def reload(self):
        """إعادة تحميل الترجمات من الملف (تحديث ديناميكي)"""
        async with self._lock:
            self.load_translations()
            print("🔄 تم إعادة تحميل الترجمات")
            return True

# إنشاء مدير الترجمات
translator = TranslationManager(TRANSLATIONS_PATH)

# ===================== دالة اختصار للحصول على النص المترجم =====================

def get_text(user_id: int, key: str, *args) -> str:
    """الحصول على النص المترجم حسب لغة المستخدم مع دعم التنسيق"""
    lang = user_language.get(user_id, 'ar')
    return translator.get_text(lang, key, *args)
...
def load_env_files():
    from dotenv import load_dotenv
    env_files = [".env", ".env.local", str(BASE_PATH / ".env"), str(BASE_PATH / "config" / ".env"), str(Path.home() / ".bot" / ".env")]
...
# ===================== تحسينات نظام الترجمة (دعم جميع اللغات) =====================
    ...
    def __init__(self):
        ...
        if DEEPL_AVAILABLE and DEEPL_API_KEY:
            try:
                ...
                print("✅ DeepL API جاهز للترجمة")
            except:
                self.deepl_translator = None
                print("⚠️ فشل تهيئة DeepL")
    async def get_session(self):
        if self.session is None:
            self.session = aiohttp.ClientSession()
        return self.session
    async def translate_with_deepl(self, text: str, target: str) -> str:
        if not self.deepl_translator:
            return None
        try:
            lang_map = {'ar': 'AR', 'en': 'EN-US', 'fr': 'FR', 'tr': 'TR', 'zh': 'ZH', 'ru': 'RU', 'de': 'DE', 'es': 'ES', 'it': 'IT', 'pt': 'PT-BR', 'ja': 'JA', 'ko': 'KO'}
            target_lang = lang_map.get(target, target.upper())
            result = self.deepl_translator.translate_text(text, target_lang=target_lang)
            return result.text
        except Exception as e:
            logger.warning(f"فشل الترجمة بـ DeepL: {e}")
            return None
    async def translate_with_google(self, text: str, target: str) -> str:
        try:
            translator = GoogleTranslator(target=target)
            return translator.translate(text)
        except Exception as e:
            logger.warning(f"فشل الترجمة بـ Google: {e}")
            try:
    ...
    async def translate(self, text: str, target: str) -> str:
        ...
        try:
            ...
            if self.deepl_translator:
                try:
                    deepl_result = await self.translate_with_deepl(text, target)
                    if deepl_result:
                        translated_text = deepl_result
                except:
                    pass
            if translated_text == text:
                google_result = await self.translate_with_google(text, target)
                if google_result and google_result != text:
                    translated_text = google_result
            await _translation_cache.set(cache_key, translated_text)
...
async def translate_text(text: str, target_lang: str, source_lang: str = 'auto') -> str:
    if target_lang == 'off' or target_lang == 'auto':
        return text
...
# ===================== نظام اللغة المحسن (مع حفظ في قاعدة البيانات) =====================
user_language = {}
...
async def get_user_language_db(user_id: int) -> str:
    async with _user_language_lock:
        if user_id in _user_lang_cache_db:
            return _user_lang_cache_db[user_id]
    async def _get(conn):
    ...
    lang = await execute_db(_get)
    async with _user_language_lock:
        _user_lang_cache_db[user_id] = lang
...
async def load_all_languages():
    async def _load(conn):
        cur = await conn.execute("SELECT user_id, language FROM users WHERE language IS NOT NULL")
        rows = await cur.fetchall()
        for user_id, lang in rows:
            user_language[user_id] = lang
            _user_lang_cache_db[user_id] = lang
    await execute_db(_load)
    logger.info(f"✅ تم تحميل لغات {len(user_language)} مستخدم من قاعدة البيانات")
    ...
    fetch('/api/logs')
        ...
        .then(data => {
            ...
            tbody.innerHTML = data.map(log => {
                ...
                const levelClass = {'ERROR':'danger','WARNING':'warning','INFO':'info','DEBUG':'secondary'}[level] || 'info';
                return `
                    <tr>
                        <td>${log.time || '-'}</td>
...
async def api_posts_handler(request):
    try:
        ...
        result = []
        for post in posts:
            result.append({'id': post[0], 'channel_db_id': post[1], 'text': post[2], 'media_type': post[3], 'created_at': post[4], 'channel_id': post[5], 'channel_name': post[6]})
        return web.json_response(result)
    except Exception as e:
        return web.json_response({'error': str(e)}, status=500)
...
async def api_contests_handler(request):
    try:
        ...
        for contest in contests:
            result.append({'id': contest[0], 'title': contest[1], 'description': contest[2], 'prize': contest[3], 'end_date': contest[4], 'participants': contest[5] if len(contest) > 5 else 0, 'status': 'active'})
        return web.json_response(result)
    except Exception as e:
        return web.json_response({'error': str(e)}, status=500)
            ...
            with open(LOG_PATH, 'r', encoding='utf-8') as f:
                ...
                for line in lines:
                    try:
                        ...
                        if len(parts) >= 4:
                            logs.append({'time': parts[0], 'level': parts[1] if len(parts) > 1 else 'INFO', 'message': parts[-1]})
                        else:
...
async def api_backups_handler(request):
    try:
        ...
        for backup in backups:
            stat = backup.stat()
            result.append({'name': backup.name, 'size': stat.st_size, 'date': datetime.fromtimestamp(stat.st_mtime).isoformat()})
        return web.json_response(result)
    except Exception as e:
        return web.json_response({'error': str(e)}, status=500)
...
async def api_backup_create_handler(request):
    try:
        ...
        return web.json_response({'success': True, 'message': '✅ تم إنشاء نسخة احتياطية', 'file': backup_path.name})
    except Exception as e:
        return web.json_response({'error': str(e)}, status=500)
...
async def api_settings_update_handler(request):
    try:
        ...
        if 'publish_interval' in data:
            ...
            await db_set_publish_interval_seconds(seconds, MAIN_ADMIN_ID, is_admin=True)
    ...
    try:
        if check_web_auth(request):
            ...
            try:
                ...
                return web.Response(text="""<!DOCTYPE html><html><head><title>ريلاكس مانيجر</title></head><body><h1>🚀 ريلاكس مانيجر يعمل!</h1><p>الرجاء تسجيل الدخول</p></body></html>""", content_type='text/html')
        else:
...
async def db_register_user(user_id: int) -> bool:
    async def _register(conn):
        ...
        if await cur.fetchone():
            return False
        await conn.execute("INSERT INTO users (user_id, auto_publish, banned, trial_used, auto_reply_enabled, auto_recycle, language) VALUES (?, 1, 0, 0, 1, 1, 'ar')", (user_id,))
        await conn.commit()
        return True
...
async def db_get_user_groups(user_id: int):
    async def _get(conn):
        try:
            ...
            cur = await conn.execute("SELECT chat_id FROM hidden_admins WHERE admin_id=?", (user_id,))
...
async def db_get_user_groups_count(user_id: int) -> int:
    async def _get(conn):
        try:
            cur = await conn.execute("SELECT COUNT(DISTINCT chat_id) FROM bot_groups WHERE added_by=? OR chat_id IN (SELECT chat_id FROM user_groups_link WHERE user_id=?)", (user_id, user_id))
            total = (await cur.fetchone())[0]
            cur = await conn.execute("SELECT COUNT(DISTINCT chat_id) FROM hidden_admins WHERE admin_id=?", (user_id,))
            hidden_admin_count = (await cur.fetchone())[0]
            cur = await conn.execute("SELECT COUNT(DISTINCT chat_id) FROM hidden_owner_groups WHERE owner_id=?", (user_id,))
            hidden_owner_count = (await cur.fetchone())[0]
            visible_count = total - (hidden_admin_count - hidden_owner_count)
...
async def db_get_all_hidden_admins(user_id: int) -> List[Dict]:
    async def _get(conn):
        cur = await conn.execute("SELECT chat_id, added_at FROM hidden_admins WHERE admin_id=?", (user_id,))
        rows = await cur.fetchall()
        ...
        async def _get(conn):
            ...
            try:
                ...
                for row in rows:
                    try:
                        if hasattr(row, 'keys'):
                            result.append((row['id'], row['title'], row['description'], row['prize'], row['end_date'], row['participants'], row['contest_type'] if 'contest_type' in row else 'raffle'))
                        else:
...
async def auto_grade_contest(contest_id: int, answer_key: str) -> dict:
    async def _get_participants(conn):
        cur = await conn.execute("SELECT user_id, answer FROM contest_participants WHERE contest_id = ?", (contest_id,))
        return await cur.fetchall()
    ...
    async def _get_stats(conn):
        ...
        if not row or row[0] == 0:
            return {
                ...
                'last_post_time': None, 'first_post_time': None,
        ...
        async def _merge_data(conn):
            ...
            if 'users' in data:
                for user in data['users']:
                    await conn.execute("INSERT OR IGNORE INTO users (user_id, auto_publish, banned, trial_used, subscription_end, referral_code, referred_by, active_channel, auto_reply_enabled, auto_recycle, language) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (user['user_id'], user['auto_publish'], user['banned'], user['trial_used'], user['subscription_end'], user['referral_code'], user['referred_by'], user['active_channel'], user['auto_reply_enabled'], user['auto_recycle'], user.get('language', 'ar')))
            await conn.commit()
...
async def execute_ban(bot, chat_id: int, user_id: int, until_date=None, reason: str = "", moderator_id: int = None):
    try:
        ...
        async def _log(conn):
            await conn.execute("INSERT INTO moderation_log (chat_id, user_id, action, duration_minutes, moderator_id, reason, created_at) VALUES (?, ?, 'ban', 0, ?, ?, ?)", (chat_id, user_id, moderator_id or MAIN_ADMIN_ID, reason[:200] if reason else "", utc_now_iso()))
            await conn.commit()
...
async def execute_mute(bot, chat_id: int, user_id: int, duration_minutes: int = None, reason: str = "", moderator_id: int = None):
    try:
        ...
        async def _log(conn):
            await conn.execute("INSERT INTO moderation_log (chat_id, user_id, action, duration_minutes, moderator_id, reason, created_at) VALUES (?, ?, 'mute', ?, ?, ?, ?)", (chat_id, user_id, duration_minutes, moderator_id or MAIN_ADMIN_ID, reason[:200] if reason else "", utc_now_iso()))
            await conn.commit()
        await execute_db(_log)
...
async def execute_kick(bot, chat_id: int, user_id: int, reason: str = "", moderator_id: int = None):
    try:
        ...
        async def _log(conn):
            await conn.execute("INSERT INTO moderation_log (chat_id, user_id, action, duration_minutes, moderator_id, reason, created_at) VALUES (?, ?, 'kick', 0, ?, ?, ?)", (chat_id, user_id, moderator_id or MAIN_ADMIN_ID, reason[:200] if reason else "", utc_now_iso()))
            await conn.commit()
        await execute_db(_log)
...
async def execute_restrict(bot, chat_id: int, user_id: int, reason: str = "", moderator_id: int = None):
    try:
        ...
        async def _log(conn):
            await conn.execute("INSERT INTO moderation_log (chat_id, user_id, action, duration_minutes, moderator_id, reason, created_at) VALUES (?, ?, 'restrict', 0, ?, ?, ?)", (chat_id, user_id, moderator_id or MAIN_ADMIN_ID, reason[:200] if reason else "", utc_now_iso()))
            await conn.commit()
        await execute_db(_log)
...
async def execute_unban(bot, chat_id: int, user_id: int, moderator_id: int = None):
    try:
        await bot.unban_chat_member(chat_id, user_id)
        async def _log(conn):
            await conn.execute("INSERT INTO moderation_log (chat_id, user_id, action, duration_minutes, moderator_id, reason, created_at) VALUES (?, ?, 'unban', 0, ?, ?, ?)", (chat_id, user_id, moderator_id or MAIN_ADMIN_ID, "", utc_now_iso()))
            await conn.commit()
        await execute_db(_log)
...
def get_replies_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ إضافة رد", callback_data=CallbackData.ADMIN_ADD_REPLY), InlineKeyboardButton("📋 عرض الردود", callback_data=CallbackData.ADMIN_LIST_REPLIES)],
        [InlineKeyboardButton("🗑️ حذف رد", callback_data=CallbackData.ADMIN_DEL_REPLY), InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_PANEL)]
    ])

def get_group_banned_words_keyboard(chat_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ إضافة كلمة", callback_data=f"{CallbackData.BANNED_WORDS_ADD_PREFIX}{chat_id}"), InlineKeyboardButton("📋 عرض الكلمات", callback_data=f"{CallbackData.BANNED_WORDS_LIST_PREFIX}{chat_id}")],
        [InlineKeyboardButton("🗑️ حذف كلمة", callback_data=f"{CallbackData.BANNED_WORDS_REMOVE_PREFIX}{chat_id}"), InlineKeyboardButton("🔙 رجوع", callback_data=f"{CallbackData.GROUPS_SETTINGS_PREFIX}{chat_id}")]
    ])

def get_banned_words_admin_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ إضافة كلمة عامة", callback_data=CallbackData.ADMIN_ADD_BANNED_WORD), InlineKeyboardButton("📋 عرض الكلمات", callback_data=CallbackData.ADMIN_LIST_BANNED_WORDS)],
        [InlineKeyboardButton("🗑️ حذف كلمة", callback_data=CallbackData.ADMIN_REMOVE_BANNED_WORD), InlineKeyboardButton("🔙 رجوع", callback_data=CallbackData.ADMIN_PANEL)]
    ])

def get_advanced_group_actions_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🛑 حظر", callback_data=f"{CallbackData.GROUP_ACTION_BAN}:{chat_id}"), InlineKeyboardButton("🔇 كتم", callback_data=f"{CallbackData.GROUP_ACTION_MUTE}:{chat_id}")],
        [InlineKeyboardButton("⚠️ تحذير", callback_data=f"{CallbackData.GROUP_ACTION_WARN}:{chat_id}"), InlineKeyboardButton("👢 طرد", callback_data=f"{CallbackData.GROUP_ACTION_KICK}:{chat_id}")],
        [InlineKeyboardButton("🔒 تقييد", callback_data=f"{CallbackData.GROUP_ACTION_RESTRICT}:{chat_id}"), InlineKeyboardButton("📌 تثبيت", callback_data=f"{CallbackData.GROUP_ACTION_PIN}:{chat_id}")],
        [InlineKeyboardButton("🔓 إلغاء حظر", callback_data=f"{CallbackData.GROUP_ACTION_UNBAN}:{chat_id}"), InlineKeyboardButton("🔙 رجوع", callback_data=f"{CallbackData.GROUPS_SETTINGS_PREFIX}{chat_id}")]
    ])

def get_advanced_mute_duration_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⏱️ 5 دقائق", callback_data=f"adv_mute_duration:5:{chat_id}"), InlineKeyboardButton("⏱️ 30 دقيقة", callback_data=f"adv_mute_duration:30:{chat_id}")],
        [InlineKeyboardButton("⏱️ 1 ساعة", callback_data=f"adv_mute_duration:60:{chat_id}"), InlineKeyboardButton("⏱️ 12 ساعة", callback_data=f"adv_mute_duration:720:{chat_id}")],
        [InlineKeyboardButton("📆 يوم", callback_data=f"adv_mute_duration:1440:{chat_id}"), InlineKeyboardButton("📆 أسبوع", callback_data=f"adv_mute_duration:10080:{chat_id}")],
        [InlineKeyboardButton("🔇 كتم دائم", callback_data=f"adv_mute_duration:0:{chat_id}"), InlineKeyboardButton("🔙 رجوع", callback_data=f"{CallbackData.ADVANCED_ACTIONS}:{chat_id}")]
    ])

def get_admin_keyboard(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(get_text(user_id, 'admin_users'), callback_data=CallbackData.ADMIN_USERS), InlineKeyboardButton(get_text(user_id, 'admin_banned'), callback_data=CallbackData.ADMIN_BANNED_USERS)],
        [InlineKeyboardButton(get_text(user_id, 'admin_channels'), callback_data=CallbackData.ADMIN_ALL_CHANNELS), InlineKeyboardButton("⛔ قنوات محظورة", callback_data=CallbackData.ADMIN_BANNED_CHANNELS)],
...
def penalty_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔴 طرد", callback_data=f"{CallbackData.PENALTY_KICK}:{chat_id}"), InlineKeyboardButton("🛑 حظر", callback_data=f"{CallbackData.PENALTY_BAN}:{chat_id}")],
        [InlineKeyboardButton("🔇 كتم", callback_data=f"{CallbackData.PENALTY_MUTE}:{chat_id}"), InlineKeyboardButton("🔙 رجوع", callback_data=f"{CallbackData.GROUPS_SETTINGS_PREFIX}{chat_id}")]
    ])

def mute_duration_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⏱️ 5 دقائق", callback_data=f"{CallbackData.GROUP_MUTE_DURATION_5}:{chat_id}"), InlineKeyboardButton("⏱️ 30 دقيقة", callback_data=f"{CallbackData.GROUP_MUTE_DURATION_30}:{chat_id}")],
        [InlineKeyboardButton("⏱️ 1 ساعة", callback_data=f"{CallbackData.GROUP_MUTE_DURATION_60}:{chat_id}"), InlineKeyboardButton("⏱️ 12 ساعة", callback_data=f"{CallbackData.GROUP_MUTE_DURATION_720}:{chat_id}")],
        [InlineKeyboardButton("📆 يوم", callback_data=f"{CallbackData.GROUP_MUTE_DURATION_1440}:{chat_id}"), InlineKeyboardButton("📆 أسبوع", callback_data=f"{CallbackData.GROUP_MUTE_DURATION_10080}:{chat_id}")],
        [InlineKeyboardButton("🔇 كتم دائم", callback_data=f"{CallbackData.GROUP_MUTE_DURATION_PERMANENT}:{chat_id}"), InlineKeyboardButton("🔙 رجوع", callback_data=f"{CallbackData.PENALTY_MENU}:{chat_id}")]
    ])
...
async def get_main_keyboard(user_id: int):
    ...
    is_admin = False
    try:
        is_admin = (user_id == MAIN_ADMIN_ID) or (await is_bot_admin(user_id))
    except:
        is_admin = False
    if is_admin:
        keyboard.append([
        ...
        if state == UserState.WAITING_CONTEST_TITLE:
            ...
            try:
                ...
                if contest_id:
                    ...
                    try:
                        await context.bot.send_message(MAIN_ADMIN_ID, get_text(MAIN_ADMIN_ID, 'contest_creator').format(user_id, title))
                    except:
                        pass
                else:
...
• 🌙 **وضع Dark Mode**
• ⏱️ **جدولة CRON**
• 🌐 **دعم DeepL API** مع Google Translate كخيار احتياطي
...
async def lang_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lang:
        ...
        return
    await set_user_language_db(uid, lang)
    lang_names = {'ar': 'العربية 🇸🇦', 'en': 'English 🇬🇧', 'fr': 'Français 🇫🇷', 'tr': 'Türkçe 🇹🇷', 'zh': '中文 🇨🇳', 'ru': 'Русский 🇷🇺', 'de': 'Deutsch 🇩🇪', 'es': 'Español 🇪🇸', 'it': 'Italiano 🇮🇹', 'pt': 'Português 🇵🇹', 'ja': '日本語 🇯🇵', 'ko': '한국어 🇰🇷'}
    lang_name = lang_names.get(lang, lang)
    kb, title, active_channel = await get_main_keyboard(uid)
    if active_channel:
...
async def syncgroup_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تفعيل المجموعة وتسجيلها مع المالك المخفي"""
    if update.effective_chat.type not in ['group', 'supergroup']:
        await update.message.reply_text("⚠️ هذا الأمر يعمل فقط في المجموعات!")
        return
    chat_id = update.effective_chat.id
    ...
    if not bot_perms['can_act']:
        ...
        return
    # إذا كان المستخدم مشرفاً، سجل مالكاً مخفياً
...
async def claim_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تسجيل المستخدم كمالك مخفي في المجموعة الحالية"""
    chat = update.effective_chat
    if chat.type not in ['group', 'supergroup']:
        await update.message.reply_text("⚠️ هذا الأمر يعمل فقط في المجموعات!")
        return
    ...
    try:
        ...
        if member.status not in ['creator', 'administrator']:
            await update.message.reply_text(get_text(user_id, 'claim_fail'))
            return
    except:
    ...
    # سجل المالك المخفي
    await db_register_hidden_owner_group(chat_id, user_id)
    await update.message.reply_text(get_text(user_id, 'claim_success'))
...
async def reload_translations_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """إعادة تحميل الترجمات من الملف الخارجي (للمشرفين فقط)"""
    user_id = update.effective_user.id
    if user_id != MAIN_ADMIN_ID and not await is_bot_admin(user_id):
        await update.message.reply_text("🔒 هذا الأمر للمشرفين فقط!")
        return
    success = await translator.reload()
    if success:
        await update.message.reply_text(get_text(user_id, 'reload_translations_success'))
    else:
        await update.message.reply_text(get_text(user_id, 'reload_translations_fail'))
...
async def auto_register_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.message.new_chat_members:
        if member.id == context.bot.id:
            ...
            # تسجيل المجموعة
            async def _register(conn):
                await conn.execute("INSERT OR IGNORE INTO bot_groups (chat_id, chat_name, added_by, added_at) VALUES (?, ?, ?, ?)",
                                   (chat_id, chat_name, adder_id, utc_now_iso()))
                await conn.commit()
            await execute_db(_register)
            # تسجيل المضيف كمالك مخفي إذا كان مشرفاً
            try:
                member_status = await context.bot.get_chat_member(chat_id, adder_id)
                if member_status.status in ['creator', 'administrator']:
                    async def _add_owner(conn):
                        await conn.execute("INSERT OR IGNORE INTO hidden_owner_groups (chat_id, owner_id) VALUES (?, ?)",
                                           (chat_id, adder_id))
                        await conn.commit()
                    await execute_db(_add_owner)
                    try:
                        await context.bot.send_message(adder_id, get_text(adder_id, 'group_added').format(chat_name))
                    except:
                        pass
                else:
                    try:
                        await context.bot.send_message(adder_id, f"✅ تم إضافة البوت إلى مجموعة {chat_name}\nلتصبح مالكاً مخفياً، استخدم /claim في المجموعة.")
                    except:
                        pass
            except:
                pass
...
async def group_msg_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة الرسائل في المجموعات (حماية، ردود تلقائية)"""
    chat = update.effective_chat
    if chat.type not in ['group', 'supergroup']:
        return
    user = update.effective_user
    ...
    settings = await db_get_security_settings(chat_id)
    lock_links = settings.get('links', False)
    lock_mentions = settings.get('mentions', False)
    slow_mode = settings.get('slow_mode', False)
    slow_mode_seconds = settings.get('slow_mode_seconds', 5)
    warn_message = settings.get('warn', True)
    delete_banned_words = settings.get('delete_banned_words', False)
    ...
    # حظر الروابط
    if lock_links and contains_link(text):
        try:
            await update.message.delete()
        except:
            pass
    ...
    # حظر الإشارات
    if lock_mentions and contains_mention(text):
        try:
            await update.message.delete()
        except:
            pass
    ...
    # ردود تلقائية
    # 1. ردود مخصصة من قاعدة البيانات
    ...
    # 2. ردود مدمجة (200 رد)
    if text.lower() in ALL_REPLIES:
        await update.message.reply_text(ALL_REPLIES[text.lower()])
        return
...
async def private_msg_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("translate_mode"):
        try:
            ...
            await update.message.reply_text("❌ فشل الترجمة")
    ...
    if state == UserState.ADDING_POSTS:
        ...
        posts.append((post_text, media_type, media_file_id))
        context.user_data[session_key] = posts
        target = context.user_data.get(f"session_target_{user_id}", 15)
        if len(posts) >= target or len(posts) >= MAX_POSTS_PER_SESSION:
    ...
    # الجدولة (دقائق، ساعات، أيام، تواريخ، وقت)
    ...
    if state == UserState.WAITING_DATES:
        ...
        await db_set_next_publish_date(ch_db_id, None)
        await update.message.reply_text(get_text(user_id, 'days_saved'))
        await schedule_menu_callback(update, context)
        return
    ...
    if state == UserState.WAITING_SENDCODE_USER:
        ...
        await security_audit.log("SENDCODE_PERMISSION_GRANTED", user_id, {"target": target_user_id}, "CRITICAL")
        await update.message.reply_text(get_text(user_id, 'sendcode_user_set').format(target_user_id), parse_mode="MarkdownV2")
        await admin_panel_callback(update, context)
        return
    ...
    if state == UserState.WAITING_ADMIN_ID_ADD:
        try:
            ...
            if target_id == MAIN_ADMIN_ID:
                ...
                await security_audit.log("ADMIN_ADDED", user_id, {"target": target_id}, "CRITICAL")
                await update.message.reply_text(get_text(user_id, 'add_admin_success').format(target_id), parse_mode="MarkdownV2")
        except ValueError:
    ...
    if state == UserState.WAITING_ADMIN_ID_REMOVE:
        try:
            ...
            if target_id == MAIN_ADMIN_ID:
                ...
                await security_audit.log("ADMIN_REMOVED", user_id, {"target": target_id}, "CRITICAL")
                await update.message.reply_text(get_text(user_id, 'remove_admin_success').format(target_id), parse_mode="MarkdownV2")
        except ValueError:
            await update.message.reply_text(get_text(user_id, 'invalid_user_id'))
        context.user_data.pop('state', None)
        await admin_panel_callback(update, context)
    ...
    # دعم التذاكر
    if context.user_data.get('support_mode') and text and not text.startswith('/'):
        ticket_num = await db_get_next_ticket_number()
        username = update.effective_user.full_name or update.effective_user.first_name or str(user_id)
    ...
    try:
        ...
        if isinstance(error, Forbidden):
            ...
            if update and update.effective_chat:
                try:
                    await context.bot.send_message(chat_id=MAIN_ADMIN_ID, text=f"⚠️ **البوت محظور أو ليس لديه صلاحيات في:**\n{update.effective_chat.title}\nID: `{update.effective_chat.id}`")
                except:
                    pass
            return
        ...
        if update and update.effective_user and context and context.bot:
            try:
                ...
                try:
                    ...
                    pass
        if MAIN_ADMIN_ID and context and context.bot:
            try:
            ...
            for contest_id in expired:
                ...
                if winner_id:
                    ...
                    try:
                        ...
                        pass
                    await bot.send_message(MAIN_ADMIN_ID, f"🤖 تم إغلاق المسابقة #{contest_id} تلقائياً.\nالفائز: {winner_id}")
                else:
    ...
    async with aiosqlite.connect(str(DB_PATH), timeout=DB_TIMEOUT) as conn:
        ...
        try:
            ...
            if 'language' not in column_names:
                await conn.execute("ALTER TABLE users ADD COLUMN language TEXT DEFAULT 'ar'")
        except:
            pass
        try:
    ...
    try:
        ...
        if words:
            async def _import(conn):
                ...
                for word in words:
                    try:
                        await conn.execute("INSERT OR IGNORE INTO banned_words (word, chat_id, added_by, added_at) VALUES (?, ?, ?, ?)", (word, -1, MAIN_ADMIN_ID, utc_now_iso()))
                        imported += 1
                    except:
                        continue
                await conn.commit()
...
async def main():
    ...
    application.add_handler(CommandHandler("claim", claim_command_handler))
    ...
    application.add_handler(CommandHandler("reload_translations", reload_translations_command_handler))
    
    # معالج إضافة البوت للمجموعة
    application.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, auto_register_group))
    ...
    application.add_handler(MessageHandler(filters.TEXT & filters.ChatType.GROUPS & ~filters.COMMAND, group_msg_filter))
    application.add_handler(MessageHandler(filters.CAPTION & filters.ChatType.GROUPS & ~filters.COMMAND, group_msg_filter))
    application.add_handler(MessageHandler(filters.TEXT & filters.ChatType.PRIVATE & ~filters.COMMAND, private_msg_handler))
    application.add_handler(MessageHandler(filters.PHOTO & filters.ChatType.PRIVATE, private_msg_handler))
    application.add_handler(MessageHandler(filters.VIDEO & filters.ChatType.PRIVATE, private_msg_handler))
    application.add_handler(MessageHandler(filters.AUDIO & filters.ChatType.PRIVATE, private_msg_handler))
    application.add_handler(MessageHandler(filters.VOICE & filters.ChatType.PRIVATE, private_msg_handler))
    application.add_handler(MessageHandler(filters.ANIMATION & filters.ChatType.PRIVATE, private_msg_handler))
    ...
    commands = [
        ...
        BotCommand("reload_translations", "إعادة تحميل الترجمات"),
    ]
    ...
    print("   • ✅ تحسين نظام الترجمة (12 لغة مع DeepL)")
    ...
    print("   • ✅ إصلاح دالة الترجمة لتدعم جميع اللغات 12 لغة مع DeepL و Google Translate")
    print("   • ✅ حفظ لغة المستخدم في قاعدة البيانات")
    print("   • ✅ إضافة أمر /claim لتسجيل مالك مخفي")
    print("   • ✅ إضافة أمر /reload_translations لتحديث الترجمات")
    print("   • ✅ إصلاح استجابة المجموعة للأوامر (تسجيل المجموعة تلقائياً)")
